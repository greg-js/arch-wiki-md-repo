...
**Причина:** rc.d references. Needs update, see [Systemd (Русский)](/index.php/Systemd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Systemd (Русский)"). ([Обсудить](https://wiki.archlinux.org/index.php/Talk:Tor_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)))
...
Пакет [arm](https://www.archlinux.org/packages/?name=arm) (Anonymizing Relay Monitor) представляет собой консольный монитор состояния использования пропускной способности сети, сведения о соединении и другое. Вы также можете использовать для настройки и управление графический клиент [vidalia](https://aur.archlinux.org/packages/vidalia/)<sup><small>AUR</small></sup> - это Qt GUI для Tor. Кроме конфигурирования и контроля над процессом работы вы также получаете возможность видеть статус сети Tor, монитор трафика, просматривать фильтровать и искать логи.

## Настройка

По умолчанию, Tor читает конфигурацию из файла `/etc/tor/torrc`. Опции подробно расписаны в `man tor` и на [сайте проекта](https://www.torproject.org/docs/tor-manual.html.en). Конфигурационный файл по умолчанию подойдет для большинства пользователей.
...
`--network-macvlan=$INTERFACE --private-network` автоматически создаст macvlan называющийся `mv-$INTERFACE` внутри контейнера, который невидим с хоста. `--private-network` подразумевает `--network-macvlan=` в соответсвии с `man systemd-nspawn`.
...
Retrieved from "[https://wiki.archlinux.org/index.php?title=Tor_(Русский)&oldid=414964](https://wiki.archlinux.org/index.php?title=Tor_(Русский)&oldid=414964)"
...
*   [Proxy servers (Русский)](/index.php/Category:Proxy_servers_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Category:Proxy servers (Русский)")
