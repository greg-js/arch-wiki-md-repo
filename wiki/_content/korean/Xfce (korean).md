...
*   [3 설정하기](#설정하기)
    *   [3.1 메뉴](#메뉴)
        *   [3.1.1 Whisker 메뉴](#Whisker_메뉴)
        *   [3.1.2 메뉴 편집](#메뉴_편집)
    *   [3.2 바탕화면](#바탕화면)
        *   [3.2.1 투명한 아이콘 제목 배경색](#투명한_아이콘_제목_배경색)
        *   [3.2.2 오른쪽 마우스 클릭 메뉴에서 Thunar 옵션 제거](#오른쪽_마우스_클릭_메뉴에서_Thunar_옵션_제거)
        *   [3.2.3 프로그램 강제 종료 단축키](#프로그램_강제_종료_단축키)
    *   [3.3 세션](#세션)
        *   [3.3.1 시작 프로그램](#시작_프로그램)
            *   [3.3.1.1 시작 프로그램 지연 설정](#시작_프로그램_지연_설정)
        *   [3.3.2 화면 잠금](#화면_잠금)
        *   [3.3.3 사용자 전환 버튼](#사용자_전환_버튼)
        *   [3.3.4 저장된 세션 비활성화하기](#저장된_세션_비활성화하기)
        *   [3.3.5 기본 창 관리자](#기본_창_관리자)
    *   [3.4 테마](#테마)
    *   [3.5 사운드](#사운드)
        *   [3.5.1 Xfce4 mixer](#Xfce4_mixer)
            *   [3.5.1.1 Xfce4 mixer의 기본 사운드카드 변경](#Xfce4_mixer의_기본_사운드카드_변경)
        *   [3.5.2 키보드의 볼륨 조절 버튼](#키보드의_볼륨_조절_버튼)
            *   [3.5.2.1 단축키](#단축키)
    *   [3.6 키보드 단축키](#키보드_단축키)
    *   [3.7 Polkit Authentication Agent](#Polkit_Authentication_Agent)
    *   [3.8 화면에 아무것도 나오지 않을 경우](#화면에_아무것도_나오지_않을_경우)
*   [4 사용관련 도움말](#사용관련_도움말)
    *   [4.1 thunar와 xfdesktop에서 파티션 숨기기](#thunar와_xfdesktop에서_파티션_숨기기)
    *   [4.2 스크린샷(화면 갈무리)](#스크린샷(화면_갈무리))
    *   [4.3 터미널 상에서 F1 and F11 단축키 비활성화](#터미널_상에서_F1_and_F11_단축키_비활성화)
    *   [4.4 터미널 색 테마, 팔레트](#터미널_색_테마,_팔레트)
        *   [4.4.1 기본 색 테마 변경하기](#기본_색_테마_변경하기)
        *   [4.4.2 터미널 tango 색 테마](#터미널_tango_색_테마)
    *   [4.5 색 관리](#색_관리)
    *   [4.6 다중 모니터 사용](#다중_모니터_사용)
    *   [4.7 SSH agents](#SSH_agents)
    *   [4.8 포커스 변경 없이 다른 창 스크롤하기](#포커스_변경_없이_다른_창_스크롤하기)
    *   [4.9 마우스 버튼 modifier키](#마우스_버튼_modifier키)
*   [5 기타 문제 해결](#기타_문제_해결)
    *   [5.1 Action 버튼에 아이콘이 없는 경우](#Action_버튼에_아이콘이_없는_경우)
    *   [5.2 데스크탑 아이콘 재정렬 문제](#데스크탑_아이콘_재정렬_문제)
    *   [5.3 다중 모니터에서 GTK 테마가 정상적으로 작동하지 않는 문제](#다중_모니터에서_GTK_테마가_정상적으로_작동하지_않는_문제)
    *   [5.4 오른쪽 마우스 클릭 메뉴에서 아이콘이 나타나지 않을 때](#오른쪽_마우스_클릭_메뉴에서_아이콘이_나타나지_않을_때)
    *   [5.5 xkb-plugin에서 키보드 설정이 저장되지 않는 문제](#xkb-plugin에서_키보드_설정이_저장되지_않는_문제)
    *   [5.6 NVIDIA와 xfce4-sensors-plugin](#NVIDIA와_xfce4-sensors-plugin)
    *   [5.7 패널이 항상 왼쪽으로 정렬되어 있는 문제](#패널이_항상_왼쪽으로_정렬되어_있는_문제)
    *   [5.8 Preferred Applications 설정 문제](#Preferred_Applications_설정_문제)
    *   [5.9 기본 설정 복구](#기본_설정_복구)
    *   [5.10 세션 실패](#세션_실패)
    *   [5.11 창 제목의 폰트로 xfce4-title가 깨지는 문제](#창_제목의_폰트로_xfce4-title가_깨지는_문제)
    *   [5.12 노트북 덮개 설정 문제](#노트북_덮개_설정_문제)
    *   [5.13 Adwaita 테마에서의 렌더링 문제](#Adwaita_테마에서의_렌더링_문제)
*   [6 참고](#참고)
...
그 후에 다시 로그인하면 기본설정으로 복구된 것을 확인할 수 있다. 만약에 로그인 도중 `Unable to load a failsafe session` 메세지가 출력되었다면 [#세션 실패](#세션_실패)를 참고한다.
