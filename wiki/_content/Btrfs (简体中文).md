...
*   [btrfs-progs-git](https://aur.archlinux.org/packages/btrfs-progs-git/)<sup><small>AUR</small></sup> 每日构建
...
确认没有问题后,完成转换通过删除备份`ext2_saved`子卷. Note that you cannot revert back to ext3/4 without it.
...
Btrfs 不支持交换文件，因为 Btrfs 因为潜在的文件系统损坏风险，没有加入交换文件需要的功能，参阅[这里](https://btrfs.wiki.kernel.org/index.php/FAQ#Does_btrfs_support_swap_files.3F). 交换文件可以挂载到 loop 设备中，但是性能比较差。[systemd-loop-swapfile](https://aur.archlinux.org/packages/systemd-loop-swapfile/)<sup><small>AUR</small></sup><sup>[[broken link](/index.php/ArchWiki:Requests#Broken_package_links "ArchWiki:Requests"): archived in [aur-mirror](http://pkgbuild.com/git/aur-mirror.git/tree/systemd-loop-swapfile)]</sup>提供了需要的服务文件。
...
在版本 3.14.12_rt9中, [linux-rt](/index.php/Kernel#-rt "Kernel") 内核不能引导Btrfs文件系统. This is due to the slow development of the _rt_ patchset.
...
当创建_btrfs_文件系统时，你可以将任意个分区或磁盘设备传给_mkfs.btrfs_。创建的文件系统将跨这些设备。你可以按这种办法**"**合并**"**多个分区或设备来得到一个大_btrfs_文件系。
...
**This article or section is a candidate for merging with [Solid State Drives](/index.php/Solid_State_Drives "Solid State Drives").**

**Notes:** General information, common feature. (Discuss in [Talk:Btrfs (简体中文)#](https://wiki.archlinux.org/index.php/Talk:Btrfs_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87)))
...
Retrieved from "[https://wiki.archlinux.org/index.php?title=Btrfs_(简体中文)&oldid=411183](https://wiki.archlinux.org/index.php?title=Btrfs_(简体中文)&oldid=411183)"

[Category](/index.php/Special:Categories "Special:Categories"):

*   [File systems (简体中文)](/index.php/Category:File_systems_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87) "Category:File systems (简体中文)")