...
*   **pkgdesc:** Здесь должно быть краткое описание пакета, обычно не более 76 символов. Учтите, что _краткость - сестра таланта_: `OpenGL accelerated X server` лучше чем `xgl is a OpenGL...`.

*   **arch:** Здесь должен быть список архитектур, где может быть использован данный PKGBUILD (обычно это "i686"). Значение данной переменной будет записано в `$arch` и может быть использованно далее в процессе сборки.
...
Например, в зависимости от имени распакованной makepkg директории, первая комманда в функции `build()` может быть `cd $srсdir/$pkgname-$pkgver`, что бывает очень часто, если только автор программы не очень-очень злой человек.
...
Теперь, когда вы находитесь в нужной директории, вы должны выяснить, какие необходимы команды для сборки пакета. В самых простых случаях можно использовать `./configure; make`, хотя существуют десятки разновидностей, включая `ant build` либо использование комманд `gcc` для компиляции.
...
Как вы наверно уже догадались, это часть, где преобретенные знания и опыт становятся необходимыми. Очень полезным оказывается просматривание `PKGBUILD`ов в дереве ABS, так как они работают и содержат некоторые трюки, которые могут оказаться полезными.
...
Retrieved from "[https://wiki.archlinux.org/index.php?title=Creating_packages_(Русский)&oldid=411564](https://wiki.archlinux.org/index.php?title=Creating_packages_(Русский)&oldid=411564)"
