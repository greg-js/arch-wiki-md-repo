...
*   [2 Configurazione](#Configurazione)
    ...
    *   [2.3 Monitor multipli](#Monitor_multipli)
        ...
        *   [2.3.4 Modalità Mosaico](#Modalità_Mosaico)
...
*   [3 Tweaking](#Tweaking)
    ...
    *   [3.2 Avanzato: 20-nvidia.conf](#Avanzato:_20-nvidia.conf)
        ...
        *   [3.2.2 Disabiliare visione logo all'avvio](#Disabiliare_visione_logo_all'avvio)
        ...
        *   [3.2.8 Abilitare il controllo della luminosità](#Abilitare_il_controllo_della_luminosità)
        *   [3.2.9 Abilitare la modalità SLI](#Abilitare_la_modalità_SLI)
        *   [3.2.10 Forzare il livello di performance Powermizer (per portatili)](#Forzare_il_livello_di_performance_Powermizer_(per_portatili))
            *   [3.2.10.1 Lasciare che la GPU imposti automaticamente il livello di performance (basato sulla temperatura)](#Lasciare_che_la_GPU_imposti_automaticamente_il_livello_di_performance_(basato_sulla_temperatura))
        *   [3.2.11 Disabilitare i *vblank interrupts* (per portatili)](#Disabilitare_i_vblank_interrupts_(per_portatili))
        *   [3.2.12 Abilitare l'overclocking](#Abilitare_l'overclocking)
            *   [3.2.12.1 Impostare un clock statico 2D/3D](#Impostare_un_clock_statico_2D/3D)
*   [4 Trucchi e consigli](#Trucchi_e_consigli)
    ...
    *   [4.2 Abilitare Video HD (VDPAU/VAAPI)](#Abilitare_Video_HD_(VDPAU/VAAPI))
    *   [4.3 Evitare screen tearing in KDE (KWin)](#Evitare_screen_tearing_in_KDE_(KWin))
    *   [4.4 Accelerazione hardware con il codec video XvMC](#Accelerazione_hardware_con_il_codec_video_XvMC)
    *   [4.5 Usare l'uscita TV](#Usare_l'uscita_TV)
    *   [4.6 X con la TV (DFP) come unico display](#X_con_la_TV_(DFP)_come_unico_display)
    *   [4.7 Controllare l'alimentazione](#Controllare_l'alimentazione)
    ...
    *   [4.9 Impostare la velocità della ventola al Login](#Impostare_la_velocità_della_ventola_al_Login)
    *   [4.10 Ordine di installazione/disinstallazione al cambiamento dei driver](#Ordine_di_installazione/disinstallazione_al_cambiamento_dei_driver)
...
*   [5 Risoluzione dei problemi](#Risoluzione_dei_problemi)
    *   [5.1 Performance scadenti, es. ridisegno lento quando si passa tra le schede in Chrome](#Performance_scadenti,_es._ridisegno_lento_quando_si_passa_tra_le_schede_in_Chrome)
    ...
    *   [5.5 Schermata corrotta: il problema dei "Sei schermi"](#Schermata_corrotta:_il_problema_dei_"Sei_schermi")
    *   [5.6 Errore '/dev/nvidia0' Input/Output](#Errore_'/dev/nvidia0'_Input/Output)
    *   [5.7 Errori '/dev/nvidiactl'](#Errori_'/dev/nvidiactl')
    *   [5.8 Le applicazioni 32 bit non si avviano](#Le_applicazioni_32_bit_non_si_avviano)
    *   [5.9 Errori dopo l'aggiornamento del kernel](#Errori_dopo_l'aggiornamento_del_kernel)
    ...
    *   [5.13 Per i laptop: X si blocca durante il login/out, ovviato con Ctrl+Alt+Backspace](#Per_i_laptop:_X_si_blocca_durante_il_login/out,_ovviato_con_Ctrl+Alt+Backspace)
    ...
    *   [5.16 Schermo(i) trovati, ma nessuna configurazione utilizzabile](#Schermo(i)_trovati,_ma_nessuna_configurazione_utilizzabile)
    *   [5.17 Nessun controllo della luminosità sui portatili](#Nessun_controllo_della_luminosità_sui_portatili)
    ...
    *   [5.26 X fallisce con "no screens found" una Intel iGPU](#X_fallisce_con_"no_screens_found"_una_Intel_iGPU)
    *   [5.27 Xorg non fallisce durante l'avvio , ma il sistema si avvia comunque](#Xorg_non_fallisce_durante_l'avvio_,_ma_il_sistema_si_avvia_comunque)
...
*   Per schede GeForce 8/9, ION e serie 100-300 [NV5x, NV8x, NV9x and NVAx] degli anni 2006-2010, [installa](/index.php/Install "Install") il pacchetto [nvidia-340xx](https://aur.archlinux.org/packages/nvidia-340xx/) o [nvidia-340xx-lts](https://aur.archlinux.org/packages/nvidia-340xx-lts/).
