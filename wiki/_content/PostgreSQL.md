...
**Note:** Commands that should be run as the postgres user are prefixed by `[postgres]$` in this article. You can change to the postgres user by running `su - postgres` as root. Alternatively, if you use [sudo](/index.php/Sudo "Sudo"), run `sudo -i -u postgres` as a regular user.
...
This is for upgrading from 9.2 to 9.3, and also works from 9.3 to 9.4.
