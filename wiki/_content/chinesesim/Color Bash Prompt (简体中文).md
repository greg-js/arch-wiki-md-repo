...
*   [1 一个完善的Bash提示符色彩设置](#.E4.B8.80.E4.B8.AA.E5.AE.8C.E5.96.84.E7.9A.84Bash.E6.8F.90.E7.A4.BA.E7.AC.A6.E8.89.B2.E5.BD.A9.E8.AE.BE.E7.BD.AE)
*   [2 安装](#.E5.AE.89.E8.A3.85)
    *   [2.1 /etc/bash.bashrc](#.2Fetc.2Fbash.bashrc)
    *   [2.2 /etc/DIR_COLORS](#.2Fetc.2FDIR_COLORS)
    *   [2.3 /etc/skel/](#.2Fetc.2Fskel.2F)
*   [3 Example of cohabitation of /etc/bash.bashrc and ~/.bashrc](#Example_of_cohabitation_of_.2Fetc.2Fbash.bashrc_and_.7E.2F.bashrc)
*   [4 Random quotations at logon](#Random_quotations_at_logon)
*   [5 Colorized Arch latest news at logon](#Colorized_Arch_latest_news_at_logon)
*   [6 Variations on a theme](#Variations_on_a_theme)
    *   [6.1 From Arch Forum #1](#From_Arch_Forum_.231)
    *   [6.2 From an italian blog…](#From_an_italian_blog.E2.80.A6)
    *   [6.3 From Arch Forum #2](#From_Arch_Forum_.232)
    *   [6.4 With directory information](#With_directory_information)
        *   [6.4.1 Version #1: with numerical error](#Version_.231:_with_numerical_error)
        *   [6.4.2 Version #2: with unicode error status symbols](#Version_.232:_with_unicode_error_status_symbols)
        *   [6.4.3 Version #3: with unicode error status symbol (non-zero only)](#Version_.233:_with_unicode_error_status_symbol_.28non-zero_only.29)
*   [7 Restoring the original /etc/bash.bashrc file](#Restoring_the_original_.2Fetc.2Fbash.bashrc_file)
*   [8 Original /etc/bash.bashrc from Gentoo](#Original_.2Fetc.2Fbash.bashrc_from_Gentoo)
*   [9 Step by step](#Step_by_step)
    *   [9.1 Basic prompts](#Basic_prompts)
        *   [9.1.1 Slightly fancier prompts](#Slightly_fancier_prompts)
    *   [9.2 Advanced prompts](#Advanced_prompts)
        *   [9.2.1 Load/Mem Status for 256colors](#Load.2FMem_Status_for_256colors)
        *   [9.2.2 List of colors for prompt and Bash](#List_of_colors_for_prompt_and_Bash)
        *   [9.2.3 Prompt escapes](#Prompt_escapes)
        *   [9.2.4 Positioning the cursor](#Positioning_the_cursor)
        *   [9.2.5 Return value visualisation](#Return_value_visualisation)
        *   [9.2.6 Wolfman's](#Wolfman.27s)
        *   [9.2.7 KitchM's](#KitchM.27s)
    *   [9.3 Set window title](#Set_window_title)
    *   [9.4 Different colors for text entry and console output](#Different_colors_for_text_entry_and_console_output)
    *   [9.5 Laptop battery status on prompt](#Laptop_battery_status_on_prompt)
    *   [9.6 See also](#See_also)
    *   [9.7 External links](#External_links)

## 一个完善的Bash提示符色彩设置

在GNU/Linux 世界，bash 的提示符可以色彩斑斓。以下的例子就是一个佐证：
...
这是一个 **对于所有用户的方案**, 所以你应该开始删除你的 `~/.bashrc` 文件，准备修改`/etc/bash.bashrc` 文件 并且创建一个`/etc/DIR_COLORS` 文件 (或者写在 `~/.bashrc` 和 `/etc/bash.bashrc` [文件中也可以](/index.php/Color_Bash_Prompt#Example_of_cohabitation_of_.2Fetc.2Fbash.bashrc_and_.7E.2F.bashrc "Color Bash Prompt")).这里是我们Arch的参考版本(原本这个方案是为Gentoo创建，但这里有一些重要的新增)。
...
这篇技巧告诉你如何使用`/etc/skel/`目录，以确保所有新用户在您的系统上获得相同的初始设置。

`/etc/skel/` 目录是使用`useradd` 创建一个新用户时里面的文件会复制新建用户的home目录下。

要修改`/etc/skel/`这个路径,可以编辑`/etc/default/useradd`文件.
...
通常`/etc/skel/` 包含一些`.rc` 文件用于shell初始化, 但是你也可以添加一个`public_html`目录, 一个`.dir_colors`文件,或者是其他的东西.
...
## Colorized [Arch latest news](https://www.archlinux.org/news/) at logon

If you want to read the latest news from the [Arch Official Website](https://www.archlinux.org/), instead of a random quotation from fortune, replace the following lines from [our](/index.php/Color_Bash_Prompt#.2Fetc.2Fbash.bashrc "Color Bash Prompt") `/etc/bash.bashrc` file:
...
that is a small and coloured RSS escaping script written by the user [grufo](https://aur.archlinux.org/account.php?Action=AccountInfo&ID=33208) which will display an output like this:
...
See [this thread](https://bbs.archlinux.org/viewtopic.php?id=146850) for details.

## Variations on a theme

Here are some `PS1` variables (i.e.: prompts) with different layout to be applyied to [our](/index.php/Color_Bash_Prompt#.2Fetc.2Fbash.bashrc "Color Bash Prompt") `/etc/bash.bashrc` file. When you choose a layout you must replace the following lines from [our](/index.php/Color_Bash_Prompt#.2Fetc.2Fbash.bashrc "Color Bash Prompt") `/etc/bash.bashrc` file:
...
Here is an unicode variation of [our](/index.php/Color_Bash_Prompt#.2Fetc.2Fbash.bashrc "Color Bash Prompt") `/etc/bash.bashrc` file freely based on [what wrote the user JeSuisNerd and others in the Arch Forum](https://bbs.archlinux.org/viewtopic.php?pid=1068202#p1068202). Here is a preview of how it will appear:
...
**Note:** **Some unicode symbols (like ✗ and ╼) are not well supported in some terminals** (in linux console, for example), so this prompt will appear a bit different depending on where is displayed. If you want to know the unicode representation of a plain text, [here](http://0xcc.net/jsescape/) you have a little plain text converter.

And finally here is the `PS1` variable for this effect to be applyied to [our](/index.php/Color_Bash_Prompt#.2Fetc.2Fbash.bashrc "Color Bash Prompt") `/etc/bash.bashrc` file (see [above](/index.php/Color_Bash_Prompt#Variations_on_a_theme "Color Bash Prompt")):
...
Here is an unicode variation of [our](/index.php/Color_Bash_Prompt#.2Fetc.2Fbash.bashrc "Color Bash Prompt") `/etc/bash.bashrc` file freely based on [an italian blog](http://dark-linux.net/personalizzare-bashrc-eo-bash-bashrc/). Here is a preview of how it will appear:
...
Here is another variation of [our](/index.php/Color_Bash_Prompt#.2Fetc.2Fbash.bashrc "Color Bash Prompt") `/etc/bash.bashrc` file freely based on [what wrote the user shumer1213 and others in the Arch Forum](https://bbs.archlinux.org/viewtopic.php?pid=1156660#p1156660). Here is a preview of how it will appear:
...
Here is the `PS1` variable for this effect to be applyied to [our](/index.php/Color_Bash_Prompt#.2Fetc.2Fbash.bashrc "Color Bash Prompt") `/etc/bash.bashrc` file (see [above](/index.php/Color_Bash_Prompt#Variations_on_a_theme "Color Bash Prompt")):
...
Here are other three variations of [our](/index.php/Color_Bash_Prompt#.2Fetc.2Fbash.bashrc "Color Bash Prompt") `/etc/bash.bashrc` file freely based on the article [8 Useful and Interesting Bash Prompts](http://maketecheasier.com/8-useful-and-interesting-bash-prompts/2009/09/04). Here is a preview of how they will appear:
...
If you repent having modified the `/etc/bash.bashrc` file, you can always restore the original Arch `/etc/bash.bashrc` file from the [bash](/index.php/Bash "Bash") package and remove the `/etc/DIR_COLORS` file. Note that there is not an "official" bash.bashrc: each distribution has its own.
...
If you want to create a style all your own, you can take a look at these tips. [This Forum thread](https://bbs.archlinux.org/viewtopic.php?id=50885) could give you more informations and examples.
...
[Xterm](/index.php/Xterm "Xterm") and many other terminal emulators (including PuTTY) allow you to set the window title using special escape sequences. You can define the `${XTERM_TITLE}` variable as follows, then insert it at the beginning of the prompt to set [xterm](/index.php/Xterm "Xterm") title (if available) to directory@user@hostname:
...
Read [this article](http://www.basicallytech.com/blog/index.php?/archives/110-Colour-coded-battery-charge-level-and-status-in-your-bash-prompt.html) and [this post](https://bbs.archlinux.org/viewtopic.php?pid=1156660#p1156660) for details.

## See also

*   [gentoo-bashrc](https://aur.archlinux.org/packages/gentoo-bashrc/) from [AUR](/index.php/AUR "AUR")
*   tput(1)
