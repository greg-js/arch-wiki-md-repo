...
*   [1 概览](#概览)
*   [2 安装](#安装)
    *   [2.1 使用Matlab安装程序安装](#使用Matlab安装程序安装)
        *   [2.1.1 桌面图标](#桌面图标)
    *   [2.2 从AUR安装](#从AUR安装)
*   [3 激活](#激活)
    *   [3.1 R2013b及更早版本](#R2013b及更早版本)
*   [4 配置](#配置)
    *   [4.1 Java](#Java)
    *   [4.2 OpenGL加速](#OpenGL加速)
    *   [4.3 字体配置（仅适用于R2014a及更早版本）](#字体配置（仅适用于R2014a及更早版本）)
    *   [4.4 音频](#音频)
    *   [4.5 GPU计算](#GPU计算)
    *   [4.6 安装额外的编译器](#安装额外的编译器)
    *   [4.7 Matlab的帮助查看器的配置](#Matlab的帮助查看器的配置)
*   [5 问题对策](#问题对策)
    *   [5.1 静态线程本地存储错误](#静态线程本地存储错误)
    *   [5.2 Matlab在显示图像时崩溃](#Matlab在显示图像时崩溃)
    *   [5.3 Blank/grey UI when using WM (non-reparenting window manager)](#Blank/grey_UI_when_using_WM_(non-reparenting_window_manager))
    *   [5.4 文本乱码或不可见](#文本乱码或不可见)
    *   [5.5 菜单和输入窗口的文字显示异常](#菜单和输入窗口的文字显示异常)
    *   [5.6 安装时的问题](#安装时的问题)
    *   [5.7 安装时library错误](#安装时library错误)
    *   [5.8 解决启动时警告/错误问题](#解决启动时警告/错误问题)
    *   [5.9 启动Matlab时与ncurses有关的错误（启动时出现分段错误）](#启动Matlab时与ncurses有关的错误（启动时出现分段错误）)
    *   [5.10 与Intel显卡有关的错误](#与Intel显卡有关的错误)
    *   [5.11 附加组件管理器无法使用](#附加组件管理器无法使用)
    *   [5.12 LiveScript错误](#LiveScript错误)
    *   [5.13 Using webcam/video device](#Using_webcam/video_device)
    *   [5.14 关闭帮助窗口时Matlab数秒无响应](#关闭帮助窗口时Matlab数秒无响应)
    *   [5.15 一些下拉菜单无法被选中](#一些下拉菜单无法被选中)
    *   [5.16 因许可证问题造成的无法启动Matlab](#因许可证问题造成的无法启动Matlab)
    *   [5.17 启动Matlab时显示"Failure loading desktop class"并崩溃](#启动Matlab时显示"Failure_loading_desktop_class"并崩溃)
*   [6 在systemd-nspawn容器中运行](#在systemd-nspawn容器中运行)
