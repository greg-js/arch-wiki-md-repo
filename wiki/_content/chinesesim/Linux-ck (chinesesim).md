...
*   [Linux-ck/Changelog](/index.php?title=Linux-ck/Changelog&action=edit&redlink=1 "Linux-ck/Changelog (page does not exist)")
*   [Repo-ck](/index.php/Repo-ck "Repo-ck")
*   [Modprobed_db](/index.php/Modprobed_db "Modprobed db")
...
*   [1 通用包细节](#通用包细节)
    ...
    *   [1.2 包默认信息](#包默认信息)
*   [2 安装选项](#安装选项)
    *   [2.1 1\. 从源代码编译](#1._从源代码编译)
    *   [2.2 2\. 使用编译好的包](#2._使用编译好的包)
        *   [2.2.1 通用和优化的内核包](#通用和优化的内核包)
        *   [2.2.2 添加源到 /etc/pacman.conf](#添加源到_/etc/pacman.conf)
        *   [2.2.3 安装范例](#安装范例)
        *   [2.2.4 怎样启用 BFQ I/O 调度器](#怎样启用_BFQ_I/O_调度器)
            *   [2.2.4.1 全局(对所有设备)](#全局(对所有设备))
            *   [2.2.4.2 选择(对指定设备)](#选择(对指定设备))
*   [3 疑难解答](#疑难解答)
    *   [3.1 与 Linux-ck 运行 Virtualbox](#与_Linux-ck_运行_Virtualbox)
    *   [3.2 论坛支持](#论坛支持)
*   [4 Package Trivia/Repo Statistics](#Package_Trivia/Repo_Statistics)
*   [5 一点关于 BFS 的说明](#一点关于_BFS_的说明)
    *   [5.1 BFS 设计目标](#BFS_设计目标)
    *   [5.2 一个排队论的视频](#一个排队论的视频)
    *   [5.3 Some Performance-Based Metrics: BFS vs. CFS](#Some_Performance-Based_Metrics:_BFS_vs._CFS)
*   [6 更多关于 BFS 和 CK 补丁集的内容](#更多关于_BFS_和_CK_补丁集的内容)
*   [7 Linux-ck Package Changelog](#Linux-ck_Package_Changelog)

## 通用包细节

[Linux-ck](https://aur.archlinux.org/packages/Linux-ck/) 是 [AUR](/index.php/AUR "AUR") 中的一个包，在[非官方 linux-ck 源](#2._使用编译好的包)中允许用户运行一个定制的内核/头文件，基于 Con Kolivas 的 ck1 补丁集，包括 Brain Fuck Scheduler (BFS)。许多 Archer 选择使用这个包，因为 BFS 在任意负载状况下都具有优异的桌面交互性能和敏捷反应特性。
...
**注意:** 源中的包包含了作为模块编译的 BFQ I/O 调度器。如果你想加载或启用它，阅读下面的[小节](#怎样启用_BFQ_I/O_调度器)。
...
参见本文的 [#更多关于 BFS 和 CK 补丁集的内容](#更多关于_BFS_和_CK_补丁集的内容) 部分获取更多信息。
