...
*   [1 概述](#概述)
    ...
    *   [1.2 局限](#局限)
    *   [1.3 购买前注意事项](#购买前注意事项)
*   [2 最大化利用SSD的技巧](#最大化利用SSD的技巧)
    *   [2.1 分区对齐](#分区对齐)
    *   [2.2 TRIM](#TRIM)
        *   [2.2.1 检验TRIM支持](#检验TRIM支持)
        *   [2.2.2 通过挂载参数启用TRIM](#通过挂载参数启用TRIM)
        *   [2.2.3 通过 cron 应用TRIM](#通过_cron_应用TRIM)
        *   [2.2.4 通过 systemd 服务应用TRIM](#通过_systemd_服务应用TRIM)
        *   [2.2.5 用 tune2fs 启用TRIM(不推荐)](#用_tune2fs_启用TRIM(不推荐))
        *   [2.2.6 为LVM启用TRIM](#为LVM启用TRIM)
        *   [2.2.7 为dm-crypt启用TRIM](#为dm-crypt启用TRIM)
    *   [2.3 I/O调度器](#I/O调度器)
        *   [2.3.1 内核参数(针对单个设备)](#内核参数(针对单个设备))
        *   [2.3.2 针对单个设备或者HDD/SDD混合情况使用udev](#针对单个设备或者HDD/SDD混合情况使用udev)
    *   [2.4 SSD上的交换空间](#SSD上的交换空间)
    *   [2.5 Hdparm 显示 "frozen" 状态](#Hdparm_显示_"frozen"_状态)
    *   [2.6 SSD存储单元的清除](#SSD存储单元的清除)
    *   [2.7 处理NCQ错误](#处理NCQ错误)
*   [3 最小化硬盘读写的技巧](#最小化硬盘读写的技巧)
    *   [3.1 智能分区方案](#智能分区方案)
    *   [3.2 noatime 挂载参数](#noatime_挂载参数)
    *   [3.3 将频繁使用的文件置于内存](#将频繁使用的文件置于内存)
        *   [3.3.1 浏览器配置文件](#浏览器配置文件)
        *   [3.3.2 其他](#其他)
    *   [3.4 在tmpfs里编译](#在tmpfs里编译)
    *   [3.5 文件系统上禁用日志](#文件系统上禁用日志)
*   [4 文件系统的选择](#文件系统的选择)
    *   [4.1 Btrfs](#Btrfs)
    *   [4.2 Ext4](#Ext4)
    *   [4.3 XFS](#XFS)
...
如果你想擦除SSD，见 [Securely wipe disk#hdparm](/index.php/Securely_wipe_disk#hdparm "Securely wipe disk") 以及 [下文](#SSD存储单元的清除)。
