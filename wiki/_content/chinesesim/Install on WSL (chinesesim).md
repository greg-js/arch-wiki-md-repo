...
**注意:** 如果你未安装 Windows 10 2017年秋季的创造者更新, 使用 `%localappdata%\lxss\rootfs` 代替 `%localappdata%\Packages\CanonicalGroupLimited.UbuntuonWindows_*\LocalState\rootfs`.
...
使用一台Linux电脑构建 [fakeroot-tcp](https://aur.archlinux.org/packages/fakeroot-tcp/) 和 [glibc-wsl](https://aur.archlinux.org/packages/glibc-wsl/), 然后复制到你的 Windows 电脑。 *glibc-wsl* 用于解决 [这个bug](https://github.com/Microsoft/BashOnWindows/issues/1878)（[在17046后已被修复，因此不再需要](https://github.com/Microsoft/WSL/issues/1878#issuecomment-350322200)）。而直到 System V IPC 被完全实现之前 *fakeroot-tcp* 都是必需的。 （[(详情请见)](https://github.com/Microsoft/BashOnWindows/issues/1016)。
...
**注意:** 如果未安装 Windows 10 2017年秋季的创造者更新, 使用这个命令: `lxrun /setdefaultuser *username*`
