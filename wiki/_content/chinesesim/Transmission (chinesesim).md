...
*   [1 安装](#安装)
*   [2 图形界面的配置](#图形界面的配置)
*   [3 Transmission 守护程序和命令行](#Transmission_守护程序和命令行)
    *   [3.1 守护程序的启动与停止](#守护程序的启动与停止)
    *   [3.2 减少多余的日志](#减少多余的日志)
    *   [3.3 仅当连接到网络时运行](#仅当连接到网络时运行)
        *   [3.3.1 Netctl](#Netctl)
        *   [3.3.2 Wicd](#Wicd)
    *   [3.4 选择一个用户](#选择一个用户)
    *   [3.5 配置守护程序](#配置守护程序)
        *   [3.5.1 Host 白名单](#Host_白名单)
        *   [3.5.2 监视目录](#监视目录)
        *   [3.5.3 启用 IPv6](#启用_IPv6)
        *   [3.5.4 CLI 使用示范](#CLI_使用示范)
*   [4 Web 界面](#Web_界面)
    *   [4.1 GUI 方式](#GUI_方式)
    *   [4.2 CLI 方式](#CLI_方式)
*   [5 故障排除](#故障排除)
    *   [5.1 无法使用网络连接至守护程序](#无法使用网络连接至守护程序)
    *   [5.2 无法显示 Web 界面](#无法显示_Web_界面)
*   [6 更多信息](#更多信息)
...
*   [transmission-cli](https://www.archlinux.org/packages/?name=transmission-cli) - 包括命令行工具、守护程序和 [#Web 界面](#Web_界面)。
...
就像 [#选择一个用户](#选择一个用户) 里面的说明那样，transmission 的守护程序可以按以下方式运行：

*   作为 *transmission* 用户运行，只要 [用 systemd](/index.php/Systemd_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87)#使用单元 "Systemd (简体中文)") start/enable `transmission.service` 服务即可。

	你可以按照 [#选择一个用户](#选择一个用户) 里的说明更改默认用户。

*   用你自己的账户，只要在账户下运行： `$ transmission-daemon` 守护程序可以这样来停止： `$ killall transmission-daemon` 

启动守护程序将会创建一个初始的 *transmission* 配置文件。详情参阅 [#配置守护程序](#配置守护程序)。
...
在文件夹 `/etc/wicd/scripts/postconnect` 里创建一个 [启动脚本](#守护程序的启动与停止)，并且在文件夹 `/etc/wicd/scripts/predisconnect` 里创建一个 [停止脚本](#守护程序的启动与停止)。记得给它们运行权限，例如：
...
设置这种方式需要 [重写](/index.php/Systemd_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87)#修改现存单元文件 "Systemd (简体中文)") 已提供的服务文件并在其中指定你的用户名：
...
**注意:** 如果你想用一个文本编辑器手动编辑配置文件，请先 [停止守护程序](#守护程序的启动与停止)；否则当它停止时将会覆盖配置文件。
...
如果你在用 systemd，按照 [systemd (简体中文)#修改现存单元文件](/index.php/Systemd_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87)#修改现存单元文件 "Systemd (简体中文)") 里面的描述编辑 `transmission.service` 单元文件。
...
设置 web 界面甚至不需要借助图形界面，因为守护程序提供了类似的选项。你无需指定任何参数即可访问 web 界面，可参阅 [#守护程序的启动与停止](#守护程序的启动与停止)。
...
在 `network.service` 初始化后守护程序就会开始运行。但是，如果启用了 `dhcpcd` 服务而不是特定设备的服务（比如 `dhcpcd@enp1s0.service`），可能 Transmission 会因为启动过早而无法绑定到相应的网络接口。于是 web 界面无法访问。一种可能的解决方案是在 systemd 单元的 [配置文件](/index.php/Systemd_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87)#修改现存单元文件 "Systemd (简体中文)") 里增加 `Requires` 行。
