**zh-CN:Wordpress (简体中文)**

（机翻0.0）[WordPress](http://wordpress.org)Wordpress是一个免费开源的博客管理系统。([CMS](https://en.wikipedia.org/wiki/Content_management_system "wikipedia:Content management system")) 由[Matt Mullenweg](https://en.wikipedia.org/wiki/Matt_Mullenweg "wikipedia:Matt Mullenweg") 开发，在2003年发行。 WordPress拥有庞大而充满活力的社区，提供数以万计的免费插件和主题，让用户轻松自定义WordPress CMS的外观和功能。 WordPress是根据GPLv2授权的.

WordPress的最大特点是易于配置和管理。 [Setting up a WordPress site takes five minutes](http://codex.wordpress.org/Installing_WordPress). WordPress管理面板允许用户轻松地配置其网站的几乎每个方面，包括获取和安装插件和主题。WordPress提供方便的自动更新。
...
使用WordPress需要安装和配置[PHP](/index.php/PHP "PHP")和[MySQL](/index.php/MySQL "MySQL")。 有关详细信息，请参阅[LAMP](/index.php/LAMP "LAMP")维基文章。在配置过程中，请注意，某些WordPress功能需要[PHP扩展](http://wordpress.stackexchange.com/questions/42098/what-are-php-extensions-and-libraries-wp-needs-and-or-us)默认情况下可能不会打开。
