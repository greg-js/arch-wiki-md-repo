...
*   [1 安装](#安装)
*   [2 插件](#插件)
    *   [2.1 添加默认更新站点](#添加默认更新站点)
    *   [2.2 Eclipse Marketplace](#Eclipse_Marketplace)
    *   [2.3 插件管理器](#插件管理器)
        *   [2.3.1 通过插件管理器升级](#通过插件管理器升级)
    *   [2.4 插件列表](#插件列表)
*   [3 启用 javadoc 集成](#启用_javadoc_集成)
    *   [3.1 在线版本](#在线版本)
    *   [3.2 离线版本](#离线版本)
*   [4 疑难问题](#疑难问题)
    *   [4.1 Ctrl+X 关闭了 Eclipse](#Ctrl+X_关闭了_Eclipse)
    *   [4.2 暗色主题](#暗色主题)
    *   [4.3 使用 Gnome 3.6 Adwaita 主题时"提示"显示为深色背景色](#使用_Gnome_3.6_Adwaita_主题时"提示"显示为深色背景色)
    *   [4.4 禁止 GTK+ 3](#禁止_GTK+_3)
    *   [4.5 切换按钮的选择/未选择状态是一样的](#切换按钮的选择/未选择状态是一样的)
    *   [4.6 改变默认窗口标题字号](#改变默认窗口标题字号)
*   [5 参阅](#参阅)
...
大部分插件只需 **pacman** 就可安装 (更多信息见 [Eclipse 插件包参考](/index.php/Eclipse_plugin_package_guidelines "Eclipse plugin package guidelines")). 这样同时能保证它们是最新的。另外，你也可以使用 [#Eclipse Marketplace](#Eclipse_Marketplace) 或内建的[#插件管理器](#插件管理器)。
...
**注意:** 确保您遵循了[#添加默认更新站点](#添加默认更新站点)部分。
...
当GTK 3 有很多bug并且不稳定时（比如，代码框不断闪烁），你可以禁止GTK 3。在启动 eclipse 时使用 SWT_GTK3=0 [environment variable](/index.php/Environment_variable "Environment variable") ：
