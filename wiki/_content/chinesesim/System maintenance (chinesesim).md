...
*   [2 备份](#备份)
    *   [2.1 配置文件](#配置文件)
    *   [2.2 安装的软件包](#安装的软件包)
    *   [2.3 Pacman 数据库](#Pacman_数据库)
    *   [2.4 LUKS headers](#LUKS_headers)
    *   [2.5 系统和用户数据](#系统和用户数据)
*   [3 更新系统](#更新系统)
    *   [3.1 升级前的准备](#升级前的准备)
    *   [3.2 避免某些 Pacman 命令](#避免某些_Pacman_命令)
    *   [3.3 不支持部分升级](#不支持部分升级)
    *   [3.4 注意更新时的提醒](#注意更新时的提醒)
    *   [3.5 处理配置文件更新](#处理配置文件更新)
    *   [3.6 回退有问题的更新](#回退有问题的更新)
    *   [3.7 检查孤立和未使用软件包](#检查孤立和未使用软件包)
    *   [3.8 使用包管理器安装软件](#使用包管理器安装软件)
    *   [3.9 选择开源驱动](#选择开源驱动)
    *   [3.10 谨慎使用非官方或测试不足的软件包](#谨慎使用非官方或测试不足的软件包)
    *   [3.11 更新镜像列表](#更新镜像列表)
*   [4 清理文件系统](#清理文件系统)
    *   [4.1 软件包缓存](#软件包缓存)
    *   [4.2 不需要的软件包](#不需要的软件包)
    ...
    *   [4.4 破损的软链接](#破损的软链接)
*   [5 技巧](#技巧)
    *   [5.1 使用经过验证的软件包](#使用经过验证的软件包)
    *   [5.2 安装内核的长期支持版本](#安装内核的长期支持版本)
*   [6 参阅](#参阅)
...
避免 [部分更新](#不支持部分升级)，不要运行 `pacman -Sy` 而是运行 `pacman -Syu`.
...
从系统里面删除不需要的软件包可以减少空间占用和维护难度。详情参考 [Pacman/Tips and tricks#Removing unused packages (orphans)](/index.php/Pacman/Tips_and_tricks#Removing_unused_packages_(orphans) "Pacman/Tips and tricks").
