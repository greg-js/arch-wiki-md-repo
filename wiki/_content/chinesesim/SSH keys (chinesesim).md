...
*   [1 背景](#背景)
*   [2 生成密钥对](#生成密钥对)
    *   [2.1 选择合适的加密方式](#选择合适的加密方式)
    *   [2.2 选择密钥存储位置以及密码短语](#选择密钥存储位置以及密码短语)
        *   [2.2.1 不修改密钥对的情况下修改密码短语](#不修改密钥对的情况下修改密码短语)
        *   [2.2.2 管理多组密钥对](#管理多组密钥对)
*   [3 将公钥复制到远程服务器上](#将公钥复制到远程服务器上)
    *   [3.1 简单的方法](#简单的方法)
    *   [3.2 传统的方法](#传统的方法)
*   [4 安全性](#安全性)
    *   [4.1 禁用密码登录](#禁用密码登录)
...
*   [5 SSH agents](#SSH_agents)
    ...
    *   [5.3 Keychain](#Keychain)
        *   [5.3.1 另一种启动 keychain 的方式](#另一种启动_keychain_的方式)
    *   [5.4 envoy](#envoy)
        *   [5.4.1 利用 KDE 电子钱包存储密码短语并和 envoy 配合工作](#利用_KDE_电子钱包存储密码短语并和_envoy_配合工作)
    *   [5.5 pam_ssh](#pam_ssh)
        *   [5.5.1 pam_ssh 已知问题](#pam_ssh_已知问题)
...
您可以将上述命令添加到 `~/.bash_profile`，以便您启动 [登录外壳](/index.php/Bash_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87)#登录外壳 "Bash (简体中文)") 的时候它自动运行。
...
但是这样做有个缺点，每次启动 [登录外壳](/index.php/Bash_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87)#登录外壳 "Bash (简体中文)") 都会产生一个 `ssh-agent` 实例，并在会话期间一直运行。不久之后，您的系统中就会有多个根本不再需要的 `ssh-agent` 进程在运行。稍后，我们将为您介绍其他 `ssh-agent` 前端，它们能够避免这个问题。
...
如有需要，请把 `id_ecdsa` 换成您的私钥路径。如果您使用的 shell 不兼容 [Bash (简体中文)](/index.php/Bash_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87) "Bash (简体中文)")，请参考 `keychain --help` 或者 [keychain(1)](https://jlk.fjfi.cvut.cz/arch/manpages/man/keychain.1)。
...
**Note:** 注意: envoy作者已经不再维护该项目，请慎重使用
