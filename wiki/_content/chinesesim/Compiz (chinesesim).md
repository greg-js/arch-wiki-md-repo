...
*   [1 安装](#安装)
    *   [1.1 安装0.9系列版本](#安装0.9系列版本)
    *   [1.2 安装0.8系列版本](#安装0.8系列版本)
    *   [1.3 窗口装饰器](#窗口装饰器)
*   [2 开始使用 Compiz](#开始使用_Compiz)
    *   [2.1 启用重要的插件](#启用重要的插件)
    *   [2.2 启动 Compiz](#启动_Compiz)
        *   [2.2.1 小工具 Fusion Icon](#小工具_Fusion_Icon)
    *   [2.3 让你的桌面环境自动启动Compiz](#让你的桌面环境自动启动Compiz)
        *   [2.3.1 KDE4](#KDE4)
            *   [2.3.1.1 使用系统设置](#使用系统设置)
            *   [2.3.1.2 Autostart 自动启动链接](#Autostart_自动启动链接)
            *   [2.3.1.3 设置 KDEWM 变量](#设置_KDEWM_变量)
        *   [2.3.2 GNOME 桌面环境](#GNOME_桌面环境)
            *   [2.3.2.1 GNOME Shell 和 Cinnamon](#GNOME_Shell_和_Cinnamon)
            *   [2.3.2.2 Unity](#Unity)
            *   [2.3.2.3 其它的GNOME会话 (Cairo dock 和 Compiz)](#其它的GNOME会话_(Cairo_dock_和_Compiz))
            *   [2.3.2.4 GNOME Flashback](#GNOME_Flashback)
        *   [2.3.3 MATE 桌面环境](#MATE_桌面环境)
            *   [2.3.3.1 使用 GSettings](#使用_GSettings)
            *   [2.3.3.2 使用 mate-session-properties](#使用_mate-session-properties)
        *   [2.3.4 Xfce](#Xfce)
            *   [2.3.4.1 修改默认 (故障保护Failsafe) 会话](#修改默认_(故障保护Failsafe)_会话)
...
*   [3 Using Compiz as a standalone window manager](#Using_Compiz_as_a_standalone_window_manager)
    ...
    *   [3.5 Utilities](#Utilities)
        *   [3.5.1 Panels & docks](#Panels_&_docks)
        *   [3.5.2 Run dialog](#Run_dialog)
*   [4 小技巧大放送](#小技巧大放送)
    *   [4.1 还原原生窗口管理器](#还原原生窗口管理器)
    *   [4.2 启用 Alt+F2 运行对话框](#启用_Alt+F2_运行对话框)
        *   [4.2.1 GNOME Panel](#GNOME_Panel)
        *   [4.2.2 MATE Panel](#MATE_Panel)
        *   [4.2.3 LXDE Panel](#LXDE_Panel)
        *   [4.2.4 Xfce 程序查看器（*Appfinder*）](#Xfce_程序查看器（Appfinder）)
        *   [4.2.5 其他的运行对话框功能](#其他的运行对话框功能)
*   [5 疑难排解](#疑难排解)
    *   [5.1 Missing GLX_EXT_texture_from_pixmaps](#Missing_GLX_EXT_texture_from_pixmaps)
        *   [5.1.1 On ATI cards (first solution)](#On_ATI_cards_(first_solution))
        *   [5.1.2 On ATI cards (second solution)](#On_ATI_cards_(second_solution))
    ...
    *   [5.6 Compiz effects not working (GConf backend)](#Compiz_effects_not_working_(GConf_backend))
    ...
    *   [5.9 Emerald refuses to start (crashes with a segfault)](#Emerald_refuses_to_start_(crashes_with_a_segfault))
...
在CCSM里面，确保选择了窗口装饰器，已经启用了窗口管理需要的必要插件。详见 [#启用重要的插件](#启用重要的插件).
