**翻译状态：** 本文是英文页面 [Systemd](/index.php/Systemd "Systemd") 的[翻译](/index.php/ArchWiki_Translation_Team_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87) "ArchWiki Translation Team (简体中文)")，最后翻译时间：2015-06-11，点击[这里](https://wiki.archlinux.org/index.php?title=Systemd&diff=0&oldid=377056)可以查看翻译后英文页面的改动。
...
*   [3 目标（target）](#.E7.9B.AE.E6.A0.87.EF.BC.88target.EF.BC.89)
    ...
    *   [3.4 切换启动级别/目标](#.E5.88.87.E6.8D.A2.E5.90.AF.E5.8A.A8.E7.BA.A7.E5.88.AB.2F.E7.9B.AE.E6.A0.87)
...
*   [7 疑难解答](#.E7.96.91.E9.9A.BE.E8.A7.A3.E7.AD.94)
    ...
    *   [7.7 Error message on reboot or shutdown](#Error_message_on_reboot_or_shutdown)
...
安装 [polkit](/index.php/Polkit "Polkit") 后才可以一般用户身份使用电源管理。
...
开机启动进的目标是 `default.target`，默认链接到 `graphical.target` （大致相当于原来的启动级别5）。可以通过[内核参数](/index.php/Kernel_parameters_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87) "Kernel parameters (简体中文)")更改默认启动级别：
...
**注意:** 该方法不能向 `/sys` 中的配置文件添加参数，因为 `systemd-tmpfiles-setup` 有可能在相关模块加载前运行。这种情况下，需要首先通过 `modinfo <模块名>` 确认需要的参数，并在 [`/etc/modprobe.d` 下的一个文件](/index.php/Kernel_modules_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87)#.E9.80.89.E9.A1.B9 "Kernel modules (简体中文)")中设置改参数。另外，还可以使用 [udev 规则](/index.php/Udev_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87)#udev.E8.A7.84.E5.88.99 "Udev (简体中文)")，在设备就绪时设置相应属性。
