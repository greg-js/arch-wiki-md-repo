...
Arch Linux 上的字体渲染包支持有字节码解释器(BCI)的*freetype2*。为了更好的字体渲染，特别是在 LCD 显示器上，一些补丁包也添加到库中，参见下面的 [#补丁包](#补丁包)。[#Infinality](#Infinality) 包同时支持自动微调和亚像素渲染，允许在无须重新编译的情况下微调LCD滤光器，而且在粗体字下自动微调仍然表现良好。
...
*   [1 字体路径](#字体路径)
*   [2 Fontconfig配置](#Fontconfig配置)
    *   [2.1 预设](#预设)
    *   [2.2 抗锯齿](#抗锯齿)
    *   [2.3 微调](#微调)
        *   [2.3.1 字节码解释器(BCI)](#字节码解释器(BCI))
        *   [2.3.2 自动微调器](#自动微调器)
        *   [2.3.3 微调样式](#微调样式)
    *   [2.4 亚像素渲染](#亚像素渲染)
        *   [2.4.1 LCD过滤器](#LCD过滤器)
        *   [2.4.2 高级LCD过滤器说明](#高级LCD过滤器说明)
    *   [2.5 取消粗体字的自动微调功能](#取消粗体字的自动微调功能)
    *   [2.6 只对较大的字体打开反锯齿功能](#只对较大的字体打开反锯齿功能)
    *   [2.7 字体替换](#字体替换)
    *   [2.8 禁用点阵字型](#禁用点阵字型)
    *   [2.9 禁用点阵字体的缩放](#禁用点阵字体的缩放)
    *   [2.10 为不完善的字体构建粗体风格和斜体风格](#为不完善的字体构建粗体风格和斜体风格)
    *   [2.11 改变规则重定义机制](#改变规则重定义机制)
    *   [2.12 fontconfig配置的例子](#fontconfig配置的例子)
*   [3 补丁包](#补丁包)
    *   [3.1 Infinality](#Infinality)
    *   [3.2 Ubuntu](#Ubuntu)
    *   [3.3 恢复到未打补丁的包](#恢复到未打补丁的包)
*   [4 没有fontconfig支持的应用程序](#没有fontconfig支持的应用程序)
*   [5 字体问题解决方法](#字体问题解决方法)
    *   [5.1 字体扭曲](#字体扭曲)
    *   [5.2 较旧的GTK和QT应用程序](#较旧的GTK和QT应用程序)
*   [6 资源](#资源)
