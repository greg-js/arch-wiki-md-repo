...
**Note:** Both procedures will install Node.js version 6.x or 7.x. While Cozy **may** totally work with these versions, they are not officially supported, as Cozy officially supports only Node.js v4.x, which can be found in [nodejs-lts-argon](https://aur.archlinux.org/packages/nodejs-lts-argon/), [nodejs-lts-argon-bin](https://aur.archlinux.org/packages/nodejs-lts-argon-bin/). If you still want to use the Node.js version currently in Archlinux's official repository, you will also need [npm](https://www.archlinux.org/packages/?name=npm).
