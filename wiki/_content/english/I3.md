...
Add to `.i3/config`: [[3]](https://faq.i3wm.org/question/853/how-to-jump-to-urgent-workspace/)
...
See also [[4]](https://faq.i3wm.org/question/138/multiple-scratchpad/i3) for multiple scratchpads.
...
A [systemd](/index.php/Systemd "Systemd") service file can be used to lock the screen before the system is being sent to sleep or hibernation state. See [Power_management#Suspend.2Fresume_service_files](/index.php/Power_management#Suspend.2Fresume_service_files "Power management").
...
Once completed, you will be presented with a prompt whenever you press `$mod+pause`. For more complex behaviour, use a separate script, and refer to it in the mode. [[5]](https://gist.github.com/anonymous/c8cd0a59bf4acb273068)
...
As workspaces are defined multiple times in i3, assigning workspace variables can be helpful. For example [[8]](https://github.com/dkeg/bloat2.0/blob/master/i3config#L55):
...
In many cases, bugs are fixed in the development versions [i3-git](https://aur.archlinux.org/packages/i3-git/) and [i3status-git](https://aur.archlinux.org/packages/i3status-git/), and upstream will ask to reproduce any errors with this version. [[10]](http://i3wm.org/docs/debugging.html) See also [Debug - Getting Traces#General](/index.php/Debug_-_Getting_Traces#General "Debug - Getting Traces").
...
i3 v4.3 and higher ignore size increment hints for tiled windows [[11]](https://www.mail-archive.com/i3-discuss@i3.zekjur.net/msg00709.html). This may cause terminals to wrap lines prematurely, amongst other issues. As a workaround, make the offending window floating, before tiling it again.
...
Some tools such as [scrot](/index.php/Taking_a_screenshot#scrot "Taking a screenshot") may not work when used with a regular key binding (executed after key press). In those cases, execute commands after key release with the `--release` argument [[12]](http://i3wm.org/docs/userguide.html#keybindings):
...
i3 does [not properly implement double buffering](https://github.com/i3/i3/issues/661) hence tearing or flickering may occur. To prevent this, install and configure [compton](/index.php/Compton "Compton"). [[13]](https://faq.i3wm.org/question/3279/do-i-need-a-composite-manager-compton.1#post-id-3282)

### Tray icons not visible

The default `tray_output primary` directive may require setting a primary output with *xrandr*, specifying the output explicitly or simply removing this directive. [[14]](https://github.com/i3/i3/issues/1144) See [Xrandr](/index.php/Xrandr "Xrandr") for details. The default configuration created by i3-config-wizard will no longer add this directive to the configuration with i3 4.12.
