...
*   [4 Configuration](#Configuration)
    ...
    *   [4.3 Colorschemes](#Colorschemes)
    *   [4.4 i3bar](#i3bar)
        *   [4.4.1 i3bar alternatives](#i3bar_alternatives)
    *   [4.5 i3status](#i3status)
        *   [4.5.1 i3status replacements](#i3status_replacements)
        *   [4.5.2 i3status wrappers](#i3status_wrappers)
        *   [4.5.3 Iconic fonts in the status bar](#Iconic_fonts_in_the_status_bar)
    *   [4.6 Terminal emulator](#Terminal_emulator)
    *   [4.7 Disable title click](#Disable_title_click)
*   [5 Tips and tricks](#Tips_and_tricks)
    ...
    *   [5.12 Automatically switch horizontal / vertical window split orientation](#Automatically_switch_horizontal_/_vertical_window_split_orientation)
*   [6 Patches](#Patches)
*   [7 Troubleshooting](#Troubleshooting)
    ...
    *   [7.9 Default workspace for Any program](#Default_workspace_for_Any_program)
*   [8 See also](#See_also)
...
If switching between multiple window managers or desktop environments, consider using [sxhkd](/index.php/Sxhkd "Sxhkd") or another environment agnostic program to manage bindings. More information can be found in [Keyboard shortcuts#Xorg](/index.php/Keyboard_shortcuts#Xorg "Keyboard shortcuts").

### Containers and layouts

*i3* manages windows in a tree structure, with containers as building blocks. This structure branches with horizontal or vertical splits. Containers are tiled by default, but can be set to [tabbed or stacking](https://i3wm.org/docs/userguide.html#_layout_mode_for_new_containers) layouts, as well as made floating (such as for dialog windows). Floating windows are always on top.
...
Alternatively, you can use [XDG Autostart](/index.php/XDG_Autostart "XDG Autostart").
...
*   **[polybar](/index.php/Polybar "Polybar")** — A fast and easy-to-use tool for creating status bars.
...
*   **j4status** — Provides a statusline, configurable via plugins, and written in C. Extra plugins are provided by [j4status-plugins-git](https://aur.archlinux.org/packages/j4status-plugins-git/).
	...
	[https://sardemff7.github.io/j4status/](https://sardemff7.github.io/j4status/) || [j4status-git](https://aur.archlinux.org/packages/j4status-git/)
...
*   **i3-cycle-focus** — Provides an Alt-Tab functionality for *i3*
	...
	[https://github.com/acrisci/i3ipc-python/blob/master/examples/i3-cycle-focus.py](https://github.com/acrisci/i3ipc-python/blob/master/examples/i3-cycle-focus.py) ||
...
3.  The newly created file needs to be edited, however this may be done with the following commands: `tail -n +2 ~/.i3/workspace_N.json | fgrep -v '// split' | sed 's|//||g' > ~/.i3/workspace_N.json` 
...
The [autotiling](https://aur.archlinux.org/packages/autotiling/) package can be used for automatic switching horizontal / vertical window split orientation resulting in a similar behavior to the spiral tiling of bspwm. After installation add the following to your `~/.config/i3/config` and reload i3.
...
### Default workspace for Any program

To assign a default workspace for a specific program by using a `assign`. The window name can be obtained with `xwininfo` command, such as

 `~/.config/i3/config` 
```
...
assign [class="<window name>"] $ws10
```
