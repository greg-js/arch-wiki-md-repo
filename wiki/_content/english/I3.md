...
When *i3* is first started, it offers to run the configuration wizard *i3-config-wizard*. This tool creates `~/.i3/config` by rewriting a template configuration file in `/etc/i3/config.keycodes`. It makes two modifications to the default template:
...
A [systemd](/index.php/Systemd "Systemd") service file can be used to lock the screen before the system is being sent to sleep or hibernation state. An example `suspend@.service` file can be found here: [Power_management#Suspend.2Fresume_service_files](/index.php/Power_management#Suspend.2Fresume_service_files "Power management"). The following line invokes the `i3lock` program.

```
ExecStart=/usr/bin/i3lock -i 'background_image.png'

```

An example i3lock service file:
...
Service fails if not forking. Soruce: [[7]](https://bbs.archlinux.org/viewtopic.php?pid=1170536#p1170536)
...
Once completed, you will be presented with a prompt whenever you press `$mod+pause`. For more complex behaviour, use a separate script, and refer to it in the mode. [[8]](https://gist.github.com/anonymous/c8cd0a59bf4acb273068)

**Note:**

*   `sleep 1` adds a small delay to prevent possible race conditions with suspend [[9]](https://bugs.launchpad.net/ubuntu/+source/unity-2d/+bug/830348)
*   The `-i` argument for `systemctl poweroff` causes a shutdown even if other users are logged-in (this requires [polkit](https://www.archlinux.org/packages/?name=polkit)), or when *logind* (wrongly) assumes so. [[10]](https://bugs.freedesktop.org/show_bug.cgi?id=62676)
...
As workspaces are defined multiple times in i3, assigning workspace variables can be helpful. For example [[11]](https://github.com/dkeg/bloat2.0/blob/master/i3config#L55):
...
In many cases, bugs are fixed in the development versions [i3-git](https://aur.archlinux.org/packages/i3-git/) and [i3status-git](https://aur.archlinux.org/packages/i3status-git/), and upstream will ask to reproduce any errors with this version. [[13]](http://i3wm.org/docs/debugging.html) See also [Debug - Getting Traces#General](/index.php/Debug_-_Getting_Traces#General "Debug - Getting Traces").
...
i3 v4.3 and higher ignore size increment hints for tiled windows [[14]](https://www.mail-archive.com/i3-discuss@i3.zekjur.net/msg00709.html). This may cause terminals to wrap lines prematurely, amongst other issues. As a workaround, make the offending window floating, before tiling it again.
...
Some tools such as [scrot](/index.php/Taking_a_screenshot#scrot "Taking a screenshot") may not work when used with a regular key binding (executed after key press). In those cases, execute commands after key release with the `--release` argument [[15]](http://i3wm.org/docs/userguide.html#keybindings):
...
i3 does [not properly implement double buffering](https://github.com/i3/i3/issues/661) hence tearing or flickering may occur. To prevent this, install and configure [compton](/index.php/Compton "Compton"). [[16]](https://faq.i3wm.org/question/3279/do-i-need-a-composite-manager-compton.1#post-id-3282)

### Tray icons not visible

The default `tray_output primary` directive may require setting a primary output with *xrandr*, specifying the output explicitly or simply removing this directive. [[17]](https://github.com/i3/i3/issues/1144) See [Xrandr](/index.php/Xrandr "Xrandr") for details. The default configuration created by i3-config-wizard will no longer add this directive to the configuration with i3 4.12.
