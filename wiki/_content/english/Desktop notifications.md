...
After installing the [notification-daemon](https://www.archlinux.org/packages/?name=notification-daemon) package, the notification server can be launched on the first call via [D-Bus](/index.php/D-Bus "D-Bus"), adding the following configuration to its services directory `/usr/share/dbus-1/services` respectively `$XDG_DATA_HOME/dbus-1/services`, e.g.
...
*   **mako** — A lightweight notification daemon for [Wayland](/index.php/Wayland "Wayland"); it currently works on [Sway](/index.php/Sway "Sway").
