...
*   [3 Nginx](#Nginx)
*   [4 Caddy](#Caddy)
*   [5 Generating Statistics](#Generating_Statistics)
    *   [5.1 systemd](#systemd)
    *   [5.2 cron](#cron)
*   [6 GeoIP (optional)](#GeoIP_(optional))
*   [7 See also](#See_also)
...
3\. Create your Caddyfile config file `/etc/caddy/caddy.conf.d/sitename` with these directives:
...
This process can be automated via an (hourly) systemd timer script. You will need to create an `awstats-aw.service` and `awstats-aw.timer` script and enable the timer script.
...
After creating the timer and service, run `systemctl start awstats-aw.service && systemctl enable awstats-aw.timer`
...
If you install [geoip-database-extra](https://www.archlinux.org/packages/?name=geoip-database-extra) you can also add these to your awstats site configuration:
...
*   AWStats user contributed tools [[1]](https://awstats.sourceforge.io/docs/awstats_contrib.html#CONTRIB)