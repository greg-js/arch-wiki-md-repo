...
Note that the `man random` command will misdirect to the library function manpage [random(3)](https://jlk.fjfi.cvut.cz/arch/manpages/man/random.3) while for information about the `/dev/random` device files you should run `man 4 random` to read [random(4)](https://jlk.fjfi.cvut.cz/arch/manpages/man/random.4).
...
1.  Use the kernel's `getrandom()` system call, whenever possible. It is provided to ensure that `/dev/urandom` has initialized appropriately. See [getrandom(2)](https://jlk.fjfi.cvut.cz/arch/manpages/man/getrandom.2).
