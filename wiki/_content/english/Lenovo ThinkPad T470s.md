...
On [gnome-shell](https://www.archlinux.org/packages/?name=gnome-shell) via [wayland](/index.php/Wayland "Wayland"), suspend-resume results in the fan holding at 100% without ever spinning down. Alternatively if you use [xorg](/index.php/Xorg "Xorg") this doesn't seem to happen. Upgrading BIOS/UEFI seems to solve the issue. Another way to solve the issue is going to BIOS → Config → Power → Adaptive Thermal Management and picking Balanced instead of Maximize Performance.
