...
*   [4 Example PKGBUILD using Dune](#Example_PKGBUILD_using_Dune)
*   [5 Example PKGBUILD using plain findlib](#Example_PKGBUILD_using_plain_findlib)
...
[Dune](https://dune.build/) is a new build system that is becoming more and more used by OCaml projects.

One thing to be aware is that a single project can build several "packages" in the OPAM/findlib sense, each with its own directory in `/usr/lib/ocaml/`. See [ocaml-cairo](https://www.archlinux.org/packages/?name=ocaml-cairo) for an example. For release builds, all "packages" have to be explicitly listed.
