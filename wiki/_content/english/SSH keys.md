...
An SSH key pair can be generated by running the `ssh-keygen` command, defaulting to 2048-bit RSA (and SHA256) which the [ssh-keygen(1)](http://jlk.fjfi.cvut.cz/arch/manpages/man/ssh-keygen.1) man page says is "*generally considered sufficient*" and should be compatible with virtually all clients and servers:
...
Minimum key size is 1024 bits, default is 2048 (see [ssh-keygen(1)](http://jlk.fjfi.cvut.cz/arch/manpages/man/ssh-keygen.1)) and maximum is 16384.
...
See [ssh_config(5)](http://jlk.fjfi.cvut.cz/arch/manpages/man/ssh_config.5) for full description of these options.
...
See `keychain --help` or [keychain(1)](http://jlk.fjfi.cvut.cz/arch/manpages/man/keychain.1) for details on setting *keychain* for other shells.
...
**Note:** Keychain is able to manage [GPG](/index.php/GPG "GPG") keys in the same fashion. By default it attempts to start *ssh-agent* only, but you can modify this behavior using the `--agents` option, *e.g.* `--agents ssh,gpg`. See [keychain(1)](http://jlk.fjfi.cvut.cz/arch/manpages/man/keychain.1).
