...
[Install](/index.php/Install "Install") the [timidity++](https://www.archlinux.org/packages/?name=timidity%2B%2B) package.
...
Add option `-in` or `-ig` for a text-based/GTK interface. E.g. as a Xfce/GNOME user you may want to set MIDI files to open with the custom command `timidity -ig`. There are many other options to TiMidity++. See [timidity(1)](https://jlk.fjfi.cvut.cz/arch/manpages/man/timidity.1) or use `-h` to get help.
