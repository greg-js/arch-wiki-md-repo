...
*   [9 DNS](#DNS)
    *   [9.1 The pull-resolv-conf custom scripts](#The_pull-resolv-conf_custom_scripts)
...
These scripts use the `resolvconf` command if present. [Systemd-resolvconf](/index.php/Systemd-resolvconf "Systemd-resolvconf") and [Openresolv](/index.php/Openresolv "Openresolv") both implement this command. See their wiki pages for more information on getting a working `resolvconf` implementation.
...
If you need to edit these scripts, copy them somewhere else and edit them there, so that your changes don't get overwritten by the next [openvpn](https://www.archlinux.org/packages/?name=openvpn) package upgrade. `/etc/openvpn/client` is a pretty good place.
