...
Installation is similar to most other Dell XPS machines although without modifications the machine will freeze a few seconds into boot.

To fix this one must blacklist the 'intel_lpss_pci' module, which can be done on a live media installation by selecting the boot option you want and pressing 'e', navigating to the end of the boot options string, and adding 'modprobe.blacklist=intel_lpss_pci'. This should allow booting into the full OS and further installation to the internal SSD.
...
In order to get Bluetooth working as of 12.09.2019 firmware drivers should be downloaded from [https://git.kernel.org/pub/scm/linux/kernel/git/firmware/linux-firmware.git/](https://git.kernel.org/pub/scm/linux/kernel/git/firmware/linux-firmware.git/) and the contents of the Intel folder should be placed /lib/firmware/intel.