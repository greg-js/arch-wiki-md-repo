...
The [rslsync-autoconfig](https://aur.archlinux.org/packages/rslsync-autoconfig/) package provides a systemd user service (`rslsync-autoconfig.service`) which, if enabled, triggers when a user's `rslsync.service` starts and creates a config file with default values if it does not already exist.

**Note:** If the config file was generated by [rslsync-autoconfig](https://aur.archlinux.org/packages/rslsync-autoconfig/) it will be configured with a different port. Rather than 8888, the port for the user's instance of `rslsync` will be `7889 + $UID`. If your `$UID` is "1000", the port will be 8889.
...
`rslsync-autoconfig.service` creates `~/.config/rslsync/rslsync.conf` if it does not exist, and guesses some default values of the settings:
