...
On some desktop environments the on screen keyboard might freeze when trying to input text after one or two characters. This is a problem with window focus. Check the settings for your [window manager](/index.php/Window_manager "Window manager") to see if it is possible to have focus follow the mouse or automatically focus new windows. Preventing the keyboard from receiving focus will fix the issue in [Awesome](/index.php/Awesome "Awesome"), see [Awesome#Steam_Keyboard](/index.php/Awesome#Steam_Keyboard "Awesome").
