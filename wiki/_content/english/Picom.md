[Picom](https://github.com/yshui/picom) is a standalone [compositor](/index.php/Compositor "Compositor") for [Xorg](/index.php/Xorg "Xorg"), suitable for use with [window managers](/index.php/Window_managers "Window managers") that do not provide compositing. Picom is a fork of [compton](https://github.com/chjj/compton/), which is a fork of [xcompmgr-dana](http://oliwer.net/xcompmgr-dana/), which in turn is a fork of [xcompmgr](/index.php/Xcompmgr "Xcompmgr").
...
[Install](/index.php/Install "Install") the [picom](https://www.archlinux.org/packages/?name=picom) package or [picom-git](https://aur.archlinux.org/packages/picom-git/) for the development version. For a [Qt](/index.php/Qt "Qt")-based configuration GUI install [compton-conf](https://aur.archlinux.org/packages/compton-conf/) or [compton-conf-git](https://aur.archlinux.org/packages/compton-conf-git/).

## Configuration

The default configuration is available in `/etc/xdg/picom.conf`. For modifications, it can be copied to `~/.config/picom/picom.conf` or `~/.config/picom.conf` or `~/.picom.conf`.
...
Many more options are available, including to set timing, displays to be managed, and the opacity of menus, window borders, and inactive application menus. See [picom(1)](https://jlk.fjfi.cvut.cz/arch/manpages/man/picom.1).

**Note:** If a different [composite manager](/index.php/Composite_manager "Composite manager") is running, it should be disabled before starting *picom*.
...
Where inactive window transparency has been enabled (the `-i` argument when running as a command), this may provide troublesome results when also using [slock](/index.php/Slock "Slock"). One solution is to amend the transparency to `0.2`. For example, where running picom arguments as a command:
...
Currently, picom is incompatible with `xsetroot`'s `-solid` option, a workaround is to use [hsetroot](https://www.archlinux.org/packages/?name=hsetroot) to set the background color:
