...
| **systemd** | This will install a basic systemd setup in your initramfs, and is meant to replace the *base*, *usr*, *udev* and *resume* hooks. For other hooks (like lvm2 or encrypt), it is **required** that you use the corresponding systemd specific hooks ("sd-*") from below. You also may wish to still include the *base* hook (before this hook) to ensure that a rescue shell exists on your initramfs. This hook also installs the service and binary helper needed for resuming from [hibernation](/index.php/Hibernation "Hibernation"). It is still recommended to use the standard (non-systemd) hooks, although this hook should work properly if you use the other sd-* hooks as needed.
...
*   **[dracut](https://en.wikipedia.org/wiki/dracut "w:dracut")** — A cross-distribution initramfs generation tool.
