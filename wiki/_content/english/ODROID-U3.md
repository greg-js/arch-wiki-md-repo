...
*   [3 CPU scaling and temperature management](#CPU_scaling_and_temperature_management)
    *   [3.1 Headless mode](#Headless_mode)
...
For servers (ie no need for a display), one can set the following in `/boot/boot.ini` to free up approx 300 M of memory at the cost of disabling the video subsystem:
