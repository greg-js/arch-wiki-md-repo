...
Some GNOME packages install [GConf schemas](https://en.wikipedia.org/wiki/GConf#Schemas "wikipedia:GConf"), even though many others already migrated to GSettings. Those packages should depend on [gconf](https://aur.archlinux.org/packages/gconf/).
...
Do not call `gconfpkg` in the .install file, as GConf schemas are automatically installed/removed (while installing/removing the GNOME package) via [pacman hooks](/index.php/Pacman_hooks "Pacman hooks") since [gconf](https://aur.archlinux.org/packages/gconf/)=3.2.6-4
