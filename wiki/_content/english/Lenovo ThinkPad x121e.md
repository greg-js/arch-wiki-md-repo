...
*   boot from GPT partitioned disk in UEFI mode (with hack to work around buggy firmware - see forum links [below](#Arch_Forums_x121e_related_threads))
...
*   SD card reader : In order to read SDHC cards, you need to install and run as daemon the [Ricoh e823 fix](https://aur.archlinux.org/packages.php?ID=52437)
...
If using laptop-mode-tools stops your machine from shutting down while on battery power, try the solution described in [this thread](https://bbs.archlinux.org/viewtopic.php?id=133108).
...
Wifi can be enabled using the [broadcom](/index.php/Broadcom "Broadcom") `brcmsmac` module. Be sure to rfkill-enable it and disable power saving :
...
Speed up trackpoint: [https://bbs.archlinux.org/viewtopic.php?id=130130](https://bbs.archlinux.org/viewtopic.php?id=130130).
