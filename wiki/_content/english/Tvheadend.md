...
Some people have issues accessing the web interface with the [Tvheadend-git](https://aur.archlinux.org/packages/Tvheadend-git/) package. If the default of no username and password is not working, then we can get around it by starting the service with the `tvheadend -C` command. This will let us create a username and password before restarting without -C. Begin by editing the services file
