...
If you want to force the EFI to use the integrated graphics card, then check the following topic: [force 2011 macbook to use integrated card](https://forums.macrumors.com/threads/force-2011-macbook-pro-8-2-with-failed-amd-gpu-to-always-use-intel-integrated-gpu-efi-variable-fix.2037591/)
...
The 8.2 macbook pro models with integrated graphics card tend to die after a while. The problem begins by seeing red lines on the monitor and then failing to boot OS X. Apple issued a [repair program](https://www.apple.com/uk/support/macbookpro-videoissues/), but the odds are that your GPU will die again.

It is still possible to boot linux using the outb 0x750 lines above, however this is a nuisance because it may be impossible to read the monitor when the problem is severe, making installation impossible. In addition, you may also wish to use another bootloader other than GRUB.
