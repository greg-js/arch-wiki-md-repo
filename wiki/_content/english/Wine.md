...
By default, installation of Wine does not create desktop menus/icons for the software which comes with Wine (e.g. for *winecfg*, *winebrowser*, etc). This can be achieved by installing [wine-installer](https://aur.archlinux.org/packages/wine-installer/) or [wine-installer-git](https://aur.archlinux.org/packages/wine-installer-git/) meta-package (the latter has no additional dependencies), otherwise these instructions will add entries for these applications.
