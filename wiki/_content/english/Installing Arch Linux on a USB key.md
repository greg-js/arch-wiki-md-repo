...
*   Before creating the initial RAM disk `# mkinitcpio -p linux`, in `/etc/mkinitcpio.conf` move the `block` hook in the hooks array to right after udev. This is necessary for appropriate module loading in early userspace. Without making this change, and running `# mkinitcpio -p linux`, your USB will fail to boot.
