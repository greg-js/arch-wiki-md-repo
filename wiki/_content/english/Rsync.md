...
**Note:** As of [rsync](https://www.archlinux.org/packages/?name=rsync)-3.1.2-5 the systemd unit `rsyncd.service` included in the package adds security feature `ProtectSystem=full` (`ProtectHome=on` has been undone in [rsync](https://www.archlinux.org/packages/?name=rsync)-3.1.2-8) under the `[Service]` section. This makes the `/boot/`, `/etc/` and `/usr/` directories read-only. If you need rsyncd write system directories you can [edit](/index.php/Edit "Edit") the unit and set `ProtectSystem=off` in the `[Service]` section of the overriding snippet.
