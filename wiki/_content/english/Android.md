...
**Note:** If you get a `no permissions` error or execution just hangs with `< waiting for any device >` then you need to run `fastboot` with `sudo`. Alternatively you can install [android-udev-git](https://aur.archlinux.org/packages/android-udev-git/).
