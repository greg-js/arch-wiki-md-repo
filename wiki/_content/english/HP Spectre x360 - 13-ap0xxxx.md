...
For the internal dual digital mic, it currently does not work with the legacy driver ( snd-hda-intel ) but works using snd-sof as described in the kernel bug [[2]](https://bugzilla.kernel.org/show_bug.cgi?id=201251#c73) . Hopefully will be merged fully by kernel 5.3-5.4 and will allow using the microphone. Fow now you can use [[3]](https://wiki.archlinux.org/index.php/HP_Spectre_x360_-_13-ap0xxxx#Advanced_/_Kernel_update)
...
You can also use this script [[4]](https://gist.github.com/Migacz85/3f544933ce5add438555ba7cd33f0413) but you have to change the line TOUCHPAD="ELAN Touchscreen" by PEN="ELAN2514:00 04F3:280E Pen (0)" and change everywhere the word TOUCHPAD by the word PEN. Indeed, the touchpad do not need to be remaped, since it is deactivated when the screen is rotated, however the PEN is.

WARNING sensor support is broken in 5.0+ kernels because of a kernel bug floating around that cause issues and error messages to be spammed. A simple fix is [[5]](https://lkml.org/lkml/2019/3/8/2) ( that patch got reverted in the current 5.2+ kernels, but can be reapplied safely ). The kernel at [[6]](https://wiki.archlinux.org/index.php/HP_Spectre_x360_-_13-ap0xxxx#Advanced_/_Kernel_update) is already patched and support autorotation.
...
This model only supports the S0ix, and so only S0 (s2idle) sleep mode is activated. Since this device use a Hynix SSD, it is affected by a bug [[8]](https://bugs.launchpad.net/ubuntu/+source/linux/+bug/1801875), resulting in 4-5% battery drain per hour during suspend. A patch set is available, but will not be merged [[9]](https://lore.kernel.org/patchwork/patch/1007283/). In any case, you can install the patched kernel from AUR [linux-hynix](https://aur.archlinux.org/packages/linux-hynix/) or compile the one at [[10]](https://wiki.archlinux.org/index.php/HP_Spectre_x360_-_13-ap0xxxx#Advanced_/_Kernel_update). The battery drain during suspend should drop to 1-2% per hour.

As it is mentioned by Intel [[11]](https://01.org/blogs/qwang59/2018/how-achieve-s0ix-states-linux), TLP may not work, and so it should not be used with S0ix.
