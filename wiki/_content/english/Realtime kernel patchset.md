This article describes the [Linux kernel](/index.php/Linux_kernel "Linux kernel") realtime patch set, and some utilities useful for trouble shooting scheduling latencies.
...
There are multiple -rt patched kernels available from the [AUR](/index.php/AUR "AUR"). The main two are [linux-rt](https://aur.archlinux.org/packages/linux-rt/) and [linux-rt-lts](https://aur.archlinux.org/packages/linux-rt-lts/), which both have a configuration based on the main [linux](https://www.archlinux.org/packages/?name=linux) kernel package. linux-rt follows the development branch of the -rt patch, while linux-rt-lts tracks a stable branch of the rt patchset. Both are also available through the [coderkun-aur-audio](/index.php/Unofficial_user_repositories#coderkun-aur-audio "Unofficial user repositories") unofficial user repository.

And then there also is [linux-rt-bfq](https://aur.archlinux.org/packages/linux-rt-bfq/), which is based on the development version of the -rt patch and includes the BFQ scheduler.

**Note:** Don't forget to add the newly installed kernel to your [boot loader](/index.php/Boot_loader "Boot loader")!
