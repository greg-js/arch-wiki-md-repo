[Ncmpcpp](http://unkart.ovh.org/ncmpcpp/) or ncmpcpp is an [mpd](/index.php/Mpd "Mpd") client (compatible with [mopidy](https://www.mopidy.com/)) with a UI very similar to *ncmpc*, but it provides new useful features such as support for regular expressions for library searches, extended song format, items filtering, the ability to sort playlists, and a local filesystem browser.
