...
iptables is a [systemd](/index.php/Systemd "Systemd") service and is [started](/index.php/Start "Start") accordingly. The Arch [iptables](https://www.archlinux.org/packages/?name=iptables) package installs an empty set of rules in `/etc/iptables/iptables.rules` which will be loaded when you [start](/index.php/Start "Start") the `iptables.service` unit for the first time. As with other services, if you want iptables to be loaded automatically on boot, you must [enable](/index.php/Enable "Enable") it.
