...
*   [1 Obecné](#Obecné)
    ...
    *   [1.2 Q) Proč bych měl chtít použivat Arch?](#Q)_Proč_bych_měl_chtít_použivat_Arch?)
    *   [1.3 Q) Proč bych neměl chtít používat Arch?](#Q)_Proč_bych_neměl_chtít_používat_Arch?)
    *   [1.4 Q) Na jaké distribuci je Arch založen?](#Q)_Na_jaké_distribuci_je_Arch_založen?)
    *   [1.5 Q) Jsem úplný linuxový začátečník. Měl bych používat Arch?](#Q)_Jsem_úplný_linuxový_začátečník._Měl_bych_používat_Arch?)
    *   [1.6 Q) Je Arch navržen pro použití jako server? Jako desktop? Jako pracovní stanice?](#Q)_Je_Arch_navržen_pro_použití_jako_server?_Jako_desktop?_Jako_pracovní_stanice?)
    *   [1.7 Q) Velmi se mi líbí Arch, ale tým vývojářů musí implementovat *"funkci X"*](#Q)_Velmi_se_mi_líbí_Arch,_ale_tým_vývojářů_musí_implementovat_"funkci_X")
    *   [1.8 Q) Kdy bude uvolněna další verze?](#Q)_Kdy_bude_uvolněna_další_verze?)
    *   [1.9 Q) Je Arch stabilní distribucí? Nedojde často k rozbití?](#Q)_Je_Arch_stabilní_distribucí?_Nedojde_často_k_rozbití?)
    *   [1.10 Q) Arch potřebuje více reklamy](#Q)_Arch_potřebuje_více_reklamy)
    *   [1.11 Q) Arch potřebuje více vývojářů](#Q)_Arch_potřebuje_více_vývojářů)
    *   [1.12 Q) Proč je můj internet tak pomalý ve srovnání s ostatními operačními systémy?](#Q)_Proč_je_můj_internet_tak_pomalý_ve_srovnání_s_ostatními_operačními_systémy?)
    *   [1.13 Q) Proč Arch využívá celou moji RAM?](#Q)_Proč_Arch_využívá_celou_moji_RAM?)
*   [2 Správa balíčků](#Správa_balíčků)
    *   [2.1 Q) Který balíček obsahuje XY?](#Q)_Který_balíček_obsahuje_XY?)
    *   [2.2 Q) Našel jsem chybu v balíčku XY, co bych měl udělat?](#Q)_Našel_jsem_chybu_v_balíčku_XY,_co_bych_měl_udělat?)
    *   [2.3 Q) Balíčky Archu potřebují jednotnou příponu. ".pkg.tar.gz" a ".pkg.tar.xz" jsou příliš dlouhé a/nebo matoucí](#Q)_Balíčky_Archu_potřebují_jednotnou_příponu._".pkg.tar.gz"_a_".pkg.tar.xz"_jsou_příliš_dlouhé_a/nebo_matoucí)
    *   [2.4 Q) Pacman potřebuje knihovnu, aby ostatní aplikace mohly mít snadný přístup k informacím o balíčku](#Q)_Pacman_potřebuje_knihovnu,_aby_ostatní_aplikace_mohly_mít_snadný_přístup_k_informacím_o_balíčku)
    *   [2.5 Q) Proč nemá pacman oficiální grafický (GUI) front-end?](#Q)_Proč_nemá_pacman_oficiální_grafický_(GUI)_front-end?)
    *   [2.6 Q) Pacman potřebuje funkci XY!](#Q)_Pacman_potřebuje_funkci_XY!)
    *   [2.7 Q) Arch potřebuje větev stabilních balíčků](#Q)_Arch_potřebuje_větev_stabilních_balíčků)
    *   [2.8 Q) Jaký je rozdíl mezi všemi repozitáři?](#Q)_Jaký_je_rozdíl_mezi_všemi_repozitáři?)
    *   [2.9 Q) Nainstaloval jsem balíček XY. Jak ho mám spustit?](#Q)_Nainstaloval_jsem_balíček_XY._Jak_ho_mám_spustit?)
    *   [2.10 Q) Podporuje pacman podepisování balíčků?](#Q)_Podporuje_pacman_podepisování_balíčků?)
*   [3 Instalace](#Instalace)
    *   [3.1 Q) Arch potřebuje instalátor. Možná grafický (GUI) instalátor.](#Q)_Arch_potřebuje_instalátor._Možná_grafický_(GUI)_instalátor.)
    *   [3.2 Q) Nainstaloval jsem Arch a teď jsem příkazové řádce. Co teď?](#Q)_Nainstaloval_jsem_Arch_a_teď_jsem_příkazové_řádce._Co_teď?)
    *   [3.3 Q) Které desktopové prostředí nebo správce oken bych měl použít?](#Q)_Které_desktopové_prostředí_nebo_správce_oken_bych_měl_použít?)
    *   [3.4 Q) Co činí Arch jedinečným mezi ostatními "minimálními" distribucemi?](#Q)_Co_činí_Arch_jedinečným_mezi_ostatními_"minimálními"_distribucemi?)
*   [4 Ostatní](#Ostatní)
    *   [4.1 Q) Co je to ta AUR věc o které pořád slýchám?](#Q)_Co_je_to_ta_AUR_věc_o_které_pořád_slýchám?)
