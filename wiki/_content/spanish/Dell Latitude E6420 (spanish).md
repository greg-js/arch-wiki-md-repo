**Estado de la traducción**
Este artículo es una traducción de [Dell Latitude E6420](/index.php/Dell_Latitude_E6420 "Dell Latitude E6420"), revisada por última vez el **2019-01-20**. Si advierte que la versión inglesa [ha cambiado](https://wiki.archlinux.org/index.php?title=Dell_Latitude_E6420&diff=0&oldid=564079) puede ayudar a actualizar la traducción, bien por [usted mismo](/index.php/ArchWiki:Translation_Team/Contributing_(Espa%C3%B1ol) "ArchWiki:Translation Team/Contributing (Español)") o bien avisando al [equipo de traducción](/index.php/ArchWiki:Translation_Team_(Espa%C3%B1ol) "ArchWiki:Translation Team (Español)").

Este artículo le explicará cómo hacer que los componentes básicos del portátil funcionen en Arch Linux.

## Hardware

Muy estable; todo parece funcionar muy bien.

**Compatible**

*   Hibernar/Reanudar
*   Teclas de volumen (teclas de hardware)
*   Teclas de brillo (Fn + flecha arriba/abajo)

**No compatible**

*   Lector de huellas dactilares

**No comprobado**

*   Bluetooth

## Instalación

Véase la [guía de instalación](/index.php/Installation_guide_(Espa%C3%B1ol) "Installation guide (Español)"). El arranque UEFI también funciona perfectamente una vez habilitado en la BIOS.

**Panel táctil**

Parece funcionar sin necesidad de configuración.

**GPU**

[Instale](/index.php/Install_(Espa%C3%B1ol) "Install (Español)") el paquete [nvidia-340xx](https://aur.archlinux.org/packages/nvidia-340xx/) desde [pacman](/index.php/Pacman_(Espa%C3%B1ol) "Pacman (Español)").