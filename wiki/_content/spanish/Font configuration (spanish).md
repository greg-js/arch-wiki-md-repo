...
Fontconfig reúne toda su configuración en un archivo principal (`/etc/fonts/fonts.conf`). Este archivo se reemplaza durante las actualizaciones y no se debe editar. Las aplicaciones compatibles con fontconfig obtienen este archivo para conocer las fuentes disponibles y saber cómo se procesan. Reiniciando simplemente las aplicaciones es suficiente para cargar la nueva configuración. Este archivo es un conjunto de reglas de la configuración global (`/etc/fonts/local.conf`), la configuración presente en `/etc/fonts/conf.d/`, y el archivo de configuración del usuario (`$XDG_CONFIG_HOME/fontconfig/fonts.conf`). `fc-cache` se puede utilizar para reconstruir la configuración de fontconfig, aunque los cambios solo serán visibles para las aplicaciones lanzadas a partir de ese momento.
...
Cuando utilize el renderizador subpixel ClearType, debe de activar el filtro LCD, que está diseñado para reducir la franja de color. Está descrito en [Filtrar LCD (en inglés)](https://www.freetype.org/freetype2/docs/reference/ft2-lcd_rendering.html) en la referencia de la API FreeType 2\. Las diferentes opciones están descritas en [FT_LcdFilter](https://www.freetype.org/freetype2/docs/reference/ft2-lcd_rendering.html#ft_lcdfilter), y están ilustradas en la página [test de filtros LCD](http://www.spasche.net/files/lcdfiltering/).
