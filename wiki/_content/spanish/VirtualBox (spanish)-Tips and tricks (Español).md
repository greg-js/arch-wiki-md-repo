...
*   [2 Gestionar el lanzamiento de la máquina virtual](#Gestionar_el_lanzamiento_de_la_máquina_virtual)
    *   [2.1 Iniciar máquinas virtuales con un servicio (autostart)](#Iniciar_máquinas_virtuales_con_un_servicio_(autostart))
...
*   [3 Utilizar dispositivos específicos en la máquina virtual](#Utilizar_dispositivos_específicos_en_la_máquina_virtual)
    *   [3.1 Utilizar webcam/micrófono USB](#Utilizar_webcam/micrófono_USB)
...
*   [9 Instalar MacOS de huésped](#Instalar_MacOS_de_huésped)
    *   [9.1 Sin entrada de teclado/ratón al intentar instalar Mojave](#Sin_entrada_de_teclado/ratón_al_intentar_instalar_Mojave)
    *   [9.2 Shell interactivo de UEFI después de reiniciar](#Shell_interactivo_de_UEFI_después_de_reiniciar)
*   [10 Mover una instalación nativa de Windows a una máquina virtual](#Mover_una_instalación_nativa_de_Windows_a_una_máquina_virtual)
    *   [10.1 Tareas en Windows](#Tareas_en_Windows)
    *   [10.2 Utilizar Disk2vhd para clonar la partición de Windows](#Utilizar_Disk2vhd_para_clonar_la_partición_de_Windows)
    *   [10.3 Tareas en GNU/Linux](#Tareas_en_GNU/Linux)
    *   [10.4 Arreglar el MBR y el gestor de arranque de Microsoft](#Arreglar_el_MBR_y_el_gestor_de_arranque_de_Microsoft)
    *   [10.5 Limitaciones conocidas](#Limitaciones_conocidas)
*   [11 Ejecutar una partición de Windows en VirtualBox](#Ejecutar_una_partición_de_Windows_en_VirtualBox)
    *   [11.1 Configurar udev para dar acceso sin formato a VirtualBox a las particiones de Windows](#Configurar_udev_para_dar_acceso_sin_formato_a_VirtualBox_a_las_particiones_de_Windows)
    *   [11.2 Configuración de VirtualBox](#Configuración_de_VirtualBox)
        *   [11.2.1 Crear archivos de imagen de disco virtual](#Crear_archivos_de_imagen_de_disco_virtual)
        *   [11.2.2 Adjuntar imágenes de disco virtual a la máquina virtual](#Adjuntar_imágenes_de_disco_virtual_a_la_máquina_virtual)
            *   [11.2.2.1 Configuración de permisos del sistema de archivos de alta seguridad](#Configuración_de_permisos_del_sistema_de_archivos_de_alta_seguridad)
            *   [11.2.2.2 Configuración de permisos del sistema de archivos de baja seguridad](#Configuración_de_permisos_del_sistema_de_archivos_de_baja_seguridad)
    *   [11.3 Configurar una ESP por separado en VirtualBox](#Configurar_una_ESP_por_separado_en_VirtualBox)
    *   [11.4 Configurar el firmware UEFI virtual para usar el cargador de arranque de Windows](#Configurar_el_firmware_UEFI_virtual_para_usar_el_cargador_de_arranque_de_Windows)
*   [12 Ejecutar un disco físico completo en Virtualbox](#Ejecutar_un_disco_físico_completo_en_Virtualbox)
*   [13 Establecer resolución de inicio del sistema huésped](#Establecer_resolución_de_inicio_del_sistema_huésped)
...
*   La importación o exportación de una máquina virtual a/desde VMware no es un problema en absoluto si utiliza el formato de disco VMDK o OVF, convirtiendo [VMDK a VDI y VDI a VMDK](/index.php/VirtualBox_(Espa%C3%B1ol)#Conversión_de_formatos_de_imagen_de_disco "VirtualBox (Español)") si es posible y tiene disponible la mencionada herramienta VMware vCenter de conversión.
*   La importación o exportación de una máquina virtual a/desde QEMU no es ningún problema: algunos formatos de QEMU tienen soporte directamente en VirtualBox y la conversión entre [QCOW2 a VDI y VDI a QCOW2](/index.php/VirtualBox_(Espa%C3%B1ol)#QCOW "VirtualBox (Español)") la tenemos disponible si es necesario.
...
### Iniciar máquinas virtuales con un servicio (autostart)
...
## Instalar MacOS de huésped

Antes de iniciar la máquina virtual, ejecute las siguientes órdenes en la máquina anfitrión [[1]](https://medium.com/@twister.mr/installing-macos-to-virtualbox-1fcc5cf22801):
...
### Sin entrada de teclado/ratón al intentar instalar Mojave

Si está intentando instalar Mojave, después de realizar los pasos mencionados anteriormente, el instalador se cargará, pero es posible que no pueda enviar la señal del teclado o del ratón. La razón parece ser que Mojave ya no es compatible con las controladoras de USB 1.1 y para solucionar el problema necesita emular USB 3.0\. Para hacerlo, primero instale el [VirtualBox pack Extension](/index.php/VirtualBox#Extension_pack "VirtualBox").

Luego vaya a *Machine > Settings > USB* y seleccione *USB 3.0*. La entrada de señal debería funcionar a partir de este punto.

### Shell interactivo de UEFI después de reiniciar

Si el instalador no puede formatear correctamente la unidad de arranque durante la instalación y le aboca a una shell de UEFI, ingrese lo siguiente:

1.  Escriba `exit` en el prompt de UEFI
2.  Seleccione *Boot Maintenance Manager*
3.  Seleccione *Boot From File*

Ahora nos adentraremos por un par de caminos PCI oscuros. El primero es el que acabamos de describir al intentar arrancar que no funcionó. El segundo (o tercero) debe ser el que tiene la partición de recuperación de MacOS que necesita cargar para continuar la instalación. Haga clic en la segunda entrada. Si está vacía, presione `Esc` para regresar y seleccionar la tercera entrada. Una vez que obtenga una con carpetas, haga clic en las carpetas. Debería mostrar algo así como *macOS Install Data > Locked Files > Boot Files > boot.efi*. Una vez que haga clic en intro sobre *boot.efi*, debería iniciar el instalador de MacOS y reanudar la instalación. Tenga en cuenta que algunos de los subdirectorios pueden faltar. Recuerde que debe obtener un `boot.efi`.[[2]](https://superuser.com/questions/1235970/stuck-on-uefi-interactive-shell-with-mac-os-x-high-sierra-vm)
...
### Utilizar Disk2vhd para clonar la partición de Windows

Inicie en Windows, limpie la instalación (con [CCleaner](http://www.piriform.com/ccleaner) por ejemplo), utilice la herramienta [disk2vhd](https://technet.microsoft.com/en-us/library/ee656415.aspx) tool to create a VHD image. Include a reserved system partition (if present) Incluya una partición reservada del sistema (si está presente) y la partición real de Windows (generalmente el disco C:). El tamaño de la imagen creada por Disk2vhd será la suma de los archivos reales presentes en la partición (espacio utilizado), no el tamaño de la partición completa. Si todo va bien, la imagen debería iniciarse en una máquina virtual y no tendrá que pasar por la molestia de lidiar con el MBR y el gestor de arranque de Windows, como en el caso de clonar una partición completa.

### Tareas en GNU/Linux

**Sugerencia:** omita las partes relacionadas con la partición si creó una imagen VHD con [Disk2vhd](#Utilizar_Disk2vhd_para_clonar_la_partición_de_Windows).
...
*   Instale VirtualBox en su sistema anfitrión GNU/Linux (vea [VirtualBox (Español)#Pasos para preparar Arch Linux como sistema anfitrión](/index.php/VirtualBox_(Espa%C3%B1ol)#Pasos_para_preparar_Arch_Linux_como_sistema_anfitrión "VirtualBox (Español)") si su sistema anfitrión es Arch Linux).
...
*   Como creó su imagen de disco como root, ajuste la propiedad correcta de la imagen de disco virtual:
...
```
# Reglas para dar a los usuarios de VirtualBox acceso sin formato a particiones de Windows

# Particiones reservadas de Microsoft (msftres)
SUBSYSTEM=="block", ENV{ID_PART_ENTRY_TYPE}=="e3c9e316-0b5c-4db8-817d-f92df00215ae", GROUP="vboxusers"

# Particiones de Windows (msftdata)
...
Para acceder a las particiones de Windows, cree un [archivo VMDK en bruto](https://www.virtualbox.org/manual/ch09.html#rawdisk-access-disk-partitions) que apunte a las particiones de Windows relevantes y establezca la propiedad:
...
##### Configuración de permisos del sistema de archivos de alta seguridad

Añada su usuario al grupo virtualbox:
...
Determine el UUID del disco de destino:
...
Cree una regla udev con el UUID encontrado:

 `/etc/udev/rules.d/99-vbox.rules` 
```
# Reglas para dar a los usuarios de VirtualBox acceso sin formato al disco de Windows	
# sdb	
ENV{ID_PART_ENTRY_TYPE}=="1234567-89ab-cdef-0123-456789abcde", GROUP="vboxusers"
```

##### Configuración de permisos del sistema de archivos de baja seguridad

Para poder agregar el archivo vmdk en «*Virtualbox Virtual Media Manager*» sin ejecutar VirtualBox como root, el usuario debe estar en los grupos `vboxusers` y `disk`.
...
Una vez que el disco se agrega al «*Virtual Media Manager*», es posible adjuntarlo a una máquina virtual a través de la interfaz.

**Advertencia:** tenga en cuenta las posibles implicaciones de seguridad de esto, ya que probablemente le otorgará a su cuenta de usuario acceso completo de lectura y escritura a algunos o todos de los dispositivos de almacenamiento.
...
## Ejecutar un disco físico completo en Virtualbox

**Nota:** puede consultar la documentación oficial de Virtualbox [9.8.1\. Utilizar un disco duro sin procesar como anfitrion desde un sistema huésped](https://www.virtualbox.org/manual/ch09.html#rawdisk).

Esto funciona de la misma manera que[#Ejecutar una partición de Windows en VirtualBox](#Ejecutar_una_partición_de_Windows_en_VirtualBox) pero el archivo vmdk contendrá el disco completo en lugar de una partición, por lo que no necesitará crear una partición ESP o MBR separada en el disco físico a utilizar.

Cree el disco sin formato:
...
Luego siga el mismo método que en [#Ejecutar una partición de Windows en VirtualBox](#Ejecutar_una_partición_de_Windows_en_VirtualBox) para la configuración y la conexión del disco virtual.

## Establecer resolución de inicio del sistema huésped

Puede cambiar la resolución de arranque de BIOS/UEFI utilizando la herramienta `VBoxManage`. Por ejemplo:
...
Las resoluciones recomendadas son 1280x720, 1920x1080, 2048x1080, 2560x1440, 3840x2160, 1280x800, 1280x1024, 1440x900, 1600x900.