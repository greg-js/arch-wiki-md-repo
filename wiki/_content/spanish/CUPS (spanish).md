...
*   [2 Instalar el paquete del cliente](#Instalar_el_paquete_del_cliente)
    *   [2.1 Configuración avanzada opcional para la red](#Configuración_avanzada_opcional_para_la_red)
...
*   [4 Configuración y soporte técnico del hardware](#Configuración_y_soporte_técnico_del_hardware)
    *   [4.1 Impresoras USB](#Impresoras_USB)
        *   [4.1.1 Introducir en blacklisting el módulo usblp](#Introducir_en_blacklisting_el_módulo_usblp)
...
*   [5 Configuración](#Configuración)
    ...
    *   [5.2 Interfaz web y herramientas](#Interfaz_web_y_herramientas)
        *   [5.2.1 Administración de CUPS](#Administración_de_CUPS)
        *   [5.2.2 Acceso remoto a la interfaz web](#Acceso_remoto_a_la_interfaz_web)
    *   [5.3 Configuración a través de línea de órdenes](#Configuración_a_través_de_línea_de_órdenes)
...
*   [7 Solución de Problemas](#Solución_de_Problemas)
    *   [7.1 Problemas relacionados con las actualizaciones](#Problemas_relacionados_con_las_actualizaciones)
        ...
        *   [7.1.2 Todos los trabajos están «detenidos»](#Todos_los_trabajos_están_«detenidos»)
        *   [7.1.3 Todos los trabajos derivan a «The printer is not responding» (La impresora no responde)](#Todos_los_trabajos_derivan_a_«The_printer_is_not_responding»_(La_impresora_no_responde))
        *   [7.1.4 La versión PPD no es compatible con gutenprint](#La_versión_PPD_no_es_compatible_con_gutenprint)
    *   [7.2 Otros](#Otros)
        *   [7.2.1 Errores de permisos CUPS](#Errores_de_permisos_CUPS)
        *   [7.2.2 La impresora HPLIP envía como error «/usr/lib/cups/backend/hp failed»](#La_impresora_HPLIP_envía_como_error_«/usr/lib/cups/backend/hp_failed»)
        ...
        *   [7.2.5 Qt instalado, pero hp-setup informa: "Qt/PyQt 4 initialization failed" (*«no se puede iniciar Qt/PyQt 4»*)](#Qt_instalado,_pero_hp-setup_informa:_"Qt/PyQt_4_initialization_failed"_(«no_se_puede_iniciar_Qt/PyQt_4»))
        *   [7.2.6 hp-setup encuentra la impresora automáticamente, pero informa que "Unable to communicate with device" (*«No se puede comunicar con el dispositivo»*) inmediatamente después de imprimir la página de prueba](#hp-setup_encuentra_la_impresora_automáticamente,_pero_informa_que_"Unable_to_communicate_with_device"_(«No_se_puede_comunicar_con_el_dispositivo»)_inmediatamente_después_de_imprimir_la_página_de_prueba)
        *   [7.2.7 hp-toolbox envía como error, «Unable to communicate with device» («No se puede comunicar con el dispositivo»)](#hp-toolbox_envía_como_error,_«Unable_to_communicate_with_device»_(«No_se_puede_comunicar_con_el_dispositivo»))
        *   [7.2.8 CUPS responde '«foomatic-rip» not available/stopped with status 3' con una impresora HP](#CUPS_responde_'«foomatic-rip»_not_available/stopped_with_status_3'_con_una_impresora_HP)
        *   [7.2.9 La impresora falla indicando error de autorización](#La_impresora_falla_indicando_error_de_autorización)
        *   [7.2.10 Botón de impresora bloqueado en aplicaciones GNOME](#Botón_de_impresora_bloqueado_en_aplicaciones_GNOME)
        *   [7.2.11 Formato soportado desconocido: application/postscript](#Formato_soportado_desconocido:_application/postscript)
        ...
        *   [7.2.14 Error /usr/lib/cups/backend/hp](#Error_/usr/lib/cups/backend/hp)
        *   [7.2.15 «Unable to get list of printer drivers»](#«Unable_to_get_list_of_printer_drivers»)
        *   [7.2.16 lp: Error - Scheduler Not Responding](#lp:_Error_-_Scheduler_Not_Responding)
        *   [7.2.17 CUPS imprime solo un vacío y una página de mensajes de error en HP LaserJet](#CUPS_imprime_solo_un_vacío_y_una_página_de_mensajes_de_error_en_HP_LaserJet)
        *   [7.2.18 Mensaje de error «Using invalid Host»](#Mensaje_de_error_«Using_invalid_Host»)
        *   [7.2.19 La impresora no funciona o devuelve como error «Filter failed» en la interfaz web de CUPS (HP printer)](#La_impresora_no_funciona_o_devuelve_como_error_«Filter_failed»_en_la_interfaz_web_de_CUPS_(HP_printer))
        *   [7.2.20 La impresora no imprime con un mensaje de "Filter failed" en la interfaz web de CUPS (impresora HP conectada por red)](#La_impresora_no_imprime_con_un_mensaje_de_"Filter_failed"_en_la_interfaz_web_de_CUPS_(impresora_HP_conectada_por_red))
        *   [7.2.21 HPLIP 3.13: el plugin está instalado, pero el administrador de dispositivos de HP se queja de que no es así](#HPLIP_3.13:_el_plugin_está_instalado,_pero_el_administrador_de_dispositivos_de_HP_se_queja_de_que_no_es_así)
        *   [7.2.22 La impresora no es reconocida por CUPS](#La_impresora_no_es_reconocida_por_CUPS)
        *   [7.2.23 No se puede cargar /etc/samba/smb.conf](#No_se_puede_cargar_/etc/samba/smb.conf)
        *   [7.2.24 El servicio systemd de CUPS no se inicia aunque está activado](#El_servicio_systemd_de_CUPS_no_se_inicia_aunque_está_activado)
...
asegúrese que [hplip](https://www.archlinux.org/packages/?name=hplip) se ha instalado, además de [los paquetes mencionados arriba](#Instalación). Consulte [este mensaje en el foro](https://bbs.archlinux.org/viewtopic.php?id=65615).
