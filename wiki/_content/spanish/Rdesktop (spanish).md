...
[rdesktop](http://www.rdesktop.org/) es un cliente gratuito de software libre lanzado bajo la licencia GNU General Public License para el protocolo propietario RDP de Microsoft. Usa rdesktop para administrar remotamente servidores RDP de Windows 2000/XP/Vista/Win7/Win10.

Desde julio de 2008, rdesktop implementa un gran conjunto de acciones para el protocolo RDP5, entre las que se encuentran:

*   Caché o cacheo de bitmap (bitmap caching).
...
*   Aplicaciones remotas como por ejempl el soporte modo "seamless" via SeamlessRDP
...
*   [4 Otros enlaces de interés](#Otros_enlaces_de_interés)

## Instalación

Instala [rdesktop](https://www.archlinux.org/packages/?name=rdesktop) desde los repositorios oficiales.

## Utilización

Para una lista completa de opciones revisa [rdesktop(1)](https://jlk.fjfi.cvut.cz/arch/manpages/man/rdesktop.1). Aquí puedes ver una sentencia típica:
...
Si no conoces la dirección IP de la máquina Windows, tienes que activar wins support. Para ello, debes instalar [samba](/index.php/Samba "Samba"). Activar wins en samba es sorprendentemente fácil: solo edita el fichero `/etc/samba/smb.conf` y añade la siguiente línea al mismo, o descoméntala si ya existiera:
...
Ahora debes instalar winbind, entonces edita el fichero `/etc/nsswitch.conf` y añade "wins" a la lista de hosts.

Reinicia los servicios `smbd` y `nmbd` y comprueba que tienes conexión realizando ping a un host Windows NetBIOS.

### Obteniendo cursores perdidos

Revisa [Cursor themes (Español)#Configurando temas de cursor](/index.php/Cursor_themes_(Espa%C3%B1ol)#Configurando_temas_de_cursor "Cursor themes (Español)").

## Otros enlaces de interés

*   [Web oficial de Rdesktop](http://www.rdesktop.org/)
*   [freerdp](https://www.archlinux.org/packages/?name=freerdp) Un fork de rdesktop que soporta funcionalidades de RDP 7.1 incluyendo autenticación de nivel de red (NLA). Revisa también [[1]](http://askubuntu.com/a/97932/217269).