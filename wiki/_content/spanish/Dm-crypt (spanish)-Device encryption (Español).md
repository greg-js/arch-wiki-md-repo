...
*   [7 Archivos de claves](#Archivos_de_claves)
    ...
    *   [7.5 Desbloquear la partición raíz en el arranque](#Desbloquear_la_partición_raíz_en_el_arranque)
        *   [7.5.1 Con un archivo de claves almacenado en un medio externo](#Con_un_archivo_de_claves_almacenado_en_un_medio_externo)
            *   [7.5.1.1 Configurar mkinitcpio](#Configurar_mkinitcpio)
            *   [7.5.1.2 Configurar parámetros del kernel](#Configurar_parámetros_del_kernel)
        *   [7.5.2 Con un archivo de clave incrustado en initramfs](#Con_un_archivo_de_clave_incrustado_en_initramfs)
...
*   `--type luks` para usar la versión de formato LUKS predeterminada (LUKS1 con [cryptsetup](https://www.archlinux.org/packages/?name=cryptsetup) < 2.1.0, LUKS2 con [cryptsetup](https://www.archlinux.org/packages/?name=cryptsetup) ≥ 2.1.0),
...
-b
 ...
 | tamaño real del disco de destino | `2048` (el dispositivo mapeado será de 512B×2048=1MiB) | Limite del tamaño máximo del dispositivo (en sectores de 512 bytes). |
| --offset

-o

 | `0` | `0` | El desplazamiento desde el principio del disco de destino (en sectores de 512 bytes) a partir del cual se inicia el mapeado |
| --skip

-p

 | `0` | `2048` (512B×2048=1MiB se saltará) | El número de sectores de datos cifrados de 512 bytes que se omiten al principio. |
| --key-file
...
**Advertencia:** GRUB no admite encabezados LUKS2; vea [GRUB bug #55093](https://savannah.gnu.org/bugs/?55093). Por lo tanto, si planea [desbloquear una partición de arranque cifrada con GRUB](/index.php/GRUB#Encrypted_/boot "GRUB"), especifique `--type luks1` en dispositivos encriptados a los que GRUB deberá acceder.
...
Suponga que un sistema de archivos loopback cifrado está almacenado en un archivo `/bigsecret`, vinculado a `/dev/loop0`, mapeado a `secret` y montado en `/mnt/secret`, como en el ejemplo en [dm-crypt (Español)/Encrypting a non-root file system (Español)#Dispositivo loop](/index.php/Dm-crypt_(Espa%C3%B1ol)/Encrypting_a_non-root_file_system_(Espa%C3%B1ol)#Dispositivo_loop "Dm-crypt (Español)/Encrypting a non-root file system (Español)").
...
**Advertencia:** asegúrese bien de usar **dos** `>`, en lugar de solo uno, de lo contrario, sobrescribirá el archivo en lugar de agregar contenido. Se recomienda hacer una copia de seguridad antes de este paso.
