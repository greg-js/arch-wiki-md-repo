...
**S4** (suspend-to-disk), зається, працює добре. **S3** (suspend-to-RAM) теж працює, але _тільки_ якщо немає uvesafb. Якщо він є, X-сервер помре одразу після пробудження, і вже не запуститься аж до перезапуска системи. Можливо, ситуація зміниться з майбутьніми версіями xf86-video-intel та uvesafb, але зараз використовувати їх разом не можна.
