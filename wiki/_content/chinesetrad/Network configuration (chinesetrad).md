...
*   [2 設定主機名稱](#設定主機名稱)
*   [3 裝置驅動程式](#裝置驅動程式)
    *   [3.1 檢查驅動程式狀態](#檢查驅動程式狀態)
    *   [3.2 載入裝置模組](#載入裝置模組)
*   [4 網路介面](#網路介面)
    *   [4.1 裝置名稱](#裝置名稱)
        *   [4.1.1 更改裝置名稱](#更改裝置名稱)
    *   [4.2 設定裝置的 MTU 與佇列長度](#設定裝置的_MTU_與佇列長度)
    *   [4.3 取得目前的裝置名稱](#取得目前的裝置名稱)
    *   [4.4 啟用與禁用網路介面](#啟用與禁用網路介面)
*   [5 組態設定 IP 位址](#組態設定_IP_位址)
    *   [5.1 動態 IP 位址](#動態_IP_位址)
        *   [5.1.1 dhcpcd](#dhcpcd)
    *   [5.2 靜態 IP 位址](#靜態_IP_位址)
        *   [5.2.1 手動指派](#手動指派)
        *   [5.2.2 永久於啟動時通過 systemd 進行組態設定與 udev 規則](#永久於啟動時通過_systemd_進行組態設定與_udev_規則)
        *   [5.2.3 計算位址](#計算位址)
    *   [5.3 systemd-networkd](#systemd-networkd)
*   [6 載入組態設定](#載入組態設定)
*   [7 附加設定](#附加設定)
    *   [7.1 筆記型電腦之 ifplugd](#筆記型電腦之_ifplugd)
    *   [7.2 Bonding 或 LAG](#Bonding_或_LAG)
    *   [7.3 IP 別名](#IP_別名)
        *   [7.3.1 範例](#範例)
    *   [7.4 更改 MAC/硬體 位址](#更改_MAC/硬體_位址)
    *   [7.5 網路共享](#網路共享)
    *   [7.6 路由組態設定](#路由組態設定)
    *   [7.7 區域網路主機名稱解析](#區域網路主機名稱解析)
    *   [7.8 雜亂模式](#雜亂模式)
*   [8 診斷](#診斷)
    ...
    *   [8.2 The TCP window scaling problem](#The_TCP_window_scaling_problem)
        ...
        *   [8.2.2 How to fix it (The bad way)](#How_to_fix_it_(The_bad_way))
        *   [8.2.3 How to fix it (The good way)](#How_to_fix_it_(The_good_way))
        *   [8.2.4 How to fix it (The best way)](#How_to_fix_it_(The_best_way))
        *   [8.2.5 More about it](#More_about_it)
    *   [8.3 Realtek no link / WOL problem](#Realtek_no_link_/_WOL_problem)
        *   [8.3.1 Method 1 - Enable the NIC directly in Linux](#Method_1_-_Enable_the_NIC_directly_in_Linux)
        *   [8.3.2 Method 2 - Rollback/change Windows driver](#Method_2_-_Rollback/change_Windows_driver)
        ...
        *   [8.3.5 Method 5 - Enable *LAN Boot ROM* in BIOS/CMOS](#Method_5_-_Enable_LAN_Boot_ROM_in_BIOS/CMOS)
...
If this method does not work, you will need to clone the MAC address of the original machine. See also [Change MAC/hardware address](/index.php/Configuring_Network#Change_MAC/hardware_address "Configuring Network").
