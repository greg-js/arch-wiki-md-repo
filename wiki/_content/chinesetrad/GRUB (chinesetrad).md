...
*   [1 前言](#前言)
    ...
    *   [1.2 Preliminary Requirements for GRUB2](#Preliminary_Requirements_for_GRUB2)
        ...
        *   [1.2.2 UEFI 系統](#UEFI_系統)
            *   [1.2.2.1 創建並掛載到UEFI系統分割區(Create and Mount the UEFI SYSTEM PARTITION)](#創建並掛載到UEFI系統分割區(Create_and_Mount_the_UEFI_SYSTEM_PARTITION))
*   [2 安裝](#安裝)
    *   [2.1 BIOS 系統](#BIOS_系統_2)
        *   [2.1.1 備份重要資料(Backup Important Data)](#備份重要資料(Backup_Important_Data))
        *   [2.1.2 安裝grub-bios套件(Install grub-bios package)](#安裝grub-bios套件(Install_grub-bios_package))
        *   [2.1.3 安裝grub-bios啟動檔案(Install grub-bios boot files)](#安裝grub-bios啟動檔案(Install_grub-bios_boot_files))
            *   [2.1.3.1 安裝到440位元組MBR啟動區(Install to 440-byte MBR boot code region)](#安裝到440位元組MBR啟動區(Install_to_440-byte_MBR_boot_code_region))
            *   [2.1.3.2 Install to Partition or Partitionless Disk](#Install_to_Partition_or_Partitionless_Disk)
            *   [2.1.3.3 單獨產生core.img(Generate core.img alone)](#單獨產生core.img(Generate_core.img_alone))
        *   [2.1.4 產生GRUB2 BOIS設置檔案(Generate GRUB2 BIOS Config file)](#產生GRUB2_BOIS設置檔案(Generate_GRUB2_BIOS_Config_file))
        *   [2.1.5 在BIOS多重啟動(Multiboot in BIOS)](#在BIOS多重啟動(Multiboot_in_BIOS))
            *   [2.1.5.1 在BIOS-MBR模式中啟動已安裝的Microsoft Windows(Boot Microsoft Windows installed in BIOS-MBR mode)](#在BIOS-MBR模式中啟動已安裝的Microsoft_Windows(Boot_Microsoft_Windows_installed_in_BIOS-MBR_mode))
    *   [2.2 UEFI系統(UEFI systems)](#UEFI系統(UEFI_systems))
        *   [2.2.1 安裝grub-uefi套件(Install grub-uefi package)](#安裝grub-uefi套件(Install_grub-uefi_package))
        *   [2.2.2 安裝grub-uefi啟動檔案(Install grub-uefi boot files)](#安裝grub-uefi啟動檔案(Install_grub-uefi_boot_files))
        ...
        *   [2.2.4 產生GRUB2 UEFI設置檔案(Generate GRUB2 UEFI Config file)](#產生GRUB2_UEFI設置檔案(Generate_GRUB2_UEFI_Config_file))
...
*   [3 設置(Configuration)](#設置(Configuration))
    *   [3.1 使用grub-mkconfig自動產生(Automatically generating using grub-mkconfig) (Recommended)](#使用grub-mkconfig自動產生(Automatically_generating_using_grub-mkconfig)_(Recommended))
        *   [3.1.1 Additional arguments](#Additional_arguments)
    *   [3.2 手動創建grub.cfg( Manually creating grub.cfg)](#手動創建grub.cfg(_Manually_creating_grub.cfg))
    *   [3.3 雙啟動(dual-boot)](#雙啟動(dual-boot))
        *   [3.3.1 使用 grub-mkconfig](#使用_grub-mkconfig)
            *   [3.3.1.1 與 GNU/Linux](#與_GNU/Linux)
            *   [3.3.1.2 與 FreeBSD](#與_FreeBSD)
            *   [3.3.1.3 與 Windows](#與_Windows)
    ...
    *   [3.4 Visual Configuration](#Visual_Configuration)
        ...
        *   [3.4.3 背景圖片和字體(Background image and bitmap fonts)](#背景圖片和字體(Background_image_and_bitmap_fonts))
        *   [3.4.4 佈景主題(Theme)](#佈景主題(Theme))
        *   [3.4.5 選單顏色(Menu colors)](#選單顏色(Menu_colors))
        *   [3.4.6 隱藏選單(Hidden menu)](#隱藏選單(Hidden_menu))
        *   [3.4.7 關閉幀緩衝(Disable framebuffer)](#關閉幀緩衝(Disable_framebuffer))
    *   [3.5 其他選項(Other Options)](#其他選項(Other_Options))
        *   [3.5.1 LVM](#LVM)
        *   [3.5.2 RAID](#RAID)
        ...
        *   [3.5.6 安全(Security)](#安全(Security))
        *   [3.5.7 Root Encryption](#Root_Encryption)
    *   [3.6 在GRUB2啟動(ISOBooting an ISO Directly From GRUB2)](#在GRUB2啟動(ISOBooting_an_ISO_Directly_From_GRUB2))
...
*   [6 parttool or legacy hide/unhide](#parttool_or_legacy_hide/unhide)
...
*   [10 參考資料(References)](#參考資料(References))
*   [11 外部連結(External Links)](#外部連結(External_Links))
...
*   [#安裝到440位元組MBR啟動區(Install to 440-byte MBR boot code region)](#安裝到440位元組MBR啟動區(Install_to_440-byte_MBR_boot_code_region)) (建議) ,
