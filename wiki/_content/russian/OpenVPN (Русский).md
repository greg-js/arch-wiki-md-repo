...
OpenVPN -- мощный и гибкий кроссплатформенный [VPN](https://en.wikipedia.org/wiki/ru:VPN "wikipedia:ru:VPN") демон, поддерживающий [SSL/TLS](https://en.wikipedia.org/wiki/ru:TLS "wikipedia:ru:TLS") безопасность, [сетевые мосты](https://en.wikipedia.org/wiki/ru:%D0%A1%D0%B5%D1%82%D0%B5%D0%B2%D0%BE%D0%B9_%D0%BC%D0%BE%D1%81%D1%82 "wikipedia:ru:Сетевой мост"), [TCP](https://en.wikipedia.org/wiki/ru:TCP "wikipedia:ru:TCP") или [UDP](https://en.wikipedia.org/wiki/ru:UDP "wikipedia:ru:UDP") [туннелирование](https://en.wikipedia.org/wiki/ru:%D0%A2%D1%83%D0%BD%D0%BD%D0%B5%D0%BB%D0%B8%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5_(%D0%BA%D0%BE%D0%BC%D0%BF%D1%8C%D1%8E%D1%82%D0%B5%D1%80%D0%BD%D1%8B%D0%B5_%D1%81%D0%B5%D1%82%D0%B8) "wikipedia:ru:Туннелирование (компьютерные сети)") через [прокси](https://en.wikipedia.org/wiki/ru:%D0%9F%D1%80%D0%BE%D0%BA%D1%81%D0%B8-%D1%81%D0%B5%D1%80%D0%B2%D0%B5%D1%80 "wikipedia:ru:Прокси-сервер") или [NAT](https://en.wikipedia.org/wiki/ru:NAT "wikipedia:ru:NAT"), поддерживает [DHCP](https://en.wikipedia.org/wiki/ru:DHCP "wikipedia:ru:DHCP"), масштабируется на сотни и тысячи пользователей.
...
Публичный сертификат _ca.crt_ нужен как серверу так и клиенту. Ключ _ca.key_ секретный используется только машиной, генерируемой ключи и сертификаты.

Для сервера необходимы server.crt, dh2048.pem server.key и _ta.key_.

Для клиента нужны _client.crt_, _client.key_, и _ta.key_.
...
То, что действительно отличает сервер от клиента (помимо используемого типа сертификата) является сам конфигурационный файл.OpenVPN демон при запуске считывает все файлы конфигурации _.conf_ в `/etc/openvpn` и запускается в соответсвии с ними . Если он находит более одного файла конфигурации, он запустит один процесс OpenVPN на каждый файл конфигурации.
...
Синтаксис автозапуска OpenVPN : `systemctl [enable](/index.php/Daemon "Daemon") openvpn@_<configuration>_`.
...
Для переадресации хост NIC (к примеру enp1s0) должен быть установлен в таком состоянии, чтобы он мог принимать пакеты с отличных от настроенного IP-адреса, называемом [неразборчивый режим](https://en.wikipedia.org/wiki/ru:Promiscuous_mode "wikipedia:ru:Promiscuous mode"). Установите `enp1s0` в неразборчивый режим средствами systemd, используя [этот файл службы](/index.php/Network_configuration_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#.22.D0.9D.D0.B5.D1.80.D0.B0.D0.B7.D0.B1.D0.BE.D1.80.D1.87.D0.B8.D0.B2.D1.8B.D0.B9.22_.D1.80.D0.B5.D0.B6.D0.B8.D0.BC "Network configuration (Русский)") и задействуйте его, выполнив:
...
*   Для маршрутизации других локальных сетей со стороны сервера к клиенту, укажите их в параметре _**push**_ файла конфигурации сервера. Однако имейте в виду, что эти сети также должны знать, как установить связь с клиентом VPN.
...
Создайте в этом каталоге файл, назвав его _bugs_, содержащий `iroute 192.168.4.0 255.255.255.0` параметр. Это укажет серверу направлять пакеты с подсети к клиенту
...
*   Для маршрутизации других LAN от клиента к серверу,укажите их в параметрах _**iroute**_ и _**route**_ в соответствующих файлах конфигурации. Однако имейте в виду, что эти подсети также должны знать, как установить связь с сервером VPN
...
По умолчанию клиенты не видят друг друга. Чтобы разрешить обмениваться пакетами между клиентами и клиентскими сетями, добавьте _client-to-client_ в файл конфигурации сервера: `/etc/openvpn/server.conf`  `client-to-client` 
...
_Caveat: The script will fail to restore the original DNS settings if your OpenVPN client.conf is set-up to drop root privileges after connection._
...
# systemctl restart openvpn@_profile_
