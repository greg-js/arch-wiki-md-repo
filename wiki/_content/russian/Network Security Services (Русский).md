...
Для управления сертификатами используйте утилиту _certutil_, входящую в состав пакета NSS.
...
```
$ certutil -d sql:$HOME/.pki/nssdb -L -n _имя_сертификата_
...
```
$ certutil -d sql:$HOME/.pki/nssdb -A -t "_TRUSTARGS_" -n _имя_сертификата_ -i _/путь/к/файлу/сертификата_
...
```
$ pk12util -d sql:$HOME/.pki/nssdb -i _/путь/к/файлу/сертификата/PKCS12.p12_
...
Вызовите _certutil_ с опцией `-M` для редактирования параметров сертификата. Например, для редактирования списка `TRUSTARGS`:

```
$ certutil -d sql:$HOME/.pki/nssdb -M -t "_TRUSTARGS_" -n _имя_сертификата_
...
```
$ certutil -d sql:$HOME/.pki/nssdb -D -n _имя_сертификата_
...
Устанавливаем корневой сертификат (вместо _WebMoneyTransferRootCA.crt_ введите путь к файлу корневого сертификата):

```
$ certutil -d sql:$HOME/.pki/nssdb -A -t "C,," -n "WebmoneyCA" -i _WebMoneyTransferRootCA.crt_

```

Устанавливаем личный сертификат (вместо _PersonalCert.p12_ введите путь к файлу личного сертификата):

```
$ pk12util -d sql:$HOME/.pki/nssdb -i _PersonalCert.p12_
