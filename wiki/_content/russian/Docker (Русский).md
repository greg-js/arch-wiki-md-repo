...
Если необходимо запускать _docker_ от обычного пользователя, добавьте его в группу `docker` и перелогиньтесь:
...
**Обратите внимание:** Предполагается, что адрес используемого прокси-сервера `192.168.1.1`; не используйте `127.0.0.1`.
...
Начиная с версии 0.9.0, Docker предоставляет новый способ запуска контейнеров без необходимости в LXC, называемый _libcontainer_.
