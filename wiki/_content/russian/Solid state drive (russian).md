...
*   [1 Обзор](#Обзор)
    *   [1.1 Преимущества перед HDD](#Преимущества_перед_HDD)
    *   [1.2 Недостатки](#Недостатки)
    *   [1.3 На что обращать внимание перед покупкой](#На_что_обращать_внимание_перед_покупкой)
*   [2 Советы по увеличению производительности SSD](#Советы_по_увеличению_производительности_SSD)
    *   [2.1 Выравнивание разделов](#Выравнивание_разделов)
    *   [2.2 TRIM](#TRIM)
        *   [2.2.1 Проверьте, поддерживается ли TRIM](#Проверьте,_поддерживается_ли_TRIM)
        *   [2.2.2 Включите TRIM с помощью флагов монтирования](#Включите_TRIM_с_помощью_флагов_монтирования)
        *   [2.2.3 Применение TRIM по раписанию cron](#Применение_TRIM_по_раписанию_cron)
        *   [2.2.4 Применение TRIM по systemd таймеру](#Применение_TRIM_по_systemd_таймеру)
        *   [2.2.5 Включение TRIM с помощью tune2fs (Discouraged)](#Включение_TRIM_с_помощью_tune2fs_(Discouraged))
        *   [2.2.6 Включение TRIM для LVM](#Включение_TRIM_для_LVM)
        *   [2.2.7 Включение TRIM для dm-crypt](#Включение_TRIM_для_dm-crypt)
    *   [2.3 Планировщик ввода/вывода](#Планировщик_ввода/вывода)
        *   [2.3.1 Параметр ядра (для единственного накопителя)](#Параметр_ядра_(для_единственного_накопителя))
        *   [2.3.2 Использование udev как для одного накопителя, так и для нескольких разных](#Использование_udev_как_для_одного_накопителя,_так_и_для_нескольких_разных)
    *   [2.4 Разделы подкачки на SSD](#Разделы_подкачки_на_SSD)
    *   [2.5 Hdparm shows "frozen" state](#Hdparm_shows_"frozen"_state)
    *   [2.6 Очистка ячеек памяти SSD](#Очистка_ячеек_памяти_SSD)
    *   [2.7 Resolving NCQ Errors](#Resolving_NCQ_Errors)
*   [3 Советы для уменьшения операций чтения/записи](#Советы_для_уменьшения_операций_чтения/записи)
    *   [3.1 Продуманная схема разделов](#Продуманная_схема_разделов)
    *   [3.2 Опция монтирования noatime](#Опция_монтирования_noatime)
    *   [3.3 Расположите часто используемые файлы в оперативной памяти](#Расположите_часто_используемые_файлы_в_оперативной_памяти)
        *   [3.3.1 Профили браузеров](#Профили_браузеров)
        *   [3.3.2 Другие файлы](#Другие_файлы)
    *   [3.4 Компиляция в tmpfs](#Компиляция_в_tmpfs)
    *   [3.5 Отключение журналирования ФС](#Отключение_журналирования_ФС)
*   [4 Выбор файловой системы](#Выбор_файловой_системы)
    *   [4.1 Btrfs](#Btrfs)
    *   [4.2 Ext4](#Ext4)
    *   [4.3 XFS](#XFS)
    *   [4.4 JFS](#JFS)
    *   [4.5 Другие файловые системы](#Другие_файловые_системы)
*   [5 Обновление прошивок](#Обновление_прошивок)
    *   [5.1 OCZ](#OCZ)
...
Раздел [#Выбор файловой системы](#Выбор_файловой_системы) в этой статье содержит более подробную информацию.
...
Пакет [util-linux](https://www.archlinux.org/packages/?name=util-linux) предоставляет [systemd](/index.php/Systemd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Systemd (Русский)") юнит файлы `fstrim.service` и `fstrim.timer` . Если [включить](/index.php/Systemd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Использование_юнитов "Systemd (Русский)") таймер, то сервис будет активироваться еженедельно, подравнивая все примонтированные файловые системы на устройствах, поддерживающих операцию discard.
...
1.  Update the firmware on the SSD. See [SSD#Firmware updates](/index.php/SSD#Firmware_updates "SSD").
2.  Update the BIOS/UEFI on the motherboard. See [Flashing BIOS from Linux](/index.php/Flashing_BIOS_from_Linux "Flashing BIOS from Linux").
...
Монтируйте разделы SSD с опцией `noatime`. См. раздел [Параметры монтирования](#Опция_монтирования_noatime).
