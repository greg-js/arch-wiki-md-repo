...
*   [GTK (Русский)](/index.php/GTK_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "GTK (Русский)")
...
*   Для запуска сессии GNOME on Xorg добавьте следующее в файл `~/.xinitrc` (смотрите [здесь](https://gitlab.gnome.org/GNOME/gtk/issues/1390#note_344758) для получения подробностей):
    ```
...
*   Пакет [xorg-server-xwayland](https://www.archlinux.org/packages/?name=xorg-server-xwayland) все еще нужен даже для запуска тех приложений, которые не портированы на [Wayland](/index.php/Wayland_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Wayland (Русский)"). Приложения, которые используют определённые графические библиотеки, такие как Qt, можно принудительно запускать с использованием Wayland при помощи установки переменных среды. См. [Wayland#GUI libraries](/index.php/Wayland#GUI_libraries "Wayland") для получения более детальной информации.
*   При использовании проприетарного драйвера [NVIDIA](/index.php/NVIDIA "NVIDIA") ненативные приложения Wayland имеют плохую производительность из-за отсутствия аппаратного ускорения XWayland. Ожидается, что эта [проблема](https://blogs.gnome.org/uraeus/2019/09/23/fedora-workstation-31-whats-new/) будет решена весной 2020 года, но точные сроки от [NVIDIA](/index.php/NVIDIA "NVIDIA") не поступали.
...
Как вы могли заметить выше, многие параметры, такие как изменение темы [GTK](/index.php/GTK "GTK") или [оконного менеджера](/index.php/%D0%9E%D0%BA%D0%BE%D0%BD%D0%BD%D1%8B%D0%B9_%D0%BC%D0%B5%D0%BD%D0%B5%D0%B4%D0%B6%D0%B5%D1%80 "Оконный менеджер"), недоступны в панели управления GNOME (*gnome-control-center*). Пользователи, желающие настроить эти параметры, возможно, захотят использовать GNOME Tweaks ([gnome-tweaks](https://www.archlinux.org/packages/?name=gnome-tweaks) - удобная графическая утилита, которая предоставляет доступ к множеству параметров.
...
Смотрите [GTK#Темы](/index.php/GTK_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Темы "GTK (Русский)") и [Icons#Manually](/index.php/Icons#Manually "Icons").
...
или через [gnome-tweaks](https://www.archlinux.org/packages/?name=gnome-tweaks), отключив опцию *Верхняя панель > Активный угол меню Обзор*
...
**Примечание:** Расширения по адресу [extensions.gnome.org](https://extensions.gnome.org) могут сразу же быть установлены с помощью [gnome-software](https://www.archlinux.org/packages/?name=gnome-software). Для других браузеров необходимо [установить](/index.php/%D0%A3%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%B8%D1%82%D1%8C "Установить") [chrome-gnome-shell](https://www.archlinux.org/packages/?name=chrome-gnome-shell) и соответствующее вашему браузеру расширение.
