[Arch Build System](/index.php/Arch_Build_System_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Arch Build System (Русский)") позволит вам собрать собственное ядро на основе официального пакета Linux. Этот метод предлагает автоматизировать весь процесс компиляции и основан на тщательно протестированном пакете. Вы можете отредактировать PKGBUILD для использования собственных настроек ядра или для добавления дополнительных патчей.
...
Поскольку сборка осуществляется при помощи [makepkg](/index.php/Makepkg_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Makepkg (Русский)"), необходимо следовать рекомендациям ниже. К примеру, вы не сможете запустить makepkg от имени root/sudo, поэтому для начала создадим папку `build` в домашней директории пользователя.
...
*   [Одновременный запуск нескольких процессов компиляции](/index.php/Makepkg_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Параллельная_компиляция "Makepkg (Русский)") может значительно сократить время компиляции на многоядерных системах.
...
*   Исходные коды ядра [подписаны PGP](https://www.kernel.org/signature.html#kernel-org-web-of-trust), и makepkg проверит это. Смотрите [makepkg#Проверка цифровых подписей](/index.php/Makepkg_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Проверка_цифровых_подписей "Makepkg (Русский)") для получения подробностей.
