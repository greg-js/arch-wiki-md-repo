...
**Обратите внимание:** По крайней мере, убедитесь, что блок *if* в примере выше, присутствует в вашем файле `.xinitrc` для того чтобы подать скрипт `/etc/X11/xinit/xinitrc.d/30-dbus.sh`. Иначе сессия [D-Bus](/index.php/D-Bus "D-Bus") не будет запущена.
...
**Обратите внимание:** *pkill* убьет все запущенные экземпляры X. Для специального убивания оконного менеджера на текущем VT, используйте:
```
