**Состояние перевода:** На этой странице представлен перевод статьи [PulseAudio/Examples](/index.php/PulseAudio/Examples "PulseAudio/Examples"). Дата последней синхронизации: 16 февраля 2018\. Вы можете [помочь](/index.php/ArchWiki_Translation_Team_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "ArchWiki Translation Team (Русский)") синхронизировать перевод, если в английской версии произошли [изменения](https://wiki.archlinux.org/index.php?title=PulseAudio/Examples&diff=0&oldid=510342).
...
*   [1 Настройка стандартных устройств ввода](#Настройка_стандартных_устройств_ввода)
*   [2 Установка стандартного устройства вывода](#Установка_стандартного_устройства_вывода)
*   [3 Одновременный HDMI и аналоговый вывод](#Одновременный_HDMI_и_аналоговый_вывод)
*   [4 Настройка HDMI вывода](#Настройка_HDMI_вывода)
    *   [4.1 Поиск HDMI вывода](#Поиск_HDMI_вывода)
    *   [4.2 Проверка на правильную карту](#Проверка_на_правильную_карту)
    *   [4.3 Ручная настройка PulseAudio, чтобы он находил HDMI Nvidia](#Ручная_настройка_PulseAudio,_чтобы_он_находил_HDMI_Nvidia)
    *   [4.4 Автоматическое переключение звука на HDMI](#Автоматическое_переключение_звука_на_HDMI)
*   [5 Настройка объёмного звука](#Настройка_объёмного_звука)
    *   [5.1 Разделение передний/задний](#Разделение_передний/задний)
    *   [5.2 Разделение 7.1 на 5.1+2.0](#Разделение_7.1_на_5.1+2.0)
    *   [5.3 Отключение ремиксинга LFE](#Отключение_ремиксинга_LFE)
    *   [5.4 Бинауральные наушники](#Бинауральные_наушники)
*   [6 PulseAudio через сеть](#PulseAudio_через_сеть)
    *   [6.1 Поддержка TCP (звук через сеть)](#Поддержка_TCP_(звук_через_сеть))
    *   [6.2 Поддержка TCP с анонимными клиентами](#Поддержка_TCP_с_анонимными_клиентами)
    *   [6.3 Zeroconf (Avahi)](#Zeroconf_(Avahi))
    *   [6.4 Переключение PulseAudio сервера используя локальные клиенты Х](#Переключение_PulseAudio_сервера_используя_локальные_клиенты_Х)
    *   [6.5 Выбор сервера](#Выбор_сервера)
    *   [6.6 Если всё остальное не работает](#Если_всё_остальное_не_работает)
*   [7 ALSA monitor source](#ALSA_monitor_source)
*   [8 Наблюдение конкретного вывода](#Наблюдение_конкретного_вывода)
*   [9 PulseAudio через JACK](#PulseAudio_через_JACK)
    *   [9.1 Метод KXStudio](#Метод_KXStudio)
    *   [9.2 Ручная настройка устройств вывода](#Ручная_настройка_устройств_вывода)
    *   [9.3 Настройка с помощью скриптов](#Настройка_с_помощью_скриптов)
    *   [9.4 Путем остановки PulseAudio](#Путем_остановки_PulseAudio)
*   [10 PulseAudio через OSS](#PulseAudio_через_OSS)
*   [11 Запуск PulseAudio из chroot](#Запуск_PulseAudio_из_chroot)
*   [12 Отключение автоматического запуска сервера PulseAudio](#Отключение_автоматического_запуска_сервера_PulseAudio)
*   [13 Отключение демона pulseaudio полностью](#Отключение_демона_pulseaudio_полностью)
*   [14 Изменить стерео на моно](#Изменить_стерео_на_моно)
*   [15 Поменять левый/правый каналы](#Поменять_левый/правый_каналы)
*   [16 PulseAudio в качестве ненавязчивого проводника ALSA](#PulseAudio_в_качестве_ненавязчивого_проводника_ALSA)
*   [17 Наушники и колонки подключены одновременно и переключаются приложением на лету](#Наушники_и_колонки_подключены_одновременно_и_переключаются_приложением_на_лету)
*   [18 Одновременное использование PulseAudio несколькими пользователями](#Одновременное_использование_PulseAudio_несколькими_пользователями)
...
На обеих машинах [запустите](/index.php/Systemd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Использование_юнитов "Systemd (Русский)") и [включите](/index.php/Systemd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Использование_юнитов "Systemd (Русский)") systemd юнит `avahi-daemon`.
...
PulseAduio выберет путь к сокету через XDG_RUNTIME_DIR, так что удостовертесь что перенесли его тоже с помощью sudo, когда используете chroot (см. [Sudo#Environment variables](/index.php/Sudo_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Переменные_окружения "Sudo (Русский)")).
