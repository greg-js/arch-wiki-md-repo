...
**Обратите внимание:** Настройки php-fpm находятся в файле `/etc/php/php-fpm.conf`. [Подробнее о настройках php-fpm](http://php-fpm.org/wiki/Configuration_File).
...
Аналогично предыдущему описанию включаем этот конфиг в `/etc/lighttpd/lighttpd.conf` строкой
 ...
 `/etc/lighttpd/lighttpd.conf`  `include "conf.d/fastcgi-php.conf"` 
**Обратите внимание:** Демон php-fpm необходимо запускать до lighttpd, иначе php-скрипты не будут обрабатываться (будут выдаваться ошибки 500 или 503).
...
**Обратите внимание:** Подробнее о создании сертификатов смотрите отдельную [статью](/index.php/OpenSSL "OpenSSL").
...
Настройка поддержки php описана [выше](/index.php/Lighttpd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#PHP "Lighttpd (Русский)").
...
Настройка поддержки php описана [выше](/index.php/Lighttpd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#PHP "Lighttpd (Русский)"). УБедитесь, что необходимые директории перечислены в директиве open_basedir в `/etc/php/php.ini`
...
**Tip:** Тоже самое Вы можете проделать через web-интерфейс [phpMyAdmin](/index.php/Lighttpd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#phpMyAdmin "Lighttpd (Русский)").
