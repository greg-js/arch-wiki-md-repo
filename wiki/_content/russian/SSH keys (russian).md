...
*   [2 Генерация пары ключей SSH](#Генерация_пары_ключей_SSH)
    *   [2.1 Выбор типа шифрования](#Выбор_типа_шифрования)
        *   [2.1.1 ECDSA](#ECDSA)
        *   [2.1.2 Ed25519](#Ed25519)
    *   [2.2 Выбор места расположения ключа и пароля](#Выбор_места_расположения_ключа_и_пароля)
        *   [2.2.1 Изменение пароля закрытого ключа без изменения самого ключа](#Изменение_пароля_закрытого_ключа_без_изменения_самого_ключа)
        *   [2.2.2 Управление несколькими ключами](#Управление_несколькими_ключами)
*   [3 Копирование открытого ключа на удаленный сервер](#Копирование_открытого_ключа_на_удаленный_сервер)
    *   [3.1 Простой метод](#Простой_метод)
    *   [3.2 Традиционный метод](#Традиционный_метод)
*   [4 Безопасность](#Безопасность)
    *   [4.1 Обеспечение безопасности файла authorized_keys](#Обеспечение_безопасности_файла_authorized_keys)
    *   [4.2 Отключение входа по паролю](#Отключение_входа_по_паролю)
    *   [4.3 Двухфакторная аутентификация и открытые ключи](#Двухфакторная_аутентификация_и_открытые_ключи)
*   [5 Агенты SSH](#Агенты_SSH)
    *   [5.1 ssh-agent](#ssh-agent)
        *   [5.1.1 ssh-agent в качестве программы-обертки](#ssh-agent_в_качестве_программы-обертки)
    *   [5.2 Агент GnuPG](#Агент_GnuPG)
    *   [5.3 Keychain](#Keychain)
        *   [5.3.1 Альтернативные методы запуска](#Альтернативные_методы_запуска)
    *   [5.4 envoy](#envoy)
        *   [5.4.1 envoy с паролями ключей, хранящимися в kwallet](#envoy_с_паролями_ключей,_хранящимися_в_kwallet)
    *   [5.5 x11-ssh-askpass](#x11-ssh-askpass)
        *   [5.5.1 Вызов x11-ssh-askpass вместе с ssh-add](#Вызов_x11-ssh-askpass_вместе_с_ssh-add)
        *   [5.5.2 Настройка внешнего вида](#Настройка_внешнего_вида)
        *   [5.5.3 Альтернативные диалоговые окна ввода пароля](#Альтернативные_диалоговые_окна_ввода_пароля)
    *   [5.6 pam_ssh](#pam_ssh)
        *   [5.6.1 Известные проблемы pam_ssh](#Известные_проблемы_pam_ssh)
    *   [5.7 GNOME Keyring](#GNOME_Keyring)
*   [6 Решение проблем](#Решение_проблем)
    *   [6.1 Использование kdm](#Использование_kdm)
...
Обратитесь к [данным заметкам для использования x11-ssh-askpass вместе с ssh-add](#Вызов_x11-ssh-askpass_вместе_с_ssh-add) по вопросу, связанному с быстрым добавлением вашего ключа в агент.
...
Данный метод хорошо работает при [использовании *ssh-agent* в качестве программы-обертки](#ssh-agent_в_качестве_программы-обертки). Вы запускаете X командой `ssh-agent startx`, а затем добавляете *ssh-add* в список автоматически запускаемых приложений вашего оконного менеджера.
...
Удостоверьтесь, что удаленная машина поддерживает тип ключей, который вы используете. Попробуйте использовать ключи RSA или DSA вместо [генерации пары ключей SSH](#Генерация_пары_ключей_SSH)
