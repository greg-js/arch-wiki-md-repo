...
**Обратите внимание:** Если один из двух компьютеров имеет гигабитный ethernet порт, перекрёстный кабель не обязателен, подойдёт обычный ethernet кабель.

## Настройка

В этом разделе предполагается, что сетевой интерфейс, подсоединяемый к клиентскому компьютеру (компьютерам), называется _**net0**_, а сетевой интерфейс, подключенный к интернету называется _**internet0**_.
...
**Обратите внимание:** Разумеется, это тоже будет работать с мобильным широкополосном соединением (обычно называемом ppp0 на раздающем компьютере).
