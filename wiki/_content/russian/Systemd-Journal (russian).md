**Состояние перевода:** На этой странице представлен перевод статьи [systemd/Journal](/index.php/Systemd/Journal "Systemd/Journal"). Дата последней синхронизации: 21 февраля 2019\. Вы можете [помочь](/index.php/ArchWiki_Translation_Team_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "ArchWiki Translation Team (Русский)") синхронизировать перевод, если в английской версии произошли [изменения](https://wiki.archlinux.org/index.php?title=Systemd/Journal&diff=0&oldid=566388).
...
*   Показать сообщения конкретного юнита: `# journalctl -u netcfg` 
*   Показать кольцевой буфер ядра: `# journalctl -k` 
*   Показать сообщения только с приоритетами error, critical и alert: `# journalctl -p err..alert` Также можно использовать числа: `journalctl -p 3..1`. Если указать одно ключевое слово/число, например, `journalctl -p 3`, то также будут показаны сообщения и с более высоким приоритетом.
...
Также возможно использование конфигурационных сниппетов вместо редактирования глобального файла конфигурации. В таком случае не забудьте поместить переопределения в разделе `[Journal]`:
