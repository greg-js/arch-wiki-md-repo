...
По умолчанию, Tor может обрабатывать до 8192 соединений одновременно. Можно увеличить это значение до 32768 [[3]](https://www.torproject.org/docs/faq.html.en#PackagedTor):
...
Если команда `grep aes /proc/cpuinfo` возвращает какой-то результат, то ваш CPU поддерживает AES-инструкции. Если соответствующий модуль был загружен (`lsmod | grep aes`), то параметр `HardwareAccel 1` включит встроенное аппаратное ускорение шифрования [[4]](http://www.torservers.net/wiki/setup/server#aes-ni_crypto_acceleration).
