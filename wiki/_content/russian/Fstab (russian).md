...
*   [1 Пример файла](#Пример_файла)
*   [2 Формат строки](#Формат_строки)
*   [3 Определение файловой системы](#Определение_файловой_системы)
    *   [3.1 По именам устройств](#По_именам_устройств)
    *   [3.2 По меткам](#По_меткам)
    *   [3.3 По UUID](#По_UUID)
    *   [3.4 По меткам GPT](#По_меткам_GPT)
    *   [3.5 По UUID GPT](#По_UUID_GPT)
*   [4 Советы и рекомендации](#Советы_и_рекомендации)
    *   [4.1 Автоматическое монтирование с systemd](#Автоматическое_монтирование_с_systemd)
    *   [4.2 Пробелы в значениях полей](#Пробелы_в_значениях_полей)
    *   [4.3 Внешние устройства](#Внешние_устройства)
    *   [4.4 Параметры atime](#Параметры_atime)
    *   [4.5 Запись в FAT32 с правами обычного пользователя](#Запись_в_FAT32_с_правами_обычного_пользователя)
    *   [4.6 Перемонтирование корневого раздела](#Перемонтирование_корневого_раздела)
...
```
	...
	*filesystem*
	...
	Физическое место размещения файловой системы, по которому определяется конкретный раздел или устройство хранения для монтирования. Смотрите подробнее в разделе [#Определение файловой системы](#Определение_файловой_системы).
...
**Примечание:** Если какое-нибудь из полей содержит пробелы, прочитайте в разделе [#Пробелы в значениях полей](#Пробелы_в_значениях_полей) о том, как правильно их указывать.
...
**Примечание:** Если какое-нибудь из полей содержит пробелы, прочитайте в разделе [#Пробелы в значениях полей](#Пробелы_в_значениях_полей) о том, как их правильно указать.
...
*   [Samba#Добавление сетевых ресурсов Sabma в /etc/fstab](/index.php/Samba#Добавление_сетевых_ресурсов_Sabma_в_/etc/fstab "Samba")