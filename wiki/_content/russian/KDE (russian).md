...
**Состояние перевода:** На этой странице представлен перевод статьи [KDE](/index.php/KDE "KDE"). Дата последней синхронизации: 4 декабря 2018\. Вы можете [помочь](/index.php/ArchWiki_Translation_Team_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "ArchWiki Translation Team (Русский)") синхронизировать перевод, если в английской версии произошли [изменения](https://wiki.archlinux.org/index.php?title=KDE&diff=0&oldid=558213).
...
*   [5 Советы и рекомендации](#Советы_и_рекомендации)
    ...
    *   [5.3 Отключение открытия Меню запуска приложений клавишей Super (Windows)](#Отключение_открытия_Меню_запуска_приложений_клавишей_Super_(Windows))
*   [6 Решение проблем](#Решение_проблем)
    ...
    *   [6.3 Графика](#Графика)
        ...
        *   [6.3.3 Отключение композитного режима](#Отключение_композитного_режима)
        *   [6.3.4 Мерцание окон в полноэкранном режиме при включённом композитном режиме](#Мерцание_окон_в_полноэкранном_режиме_при_включённом_композитном_режиме)
        *   [6.3.5 Разрыв изображения с NVIDIA](#Разрыв_изображения_с_NVIDIA)
        *   [6.3.6 Курсор иногда отображается неправильно](#Курсор_иногда_отображается_неправильно)
    *   [6.4 Непригодное для использования множество разрешений экрана](#Непригодное_для_использования_множество_разрешений_экрана)
    *   [6.5 Звук](#Звук)
        *   [6.5.1 Отсутствие звука после выхода из ждущего режима](#Отсутствие_звука_после_выхода_из_ждущего_режима)
        *   [6.5.2 MP3-файлы не воспроизводятся с бекендом GStreamer в Phonon](#MP3-файлы_не_воспроизводятся_с_бекендом_GStreamer_в_Phonon)
    *   [6.6 Управление электропитанием](#Управление_электропитанием)
        *   [6.6.1 Отсутствие ждущего/спящего режима](#Отсутствие_ждущего/спящего_режима)
    *   [6.7 KMail](#KMail)
        *   [6.7.1 Сброс настроек Akonadi для решения проблем с KMail](#Сброс_настроек_Akonadi_для_решения_проблем_с_KMail)
        *   [6.7.2 Пустая папка входящих сообщений в IMAP-аккаунте KMail](#Пустая_папка_входящих_сообщений_в_IMAP-аккаунте_KMail)
    *   [6.8 Сеть](#Сеть)
        *   [6.8.1 Зависания при использовании автоматического монтирования NFS-раздела](#Зависания_при_использовании_автоматического_монтирования_NFS-раздела)
    *   [6.9 Слишком частое журналирование событий QXcbConnection](#Слишком_частое_журналирование_событий_QXcbConnection)
    *   [6.10 Приложения на KF5/Qt 5 не отображают значки в i3/FVWM/awesome](#Приложения_на_KF5/Qt_5_не_отображают_значки_в_i3/FVWM/awesome)
    *   [6.11 Проблемы с сохранением учётных данных и постоянно появляющимися диалогами KWallet](#Проблемы_с_сохранением_учётных_данных_и_постоянно_появляющимися_диалогами_KWallet)
    *   [6.12 Discover не отображает никакие приложения](#Discover_не_отображает_никакие_приложения)
    *   [6.13 Высокая нагрузка kscreenlocker_greet на ЦП при использовании драйверов NVIDIA](#Высокая_нагрузка_kscreenlocker_greet_на_ЦП_при_использовании_драйверов_NVIDIA)
    *   [6.14 "OS error 22" при выполнении Akonadi на ZFS](#"OS_error_22"_при_выполнении_Akonadi_на_ZFS)
    *   [6.15 Не работает прокрутка неактивных окон в некоторых программах](#Не_работает_прокрутка_неактивных_окон_в_некоторых_программах)
...
После замены Kwin на [оконный менеджер](/index.php/%D0%9E%D0%BA%D0%BE%D0%BD%D0%BD%D1%8B%D0%B9_%D0%BC%D0%B5%D0%BD%D0%B5%D0%B4%D0%B6%D0%B5%D1%80 "Оконный менеджер") без [композитного менеджера](/index.php/Xorg_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Композит "Xorg (Русский)") (например, [Openbox (Русский)](/index.php/Openbox_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Openbox (Русский)")), все композитные эффекты рабочего стола (прозрачность и т.п.) пропадут. В таком случае, установите и запустите отдельный композитный менеджер, например, [Xcompmgr (Русский)](/index.php/Xcompmgr_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Xcompmgr (Русский)") или [Compton (Русский)](/index.php/Compton_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Compton (Русский)").
