...
**Состояние перевода:** На этой странице представлен перевод статьи [KDE](/index.php/KDE "KDE"). Дата последней синхронизации: 2 февраля 2019\. Вы можете [помочь](/index.php/ArchWiki_Translation_Team_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "ArchWiki Translation Team (Русский)") синхронизировать перевод, если в английской версии произошли [изменения](https://wiki.archlinux.org/index.php?title=KDE&diff=0&oldid=596144).
...
*   [4 Приложения](#Приложения)
    ...
    *   [4.4 PIM](#PIM)
        *   [4.4.1 Akonadi](#Akonadi)
            *   [4.4.1.1 MySQL](#MySQL)
                *   [4.4.1.1.1 Общесистемный экземпляр MySQL](#Общесистемный_экземпляр_MySQL)
            *   [4.4.1.2 PostgreSQL](#PostgreSQL)
                *   [4.4.1.2.1 Экземпляр PostgreSQL для каждого пользователя](#Экземпляр_PostgreSQL_для_каждого_пользователя)
                *   [4.4.1.2.2 Общесистемный экземпляр PostgreSQL](#Общесистемный_экземпляр_PostgreSQL)
            *   [4.4.1.3 SQLite](#SQLite)
            *   [4.4.1.4 Отключение Akonadi](#Отключение_Akonadi)
...
*   [5 Советы и рекомендации](#Советы_и_рекомендации)
    ...
    *   [5.4 Настройка ICC-профилей](#Настройка_ICC-профилей)
    *   [5.5 Отключение открытия Меню запуска приложений клавишей Super (Windows)](#Отключение_открытия_Меню_запуска_приложений_клавишей_Super_(Windows))
    *   [5.6 Отключение отображения закладок браузера в меню приложений](#Отключение_отображения_закладок_браузера_в_меню_приложений)
*   [6 Решение проблем](#Решение_проблем)
    ...
    *   [6.2 Настройки](#Настройки)
        ...
        *   [6.2.3 Не работают уведомления, мультимедийные клавиши или изменение громкости](#Не_работают_уведомления,_мультимедийные_клавиши_или_изменение_громкости)
        *   [6.2.4 KCM входа в систему не синхронизирует настройки курсора с SDDM](#KCM_входа_в_систему_не_синхронизирует_настройки_курсора_с_SDDM)
    *   [6.3 Графика](#Графика)
    ...
    *   [6.15 TeamViewer работает медленно](#TeamViewer_работает_медленно)
...
Также темы KDE Plasma устанавливаются в приложении *Параметры системы > Оформление рабочей среды > Загрузить оформления...*.
...
Рекомендуемая тема для приложений на GTK – [breeze-gtk](https://www.archlinux.org/packages/?name=breeze-gtk). Данные темы имитируют дизайн Breeze, использующийся в KDE Plasma. Установите [kde-gtk-config](https://www.archlinux.org/packages/?name=kde-gtk-config) (часть группы пакетов [plasma](https://www.archlinux.org/groups/x86_64/plasma/)) и выберите `Breeze` или `Breeze-Dark` в качестве GTK2-/GTK3-темы в приложении *Параметры системы > Оформление приложений > Стиль программ GNOME/GTK+*.
...
Некоторые GTK2-приложения (например, [vuescan-bin](https://aur.archlinux.org/packages/vuescan-bin/)) всё равно плохо смотрятся из-за невидимых флажков с темой Breeze или Adwaita в KDE Plasma. Для решения этой проблемы, установите и выберите, к примеру, тему Numix-Frost-Light из пакета [numix-frost-themes](https://aur.archlinux.org/packages/numix-frost-themes/) в приложении *Параметры системы* > *Оформление приложений* > *Стиль программ GNOME/GTK+* > *Стиль программ GTK2*. Numix-Frost-Light также выглядит похоже на Breeze.

##### Аватар

Plasma и [SDDM](/index.php/SDDM_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "SDDM (Русский)") используют PNG-файл `~/.face.icon` в качестве аватара пользователя. Чтобы настроить его с помощью графического интерфейса, перейдите в *Параметры системы > Учётная запись > Управление пользователями* (может потребоваться [установить](/index.php/%D0%A3%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%B8%D1%82%D1%8C "Установить") пакет [user-manager](https://www.archlinux.org/packages/?name=user-manager)). Иконка по умолчанию находится в `/usr/share/sddm/faces/`.
...
Самый лёгкий способ установить скриптовые плазмоиды – вызвать контекстное меню панели или рабочего стола и выбрать *Добавить виджеты > Пополнить список виджетов... > Получить новые виджеты Plasma...*. Здесь вы сможете устанавливать, удалять и обновлять любые виджеты, размещенные на [store.kde.org](https://store.kde.org/).
...
**Совет:** В связи с тем, что некоторые темы значков не наследуют их от стандартной темы, вам может не хватать определённых значков. Чтобы наследовать их от Breeze, добавьте `breeze` в массив `Inherits=` в файле `/usr/share/icon/*theme-name*/index.theme`. Например: `Inherits=breeze,hicolor`. Данный процесс придётся повторять после каждого обновления темы значков. Как вариант, воспользуйтесь [хуками pacman](/index.php/Pacman_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Хуки "Pacman (Русский)") для автоматизации этих действий.
...
Для генерирования эскизов медиафайлов, их предпросмотра в Dolphin или на рабочем столе, [установите](/index.php/%D0%A3%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%B8%D1%82%D0%B5 "Установите") пакеты [kdegraphics-thumbnailers](https://www.archlinux.org/packages/?name=kdegraphics-thumbnailers) и [ffmpegthumbs](https://www.archlinux.org/packages/?name=ffmpegthumbs).
...
Установите [Samba](/index.php/Samba "Samba") (доступна с пакетом [samba](https://www.archlinux.org/packages/?name=samba)), если необходимо получить доступ к службам Windows.

Для обмена файлами Dolphin требует пакет [kdenetwork-filesharing](https://www.archlinux.org/packages/?name=kdenetwork-filesharing) и настроить общий каталог (userhares), который отсутствует в стандартном файле настроек (`smb.conf`). Инструкции по его добавлению приведены в разделе [Samba (Русский)#Создание ресурсов общего доступа от имени обычного пользователя](/index.php/Samba_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Создание_ресурсов_общего_доступа_от_имени_обычного_пользователя "Samba (Русский)"). После выполнения инструкций, обмен файлами в Dolphin заработает после перезапуска Samba.
...
На выбор предоставляются бекенды, основанные на [GStreamer](/index.php/GStreamer "GStreamer") и [VLC](/index.php/VLC "VLC"), каждый из которых имеет версию для приложений на Qt 4 и Qt 5 ([phonon-qt4-gstreamer](https://aur.archlinux.org/packages/phonon-qt4-gstreamer/), [phonon-qt5-gstreamer](https://www.archlinux.org/packages/?name=phonon-qt5-gstreamer) – [phonon-qt4-vlc](https://aur.archlinux.org/packages/phonon-qt4-vlc/), [phonon-qt5-vlc](https://www.archlinux.org/packages/?name=phonon-qt5-vlc)).
...
KDE поставляет свой собственный набор приложений для [управления персональной информацией](https://en.wikipedia.org/wiki/Personal_information_management "wikipedia:Personal information management") (PIM), что включает в себя работу с электронными письмами, контактами, календарями и так далее. Чтобы установить все PIM-пакеты, воспользуйтесь группой пакетов [kdepim](https://www.archlinux.org/groups/x86_64/kdepim/) или мета-пакетом [kdepim-meta](https://www.archlinux.org/packages/?name=kdepim-meta).
...
По умолчанию Akonadi использует `/usr/bin/mysqld` (по умолчанию это [MariaDB](/index.php/MariaDB "MariaDB"), см. статью [MySQL](/index.php/MySQL "MySQL") для получения информации об альтернативных провайдерах) для запуска экземпляра MySQL с базой данных в `~/.local/share/akonadi/db_data/`.

###### Общесистемный экземпляр MySQL

Akonadi поддерживает использование общесистемного экземпляра [MySQL (Русский)](/index.php/MySQL_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "MySQL (Русский)") для своей базы данных.[[6]](https://techbase.kde.org/KDE_PIM/Akonadi#Can_Akonadi_use_a_normal_MySQL_server_running_on_my_system.3F)
...
Akonadi поддерживает использование как существующего общесистемного экземпляра [PostgreSQL (Русский)](/index.php/PostgreSQL_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "PostgreSQL (Русский)") (например, `postgresql.service`), так и запуск PostgreSQL с правами пользователя и базой данных в `~/.local/share/akonadi/db_data/`.

###### Экземпляр PostgreSQL для каждого пользователя

[Установите](/index.php/%D0%A3%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%B8%D1%82%D0%B5 "Установите") пакеты [postgresql](https://www.archlinux.org/packages/?name=postgresql) и [postgresql-old-upgrade](https://www.archlinux.org/packages/?name=postgresql-old-upgrade).
...
*   База данных будет храниться в директории `~/.local/share/akonadi/db_data/`.

Запустите Akonadi командой `akonadictl start` и проверьте его статус: `akonadictl status`.

**Примечание:**

*   Начиная с [akonadi](https://www.archlinux.org/packages/?name=akonadi) 19.08.0-1 кластер базы данных PostgreSQL `~/.local/share/akonadi/db_data/` автоматически обновляется при обнаружении нового крупного обновления PostgreSQL.
*   Более старые версии [akonadi](https://www.archlinux.org/packages/?name=akonadi) требуют обновления базы данных вручную после крупных обновлений PostgreSQL. Следуйте [инструкциям обновления на KDE UserBase Wiki](https://userbase.kde.org/Akonadi/Postgres_update), чтобы обновить базу данных PostgreSQL в `~/.local/share/akonadi/db_data/`, используемую Akonadi. Убедитесь, что заданы правильные пути к бинарным файлам PostgreSQL, используемые [postgresql](https://www.archlinux.org/packages/?name=postgresql) и [postgresql-old-upgrade](https://www.archlinux.org/packages/?name=postgresql-old-upgrade), см. [PostgreSQL#Upgrading PostgreSQL](/index.php/PostgreSQL#Upgrading_PostgreSQL "PostgreSQL").

###### Общесистемный экземпляр PostgreSQL

Данный метод требует уже настроенный и запущенный экземпляр [PostgreSQL (Русский)](/index.php/PostgreSQL_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "PostgreSQL (Русский)").
...
*   Секция `[QSQLITE3]` и её значения будут созданы во время запуска Akonadi.
...
KDE Connect можно использовать и с другими средами рабочего стола. Для тех, которые используют AppIndicators (например, Unity) [установите](/index.php/%D0%A3%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%B8%D1%82%D0%B5 "Установите") также пакет [indicator-kdeconnect](https://aur.archlinux.org/packages/indicator-kdeconnect/). Пользователи GNOME могут улучшить интеграцию с помощью установки [gnome-shell-extension-gsconnect](https://aur.archlinux.org/packages/gnome-shell-extension-gsconnect/) вместо [kdeconnect](https://www.archlinux.org/packages/?name=kdeconnect). Чтобы запустить демон KDE Connect вручную, выполните `/usr/lib/kdeconnectd`.
...
Plasma больше не позволяет выбирать альтернативный оконный менеджер в *Параметрах системы*. [[7]](https://github.com/KDE/plasma-desktop/commit/2f83a4434a888cd17b03af1f9925cbb054256ade) Теперь для этого нужно задать значение [переменной окружения](/index.php/Environment_variables_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Environment variables (Русский)") `KDEWM` перед запуском Plasma. [[8]](https://wiki.haskell.org/Xmonad/Using_xmonad_in_KDE) Сделать это можно создав скрипт `set_window_manager.sh` в `~/.config/plasma-workspace/env` и экспортируя там переменную `KDEWM`. Например, для использования оконного менеджера [i3 (Русский)](/index.php/I3_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "I3 (Русский)") скрипт будет выглядеть следующим образом:
...
**Примечание:** Необходимо вручную задать диалогам открытие в плавающем режиме для их корректного отображения в оконном менеджере i3\. См. раздел [I3 (Русский)#Правильное обращение с плавающими диалогами](/index.php/I3_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Правильное_обращение_с_плавающими_диалогами "I3 (Русский)") для получения более подробной информации.
...
[KWin-lowlatency](https://github.com/tildearrow/kwin-lowlatency) — попытка уменьшить задержки и прерывания в композиторе KWin. Для установки доступен пакет [kwin-lowlatency](https://aur.archlinux.org/packages/kwin-lowlatency/).
...
Дополнительно, удалите содержимое директории `~/.cache`. Обратите внимание, что это также очистит и кэш других программ:
...
KCM входа в систему считывает настройки курсоров из `~/.config/kcminputrc`, без этого файла настройки не будут синхронизироваться. Наиболее простой способ сгенерировать данный файл — изменить настройки курсора в приложении *Параметры системы > Курсоры мыши*, после чего вернуть предпочитаемую тему курсоров.
...
См. [данный](https://docs.kde.org/trunk5/en/pim/kmail2/clean-start-after-a-failed-migration.html) документ для получения подробной информации
...
Для некоторых аккаунтов KMail использующих протокол IMAP, папка входящих сообщений может отображаться в виде контейнера верхнего уровня (таким образом, делая прочтение сообщений невозможным) со всеми остальными директориями в нём.[[9]](https://bugs.kde.org/show_bug.cgi?id=284172). Для решений этой проблемы достаточно отключить подписки на стороне сервера (server-side subscriptions) в настройках аккаунта в KMail.
...
TeamViewer может медленно работать при включённых плавных анимациях (таких как сворачивание окон). Временным решением является [#Отключение композитного режима](#Отключение_композитного_режима).

## Смотрите также

*   [Домашняя страница KDE](https://www.kde.org/) (англ.)
*   [Домашняя страница KDE Россия](https://kde.ru/)
*   [Новости KDE](https://dot.kde.org/) (англ.)
*   [Блоги KDE](https://planet.kde.org/) (англ.)
*   [Форумы KDE](https://forum.kde.org/) (англ.)
*   [Вики KDE](https://wiki.kde.org/) (англ.)
*   [Баг-трекер KDE](https://bugs.kde.org/) (англ.)
*   [Блог Мартина Греслина](https://blog.martin-graesslin.com/blog/kategorien/kde/) (англ.)