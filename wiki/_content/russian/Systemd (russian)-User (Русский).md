...
**Состояние перевода:** На этой странице представлен перевод статьи [systemd/User](/index.php/Systemd/User "Systemd/User"). Дата последней синхронизации: 19 июля 2019‎. Вы можете [помочь](/index.php/ArchWiki_Translation_Team_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "ArchWiki Translation Team (Русский)") синхронизировать перевод, если в английской версии произошли [изменения](https://wiki.archlinux.org/index.php?title=Systemd/User&diff=0&oldid=577514).
...
**Примечание:** В systemd `v242` присутствует баг, который не позволяет включить долговременную службу с помощью `loginctl`, см. [issue на Github](https://github.com/systemd/systemd/issues/12401). В качестве временного решения воспользуйтесь командой `touch`. `# touch /var/lib/systemd/linger/*имя_пользователя*` 
