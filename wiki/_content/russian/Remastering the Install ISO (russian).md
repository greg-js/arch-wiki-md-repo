...
Установщик ISO содержит две отдельные системы archlinux. Одна для 32-бит (i686) и другая для 64-бит (x86_64). Корень файловых систем хранится в `arch/i686/airootfs.sfs` и `arch/x86_64/airootfs.sfs`. Ядра и initramfs находятся в `arch/boot/i686` и `arch/boot/x86_64`. Это означает, что для полного обновления ISO необходимо выполнить свои изменения как для 32-разрядной, так и для 64-разрядной системы.

При загрузке initramfs будет искать устройство, с которого он был загружен с помощью его метки, например `ARCH_201410`, и смонтирует корневую файловую систему для правильной архитектуры.
...
Выполните те же действия, что и для системы x86_64, но со следующими отличиями:

1.  В каждой команде используйте `i686` вместо `x86_64`
2.  Когда выполняется chroot вместо этого используйте эту команду:
...
**Примечание:** Если у вас нет скрипта arch-chroot, поставьте `setarch i686` перед строкой, где вы делаете `chroot`
...
[larch](/index.php/Archiso_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Archiso (Русский)") призвана обеспечить более desktop-based подход и не требует хост-системы Arch Linux.

**Примечание:**

*   Начиная с версии 2.6.39, ванильное ядро больше не поддерживает aufs, что делает создание пользовательского ядра необходимостью.
*   [linux-aufs_friendly](https://aur.archlinux.org/packages/linux-aufs_friendly/) является версией arch ядра AUX, и larch поддерживает сборку i686, вам не нужно компилировать что-либо, если вы не хотите 64 бит.

## poison-livecd-creator

Это очень простой создатель live CD. Он использует только Makefile для создания live CD образов и использует pacman для установки базовых и дополнительных пакетов на live CD. Вы можете выбрать свои пакеты и собрать их на live CD. Кроме того, он использует [GRUB](/index.php/GRUB_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "GRUB (Русский)") для загрузки live CD, чтобы добавить большую гибкость. Это означает, что гораздо проще сделать live USB-накопитель без его форматирования Для этого вам просто нужно установить GRUB на свой USB-накопитель и скопировать файлы ISO в корневой каталог накопителя. Он полагается на хост-систему Arch Linux и [pacman](/index.php/Pacman_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Pacman (Русский)").

[poison-livecd-creator](https://aur.archlinux.org/packages/poison-livecd-creator/).

## Linux-pf

Он поддерживает, помимо прочего, aufs, что делает его доступным live CDs.

[linux-pf](https://aur.archlinux.org/packages/linux-pf/).

**Примечание:** Начиная с версии 2.6.39, ванильное ядро больше не поддерживает aufs, что делает создание пользовательского ядра необходимостью.
