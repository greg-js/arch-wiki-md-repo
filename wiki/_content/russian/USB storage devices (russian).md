...
Если `mount` не распознаёт формат устройства (файловой системы), то можно попробовать с ключом `-t <тип файловой системы>`, а также глянуть в [mount(8)](http://jlk.fjfi.cvut.cz/arch/manpages/man/mount.8) для просвещения.
