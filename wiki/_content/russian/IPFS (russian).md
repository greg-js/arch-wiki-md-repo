...
From the [IPFS README.md on GitHub](https://github.com/ipfs/ipfs):
	...
	[IPFS](/index.php/IPFS "IPFS") is a distributed file system that seeks to connect all computing devices with the same system of files. In some ways, this is similar to the original aims of the Web, but IPFS is actually more similar to a single bittorrent swarm exchanging git objects.
...
For convenience you can automate the startup of [IPFS](/index.php/IPFS "IPFS") daemon using a [Systemd/User](/index.php/Systemd/User "Systemd/User") service. This ensures that the daemon starts when you log in, and that it is restarted if it crashes.
