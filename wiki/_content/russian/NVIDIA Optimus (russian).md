...
*   [8 Используя PRIME Render Offload](#Используя_PRIME_Render_Offload)
    *   [8.1 Установка](#Установка)
    *   [8.2 Настройка](#Настройка)
    *   [8.3 Использование](#Использование)
    *   [8.4 Решение проблем](#Решение_проблем)
        *   [8.4.1 Не запускается модуль Nvidia](#Не_запускается_модуль_Nvidia)
...
На текущий момент [xorg-server](https://www.archlinux.org/packages/?name=xorg-server) 1.20.5-2 не содержит нужных патчей для работы PRIME Render Offload, но можно воспользоваться [xorg-server-git](https://aur.archlinux.org/packages/xorg-server-git/).
