**Состояние перевода:** На этой странице представлен перевод статьи [Video game platform emulators](/index.php/Video_game_platform_emulators "Video game platform emulators"). Дата последней синхронизации: 31 мая 201‎9\. Вы можете [помочь](/index.php/ArchWiki_Translation_Team_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "ArchWiki Translation Team (Русский)") синхронизировать перевод, если в английской версии произошли [изменения](https://wiki.archlinux.org/index.php?title=Video_game_platform_emulators&diff=0&oldid=574255).

Эмулятор – программа, повторяющая функции другой платформы или системы для запуска приложений и игр в окружении, для которого они изначально не создавались.

**Примечание:** Для получения, возможно, более актуального списка эмуляторов, попробуйте проверить [ключевое слово 'emulator'](https://aur.archlinux.org/packages/?SeB=k&K=emulator) в [AUR](/index.php/AUR_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "AUR (Русский)").

**Важно:** Владение эмулятором высокого уровня полностью легально, но распространение образов ROM (ПЗУ), защищённых авторским правом, и несанкционированная эмуляция (без письменного разрешения владельца авторских прав, разрешающего это пользователю) являются **незаконными**. Следовательно, Arch Linux не распространяет подобный контент, защищённый авторским правом, включая игровые образы ROM и скопированные (ripped) BIOS консолей. Вы несёте полную ответственность за какое-либо использование эмуляторов, полученных с [официальных репозиториев](/index.php/Official_repositories_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Official repositories (Русский)") или [Arch User Repository (Русский)](/index.php/Arch_User_Repository_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Arch User Repository (Русский)") и последствия ваших действий. Arch Linux не несёт какой-либо ответственности.

## Консоли

*   **Citra** — Эмулятор Nintendo 3DS.

	[https://citra-emu.org/](https://citra-emu.org/) || [citra-git](https://aur.archlinux.org/packages/citra-git/)

*   **DeSmuME** — Эмулятор Nintendo DS.

	[https://desmume.org/](https://desmume.org/) || [desmume](https://www.archlinux.org/packages/?name=desmume)

*   **[Dolphin](/index.php/Dolphin_emulator "Dolphin emulator")** — Эмулятор GameCube и Wii с широкими возможностями.

	[https://dolphin-emu.org/](https://dolphin-emu.org/) || [dolphin-emu](https://www.archlinux.org/packages/?name=dolphin-emu)

*   **epsxe** — Эмулятор PlayStation для ПК с архитектурой x86.

	[http://www.epsxe.com/](http://www.epsxe.com/) || [epsxe](https://aur.archlinux.org/packages/epsxe/)

*   **FCEUX** — NTSC и PAL 8-битный эмулятор Nintendo/Famicom, являющийся продолжением оригинального эмулятора FCE Ultra. Он точный, совместимый и активно поддерживаемый.

	[http://fceux.com/](http://fceux.com/) || [fceux](https://www.archlinux.org/packages/?name=fceux)

*   **Gambatte** — Точный эмулятор Game Boy Color

	[https://github.com/sinamas/gambatte](https://github.com/sinamas/gambatte) || Графический [Qt](/index.php/Qt_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Qt (Русский)")-фронтенд ([gambatte-qt](https://aur.archlinux.org/packages/gambatte-qt/)), консольный SDL-фронтенд ([gambatte-sdl](https://aur.archlinux.org/packages/gambatte-sdl/)).

*   **Gens2** — Эмулятор Sega Genesis, Sega CD и 32X, написанный на ассемблере и не очень активно поддерживаемый в настоящее время.

*   активируйте OpenGL, задайте разрешение видео 1024x600 для растянутого полноэкранного режима или 800x600 для нерастянутого;
*   используйте рендерер "Normal", я не смог найти ощутимых преимуществ перед другими вариантами.

	[http://www.gens.me/](http://www.gens.me/) || [gens](https://www.archlinux.org/packages/?name=gens)

*   **Gens-GS** — Gens2, переписанный на C++ и комбинирующий возможности различных форков Gens.

	[https://segaretro.org/Gens/GS](https://segaretro.org/Gens/GS) || [gens-gs](https://www.archlinux.org/packages/?name=gens-gs)

*   **gngeo** — Консольный эмулятор NeoGeo.

	[https://github.com/pepone42/gngeo](https://github.com/pepone42/gngeo) || [gngeo](https://aur.archlinux.org/packages/gngeo/)

*   **higan** — Мультисистемный эмулятор, фокусирующийся на точности и поддерживаемый SNES, NES, GB, GBC, GBA.

	[https://byuu.org/emulation/higan/](https://byuu.org/emulation/higan/) || [higan](https://www.archlinux.org/packages/?name=higan)

*   **mednafen** — Консольный мультисистемный эмулятор.

	[https://mednafen.github.io/](https://mednafen.github.io/) || [mednafen](https://www.archlinux.org/packages/?name=mednafen)

*   **Mupen64Plus** — Высокосовместимый эмулятор Nintendo 64 с системой плагинов.

	[http://www.mupen64plus.org/](http://www.mupen64plus.org/) || [mupen64plus](https://www.archlinux.org/packages/?name=mupen64plus) или графический фронтенд, например, [m64py](https://aur.archlinux.org/packages/m64py/) или [cutemupen](https://aur.archlinux.org/packages/cutemupen/)

*   **[Nestopia](/index.php/Nestopia "Nestopia")** — Очень точный эмулятор NES.

	[http://0ldsk00l.ca/nestopia/](http://0ldsk00l.ca/nestopia/) || [nestopia](https://aur.archlinux.org/packages/nestopia/)

*   **pSX** — Эмулятор PlayStation с относительно высокой совместимостью и без системы плагинов.

	[http://psxemulator.gazaxian.com/](http://psxemulator.gazaxian.com/) || [psx](https://aur.archlinux.org/packages/psx/)

*   **[PCSXR](/index.php/PCSX-Reloaded "PCSX-Reloaded")** — Эмулятор PlayStation; Форк Debian заброшенного оригинального PCSX.

	[pcsxr](https://aur.archlinux.org/packages/pcsxr/) || <small>not packaged? [search in AUR](https://aur.archlinux.org/packages/)</small>

*   **PCSX2** — Эмулятор PlayStation 2\. До сих пор поддерживается и разрабатывается. Требуются файлы BIOS.

	[https://pcsx2.net/](https://pcsx2.net/) || [pcsx2](https://www.archlinux.org/packages/?name=pcsx2)

*   **PPSSPP** — Эмулятор PlayStation Portable.

	[https://ppsspp.org/](https://ppsspp.org/) || [ppsspp](https://www.archlinux.org/packages/?name=ppsspp)

*   **RPCS3** — Экспериментальный свободный эмулятор и отладчик Sony PlayStation 3.

	[https://rpcs3.net/](https://rpcs3.net/) || [rpcs3](https://aur.archlinux.org/packages/rpcs3/)

*   **Snes9x** — Портативный, бесплатный (freeware) эмулятор Super Nintendo Entertainment System (SNES).

	[http://www.snes9x.com/](http://www.snes9x.com/) || [snes9x-gtk](https://www.archlinux.org/packages/?name=snes9x-gtk)

*   **[Visual Boy Advance](/index.php/Visual_Boy_Advance "Visual Boy Advance")** — Эмулятор Game Boy с поддержкой Game Boy Advance, Game Boy Color и Super Game Boy.

	[http://vba-m.com/](http://vba-m.com/) || [vbam-wx](https://www.archlinux.org/packages/?name=vbam-wx)

*   **XQEMU** — Оригинальный эмулятор Xbox.

	[https://xqemu.com/](https://xqemu.com/) || [xqemu-git](https://aur.archlinux.org/packages/xqemu-git/)

*   **Yuzu** — Экспериментальный свободный эмулятор и отладчик Nintendo Switch.

	[https://github.com/yuzu-emu/yuzu/](https://github.com/yuzu-emu/yuzu/) || [yuzu-git](https://aur.archlinux.org/packages/yuzu-git/)

*   **ZSNES** — Высокосовместимый эмулятор Super Nintendo.

	[http://www.zsnes.com/](http://www.zsnes.com/) || [zsnes](https://www.archlinux.org/packages/?name=zsnes)

## Другое

*   **DOSBox** — Свободный эмулятор DOS, в основном фокусирующийся на запуске DOS-игр.

	[https://www.dosbox.com/](https://www.dosbox.com/) || [dosbox](https://www.archlinux.org/packages/?name=dosbox)

*   **DOSEmu** — Свободный эмулятор DOS.

	[http://www.dosemu.org/](http://www.dosemu.org/) || [dosemu](https://www.archlinux.org/packages/?name=dosemu)

*   **MAME** — Эмулятор различных аркадных автоматов.

	[https://www.mamedev.org/](https://www.mamedev.org/) || [mame](https://www.archlinux.org/packages/?name=mame)

*   **ResidualVM** — Кроссплатформенный интерпретатор 3D-игр, позволяющий играть в 3D-приключения на основе Lua от LucasArts.

	[http://residualvm.org/](http://residualvm.org/) || [residualvm](https://aur.archlinux.org/packages/residualvm/)

*   **[RetroArch](/index.php/RetroArch "RetroArch")** — Фронтенд для libretro (библиотеки для эмуляции, использующей модифицированные версии существующих эмуляторов в качестве плагинов).

	[https://www.libretro.com/](https://www.libretro.com/) || [retroarch](https://www.archlinux.org/packages/?name=retroarch)

*   **ScummVM** — Виртуальная машина для "олд скульных" приключений.

	[https://www.scummvm.org/](https://www.scummvm.org/) || [scummvm](https://www.archlinux.org/packages/?name=scummvm)

*   **X Neko Project II** — Эмулятор PC-9801.

	[http://www.asahi-net.or.jp/~aw9k-nnk/np2/](http://www.asahi-net.or.jp/~aw9k-nnk/np2/) || [xnp2](https://aur.archlinux.org/packages/xnp2/)

## Смотрите также

*   [Общий вики-ресурс по эмуляции](http://emulation.gametechwiki.com/index.php/Main_Page)
*   [Список эмуляторов игровых приставок](https://en.wikipedia.org/wiki/ru:%D0%A1%D0%BF%D0%B8%D1%81%D0%BE%D0%BA_%D1%8D%D0%BC%D1%83%D0%BB%D1%8F%D1%82%D0%BE%D1%80%D0%BE%D0%B2_%D0%B8%D0%B3%D1%80%D0%BE%D0%B2%D1%8B%D1%85_%D0%BF%D1%80%D0%B8%D1%81%D1%82%D0%B0%D0%B2%D0%BE%D0%BA "wikipedia:ru:Список эмуляторов игровых приставок")