...
**Совет:** Для удобства все переменные можно поместить в ваш `~/.bash_profile` (или его эквивалент)
...
**Примечание:** После каждого обновления версии языка Go данную библиотек также следует пересобрать, предварительно удалив старую:
```
# rm -r $(go env GOROOT)/pkg/$(go env GOOS)_$(go env GOARCH)_dynlink
...
*   прописать путь до библиотеки в файле `/etc/ld.so.conf`
