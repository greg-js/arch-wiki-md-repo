...
*   [1 Пошаговая установка на хост-компьютер под управлением Arch Linux](#Пошаговая_установка_на_хост-компьютер_под_управлением_Arch_Linux)
    *   [1.1 Установка базовых пакетов](#Установка_базовых_пакетов)
    *   [1.2 Установка модулей ядра VirtualBox](#Установка_модулей_ядра_VirtualBox)
        *   [1.2.1 Использование различных версий ядра Linux на хост-машине](#Использование_различных_версий_ядра_Linux_на_хост-машине)
        *   [1.2.2 Обновление ядра Linux на хост-машине](#Обновление_ядра_Linux_на_хост-машине)
        *   [1.2.3 Стороннее ядро на хост-машине](#Стороннее_ядро_на_хост-машине)
    *   [1.3 Загрузка модулей ядра VirtualBox](#Загрузка_модулей_ядра_VirtualBox)
    *   [1.4 Дополнительные модули VirtualBox](#Дополнительные_модули_VirtualBox)
    *   [1.5 Добавление пользователей в группу vboxusers](#Добавление_пользователей_в_группу_vboxusers)
    *   [1.6 Образ диска с гостевыми дополнениями](#Образ_диска_с_гостевыми_дополнениями)
    *   [1.7 Пакет дополнений](#Пакет_дополнений)
    *   [1.8 Правильное использование в фронт-энде](#Правильное_использование_в_фронт-энде)
*   [2 Пошаговая установка Arch Linux как гостевой ОС](#Пошаговая_установка_Arch_Linux_как_гостевой_ОС)
    *   [2.1 Установка Arch Linux в виртуальную машину](#Установка_Arch_Linux_в_виртуальную_машину)
        *   [2.1.1 Установка в режиме EFI](#Установка_в_режиме_EFI)
    *   [2.2 Установка гостевых дополнений](#Установка_гостевых_дополнений)
    *   [2.3 Установка гостевых модулей ядра VirtualBox](#Установка_гостевых_модулей_ядра_VirtualBox)
        *   [2.3.1 Запуск гостевой ОС с официальным ядром](#Запуск_гостевой_ОС_с_официальным_ядром)
        *   [2.3.2 Запуск гостевой ОС со сторонним ядром](#Запуск_гостевой_ОС_со_сторонним_ядром)
    *   [2.4 Загрузка модулей ядра VirtualBox](#Загрузка_модулей_ядра_VirtualBox_2)
    *   [2.5 Запуск гостевых сервисов VirtualBox](#Запуск_гостевых_сервисов_VirtualBox)
    *   [2.6 Расшаривание директорий](#Расшаривание_директорий)
        *   [2.6.1 Ручное монтирование](#Ручное_монтирование)
        *   [2.6.2 Автомонтирование](#Автомонтирование)
        *   [2.6.3 Монтирование при загрузке](#Монтирование_при_загрузке)
*   [3 Импорт/экспорт виртуальных машин VirtualBox в/из других гипервизоров](#Импорт/экспорт_виртуальных_машин_VirtualBox_в/из_других_гипервизоров)
    *   [3.1 Удаление дополнений гостевой ОС](#Удаление_дополнений_гостевой_ОС)
    *   [3.2 Использование правильного формата виртуального диска](#Использование_правильного_формата_виртуального_диска)
        *   [3.2.1 Автоматические инструменты](#Автоматические_инструменты)
        *   [3.2.2 Ручное преобразование](#Ручное_преобразование)
    *   [3.3 Создание конфигурации VM для гипервизора](#Создание_конфигурации_VM_для_гипервизора)
*   [4 Управление виртуальными дисками](#Управление_виртуальными_дисками)
    *   [4.1 Форматы, поддерживаемые VirtualBox](#Форматы,_поддерживаемые_VirtualBox)
    *   [4.2 Преобразование виртуальных дисков разных форматов](#Преобразование_виртуальных_дисков_разных_форматов)
        *   [4.2.1 VMDK в VDI и VDI в VMDK](#VMDK_в_VDI_и_VDI_в_VMDK)
        *   [4.2.2 VHD в VDI и VDI в VDH](#VHD_в_VDI_и_VDI_в_VDH)
        *   [4.2.3 QCOW2 в VDI и VDI в QCOW2](#QCOW2_в_VDI_и_VDI_в_QCOW2)
    *   [4.3 Монтирование виртуальных дисков](#Монтирование_виртуальных_дисков)
        *   [4.3.1 VDI](#VDI)
    *   [4.4 Сжатие виртуальных дисков](#Сжатие_виртуальных_дисков)
    *   [4.5 Увеличение размера виртуальных дисков](#Увеличение_размера_виртуальных_дисков)
    *   [4.6 Замена виртуального диска из файла .vbox вручную](#Замена_виртуального_диска_из_файла_.vbox_вручную)
    *   [4.7 Клонирование виртуального диска и назначение ему нового UUID](#Клонирование_виртуального_диска_и_назначение_ему_нового_UUID)
*   [5 Расширенная настройка](#Расширенная_настройка)
    *   [5.1 Управление запуском виртуальной машины](#Управление_запуском_виртуальной_машины)
        *   [5.1.1 Запуск виртуальный машин как сервиса systemd](#Запуск_виртуальный_машин_как_сервиса_systemd)
        *   [5.1.2 Запуск виртуальной машины по горячей клавише](#Запуск_виртуальной_машины_по_горячей_клавише)
    *   [5.2 Использование конкретных устройств в виртуальной машине](#Использование_конкретных_устройств_в_виртуальной_машине)
        *   [5.2.1 Использование USB веб-камеры/микрофона](#Использование_USB_веб-камеры/микрофона)
        *   [5.2.2 Обнаружение веб-камер и прочих USB устройств](#Обнаружение_веб-камер_и_прочих_USB_устройств)
    *   [5.3 Доступ к гостевому серверу](#Доступ_к_гостевому_серверу)
    *   [5.4 D3D ускорение в гостевой Windows](#D3D_ускорение_в_гостевой_Windows)
    *   [5.5 VirtualBox c USB-ключом](#VirtualBox_c_USB-ключом)
    *   [5.6 Запуск установленного Arch Linux внутри VirtualBox](#Запуск_установленного_Arch_Linux_внутри_VirtualBox)
        *   [5.6.1 Убедитесь, что система наименования разделов не изменяется](#Убедитесь,_что_система_наименования_разделов_не_изменяется)
        *   [5.6.2 Убедитесь в корректности образа mkinitcpio](#Убедитесь_в_корректности_образа_mkinitcpio)
        *   [5.6.3 Создание конфигурации виртуальной машины для загрузки с физического диска](#Создание_конфигурации_виртуальной_машины_для_загрузки_с_физического_диска)
            *   [5.6.3.1 Создайте потоковый(raw) образ .vmdk](#Создайте_потоковый(raw)_образ_.vmdk)
            *   [5.6.3.2 Создание файла конфигурации виртуальной машины](#Создание_файла_конфигурации_виртуальной_машины)
        *   [5.6.4 Установка дополнений гостевой ОС](#Установка_дополнений_гостевой_ОС)
    *   [5.7 Физическая установка системы Arch Linux из VirtualBox](#Физическая_установка_системы_Arch_Linux_из_VirtualBox)
    *   [5.8 Перемещение физически установленного Windows в виртуальную машину](#Перемещение_физически_установленного_Windows_в_виртуальную_машину)
        *   [5.8.1 Задачи в Windows](#Задачи_в_Windows)
        *   [5.8.2 Задачи в GNU/Linux](#Задачи_в_GNU/Linux)
        *   [5.8.3 Исправление MBR и загрузчика Microsoft](#Исправление_MBR_и_загрузчика_Microsoft)
        *   [5.8.4 Известные ограничения](#Известные_ограничения)
*   [6 Возможные проблемы](#Возможные_проблемы)
    *   [6.1 VERR_ACCESS_DENIED](#VERR_ACCESS_DENIED)
    *   [6.2 Клавиатура и мышка заблокированы виртуальной машиной](#Клавиатура_и_мышка_заблокированы_виртуальной_машиной)
    *   [6.3 Не могу отправить комбинацию CTRL+ALT+Fn в виртуальную машину](#Не_могу_отправить_комбинацию_CTRL+ALT+Fn_в_виртуальную_машину)
    *   [6.4 Исправление проблем в ISO-образах](#Исправление_проблем_в_ISO-образах)
    *   [6.5 VirtualBox GUI не видит мою тему GTK 2x/3x](#VirtualBox_GUI_не_видит_мою_тему_GTK_2x/3x)
    *   [6.6 OpenBSD не работает при недоступных инструкциях виртуализации](#OpenBSD_не_работает_при_недоступных_инструкциях_виртуализации)
    *   [6.7 VBOX_E_INVALID_OBJECT_STATE (0x80BB0007)](#VBOX_E_INVALID_OBJECT_STATE_(0x80BB0007))
    *   [6.8 Подсистема USB не работает в хост-машине или гостевой ОС](#Подсистема_USB_не_работает_в_хост-машине_или_гостевой_ОС)
    *   [6.9 Ошибка создания сетевого интерфейса "host-only"](#Ошибка_создания_сетевого_интерфейса_"host-only")
    *   [6.10 WinXP: глубина цвета не может превышать 16 цветов](#WinXP:_глубина_цвета_не_может_превышать_16_цветов)
    *   [6.11 Использование последовательных портов в гостевой ОС](#Использование_последовательных_портов_в_гостевой_ОС)
    *   [6.12 Windows 8.x ошибка 0x000000C4](#Windows_8.x_ошибка_0x000000C4)
    *   [6.13 Windows 8 VM вылетает при загрузке с ошибкой "ERR_DISK_FULL"](#Windows_8_VM_вылетает_при_загрузке_с_ошибкой_"ERR_DISK_FULL")
    *   [6.14 Гостевая ОС Linux выдает искажённый или запаздывающий звук](#Гостевая_ОС_Linux_выдает_искажённый_или_запаздывающий_звук)
    *   [6.15 Гостевая ОС зависает после запуска Xorg](#Гостевая_ОС_зависает_после_запуска_Xorg)
    *   [6.16 Исчезновение пунктов меню и ошибка "NS_ERROR_FAILURE"](#Исчезновение_пунктов_меню_и_ошибка_"NS_ERROR_FAILURE")
    *   [6.17 "Указанный путь не существует. Проверьте путь и попробуйте еще раз." Ошибка в гостевой ОС Windows](#"Указанный_путь_не_существует._Проверьте_путь_и_попробуйте_еще_раз."_Ошибка_в_гостевой_ОС_Windows)
    *   [6.18 Не работают 64-битные гостевые ОС](#Не_работают_64-битные_гостевые_ОС)
    *   [6.19 Ошибка "VT-x is not available. (VERR_VMX_NO_VMX)" и процессор без поддержки виртуализации](#Ошибка_"VT-x_is_not_available._(VERR_VMX_NO_VMX)"_и_процессор_без_поддержки_виртуализации)
...
Для автоматической загрузки модуля VirtualBox при старте, посмотрите [Модули ядра#Автоматическое управление модулями](/index.php/Kernel_modules_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Автоматическое_управление_модулями "Kernel modules (Русский)") и создайте файл `*.conf` (например `virtualbox.conf`) в каталоге `/etc/modules-load.d/` с записью:
...
Если вы установили [qt4](https://aur.archlinux.org/packages/qt4/) из дополнительных зависимостей, у вас будет приятный GUI-интерфейс.
...
**Важно:** Если вы собираетесь хранить образы виртуальных дисков в файловой системе [Btrfs](/index.php/Btrfs "Btrfs") , то перед созданием каких-либо образов вы должны отключить журналирование [Copy-on-Write](/index.php/Btrfs#Copy-on-Write_(CoW) "Btrfs") для каталога с образами.
...
Чтобы загрузить модуль VirtualBox во время загрузки системы, обратитесь к [Загрузка ядра#Автоматическое управление модулями](/index.php/Kernel_modules_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Автоматическое_управление_модулями "Kernel modules (Русский)") и создайте `*.conf` файл (например, `virtualbox.conf`) в директории `/etc/modules-load.d/` с следующим содержанием:
...
Во-первых, ознакомьтесь с [форматами, поддерживаемыми Virtualbox](#Форматы,_поддерживаемые_VirtualBox) и [форматами других гипервизоров](https://en.wikipedia.org/wiki/Comparison_of_platform_virtual_machines#Image_type_compatibility "wikipedia:Comparison of platform virtual machines").

*   Импорт и экспорт виртуальной машины из/в формат VMware не является проблемой, если вы используете формат диска VMDK или OVF, в противном случае преобразования [VMDK в VDI и VDI в VMDK](#VMDK_в_VDI_и_VDI_в_VMDK) можно осуществить вышеописанным VMware vCenter Converter.

*   Импорт и экспорт из/в QEMU почти не проблема: некоторые форматы QEMU поддерживает непосредственно VirtualBox и преобразование между [QCOW2 в VDI и VDI в QCOW2](#QCOW2_в_VDI_и_VDI_в_QCOW2) по-прежнему доступны в случае необходимости.
...
*   В VirtualBox, если вы не хотите просмотривать весь графический интерфейс, чтобы найти нужное место где можно добавить новый виртуальный диск устройства, вы можете [Заменить виртуальный диск вручную из файла .vbox](#Замена_виртуального_диска_из_файла_.vbox_вручную), или использовать `VBoxManage storageattach`, описанный в [увеличении вируального диска](#Увеличение_размера_виртуальных_дисков) или в [руководстве VirtualBox](https://www.virtualbox.org/manual/ch08.html#vboxmanage-storageattach)
...
Так как QCOW2 предостовляется в двух версиях (0.10 и 1.1) (см. [форматы, поддерживаемые VirtualBox](#Форматы,_поддерживаемые_VirtualBox). Используйте параметр `-o compat=` для выбора версии.
...
**Примечание:** Все указанные команды поддерживают [все форматы виртуальных дисков, поддерживаемые VirtualBox](#Форматы,_поддерживаемые_VirtualBox).
...
**Примечание:** Вам понадобятся установленные гостевые дополнения, прежде чем следовать приведенному ниже примеру. См. [Дополнения гостевой ОС](#Образ_диска_с_гостевыми_дополнениями) для более подробной информации.
...
Убедитесь, что в конфигурации вашего [mkinitcpio](/index.php/Mkinitcpio_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Mkinitcpio (Русский)") есть [хук](/index.php/Mkinitcpio_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Запрет_обработчиков "Mkinitcpio (Русский)") `block`:
...
Если это не так, добавьте хук и сгенерируйте свой initramfs, с помощью команды `# mkinitcpio -p linux`, что [описано здесь более подробно](/index.php/Mkinitcpio_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Создание_образа "Mkinitcpio (Русский)").
...
Наконец, вы можете легко интегрировать Arch Linux с хост-системой и синхронизировать буфер обмена между двумя ОС. Обратитесь к [установке гостевых дополнений](#Образ_диска_с_гостевыми_дополнениями) для этого.

**Важно:** Для [Xorg (Русский)](/index.php/Xorg_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Xorg (Русский)"), чтобы работать в родной и в виртуальной машине, так как очевидно, он должен использовать другой драйвер, то лучше, если не будет `/etc/X11/xorg.conf` - так как Xorg будет собирать все, что необходимо на лету. Однако, если вам действительно нужно свою собственную конфигурацию Xorg, может быть, стоит установить используемые по умолчанию цели Systemd к `multi-user.target` с `# systemctl isolate graphical.target` (более [подробно](/index.php/Systemd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Изменение_текущей_цели "Systemd (Русский)")). Таким образом, графический интерфейс будет отключен (т.е. Xorg не запустится) и после входа в систему вы сможете выполнить `startx` вручную с пользовательским `xorg.conf`.
...
Этот сценарий очень похож на [Запуск установленного ArchLinux в VirtualBox](#Запуск_установленного_Arch_Linux_внутри_VirtualBox), но будет реализовывать эти шаги в другом порядке: начать с создания [.vmdk образа жёсткого диска](#Создайте_потоковый(raw)_образ_.vmdk), а затем [создавать файл конфигурации виртуальной машины](#Создание_файла_конфигурации_виртуальной_машины).

Теперь вы должны иметь рабочую конфигурацию виртуальной машины, чей виртуальный VMDK-диск связан с реальным диском. Процесс установки точно такой же, как и описанный в [пошаговой установке ArchLinux в VirtualBox](#Пошаговая_установка_Arch_Linux_как_гостевой_ОС), но сначала [убедитесь в постоянной схеме наименования разделов](#Убедитесь,_что_система_наименования_разделов_не_изменяется) и [корректности образа mkinitcpio](#Убедитесь_в_корректности_образа_mkinitcpio).
...
*   Установите VirtualBox на ваш GNU/Linux-хост (см. [подробнее](#Пошаговая_установка_на_хост-компьютер_под_управлением_Arch_Linux) ).
...
Для того, чтобы управление мышкой возвращалось основной ОС при выходе курсора за границы окна виртуальной машины, без нажатия каких-либо клавиш, и для возможности бесшовной интеграции, установите гостевое дополнение на виртуальную машину. Читайте шаг [#Установка гостевых дополнений](#Установка_гостевых_дополнений) если вы новичок в Arch Linux, или читайте официальную справку VirtualBox.
...
Ваш пользователь должен быть в группе `vboxusers`, и вы должны установить [пакет дополнений](#Пакет_дополнений), если хотите иметь поддержку USB 2\. Тогда вы сможете включить USB 2 в настройках виртуальной машины и добавить один или несколько фильтров для устройств, которые будут иметь доступ из гостевой ОС.
...
Убедитесь в том, что все модули ядра VirtualBox загружены (см. [Загрузка модулей ядра VirtualBox](#Загрузка_модулей_ядра_VirtualBox)).
