...
*   [3 Настройка устройств](#Настройка_устройств)
    *   [3.1 Проводная сеть (Ethernet)](#Проводная_сеть_(Ethernet))
    *   [3.2 Беспроводная сеть (Wi-Fi)](#Беспроводная_сеть_(Wi-Fi))
        *   [3.2.1 Atheros AR9285](#Atheros_AR9285)
        *   [3.2.2 Realtek 8192E](#Realtek_8192E)
        *   [3.2.3 Broadcom BCM4313 (Samsung N150+)](#Broadcom_BCM4313_(Samsung_N150+))
    *   [3.3 Графика](#Графика)
        *   [3.3.1 Подсветка](#Подсветка)
    *   [3.4 Звук](#Звук)
    *   [3.5 Ждущий режим (STR/S3)](#Ждущий_режим_(STR/S3))
    *   [3.6 Спящий режим (hibernate)](#Спящий_режим_(hibernate))
    *   [3.7 Сенсорная панель (Тачпад)](#Сенсорная_панель_(Тачпад))
    *   [3.8 Управление частотой процессора](#Управление_частотой_процессора)
*   [4 Клавиша Fn](#Клавиша_Fn)
*   [5 Уменьшение времени загрузки](#Уменьшение_времени_загрузки)
...
Беспроводной адаптер на этом чипе работает "из коробки", включая полную поддержку шифрования WPA2-PSK через NetworkManager. Мощность передатчика может быть изменена при установке пакета rfkill, с помощью следующего скрипта, помещенного, к примеру, в /usr/local/sbin/rftoggle
...
Чтобы тачпад понимал нажатия на него, самым простым решением будет установить пакеты [gsynaptics](https://www.archlinux.org/packages/?name=gsynaptics) или [gpointing-device-settings](https://aur.archlinux.org/packages/gpointing-device-settings/). Хотя согласно документам поставляемым с нетбуком, заявлена поддержка мультитача (для двухпальцевых нажатий, прокрутки двумя пальцами и т.д.), не похоже, что это работает правильно.
