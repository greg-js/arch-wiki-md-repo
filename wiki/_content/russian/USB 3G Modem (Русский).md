...
Если переключение модема прошло успешно, в `/dev` появится новое устройство с именем вида `ttyUSBn`, где _n_ - число.

**Обратите внимание:** Если у вас не появилось устройства с именем `ttyUSBn`, обратите внимание на устройства `ttyACMn`. Некоторые устройства "прописываются" под такими именами.
...
**Обратите внимание:** При отправке запросов USSD используется кодировка PDU

Чтобы закодировать запрос USSD в PDU, используйте команду:

```
perl -e '@a=split(//,unpack("b*","_Запрос USSD_")); for ($i=7; $i < $#a; $i+=8) { $a[$i]="" } print uc(unpack("H*", pack("b*", join("", @a))))."\n"'
...
```
perl -e 'print pack("H*", "_Полученный ответ на запрос USSD_");'
...
```
perl -e '@a=split(//,unpack("b*", pack("H*","_Ответ в USSD_"))); for ($i=6; $i < $#a; $i+=7) {$a[$i].="0" } print pack("b*", join("", @a)).""'
...
```
gammu sendsms TEXT _<номер телефона: +7..........>_ -text _<текст сообщения>_
