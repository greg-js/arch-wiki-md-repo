**Состояние перевода:** На этой странице представлен перевод статьи [Xfce](/index.php/Xfce "Xfce"). Дата последней синхронизации: 14 января 2015\. Вы можете [помочь](/index.php/ArchWiki_Translation_Team_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "ArchWiki Translation Team (Русский)") синхронизировать перевод, если в английской версии произошли [изменения](https://wiki.archlinux.org/index.php?title=Xfce&diff=0&oldid=356524).
...
*   [2 Запуск Xfce](#Запуск_Xfce)
    *   [2.1 Вход через интерфейс](#Вход_через_интерфейс)
    *   [2.2 Виртуальная консоль](#Виртуальная_консоль)
*   [3 Настройка](#Настройка)
    *   [3.1 Меню](#Меню)
        *   [3.1.1 Whisker Menu](#Whisker_Menu)
        *   [3.1.2 Редактирование записей](#Редактирование_записей)
    *   [3.2 Рабочий стол](#Рабочий_стол)
        *   [3.2.1 Прозрачный фон для подписей значков](#Прозрачный_фон_для_подписей_значков)
        *   [3.2.2 Скрытие разделов](#Скрытие_разделов)
        *   [3.2.3 Убрать пункты Thunar из контекстного меню](#Убрать_пункты_Thunar_из_контекстного_меню)
        *   [3.2.4 Комбинация клавиш для закрытия окон](#Комбинация_клавиш_для_закрытия_окон)
    *   [3.3 Сеанс Xfce](#Сеанс_Xfce)
        *   [3.3.1 Автозапуск приложений](#Автозапуск_приложений)
        *   [3.3.2 Блокировка экрана](#Блокировка_экрана)
        *   [3.3.3 Переключение пользователей](#Переключение_пользователей)
        *   [3.3.4 Отключение сохранения сеансов](#Отключение_сохранения_сеансов)
        *   [3.3.5 Оконный менеджер по умолчанию](#Оконный_менеджер_по_умолчанию)
    *   [3.4 Темы](#Темы)
        *   [3.4.1 Согласованность компонентов](#Согласованность_компонентов)
        *   [3.4.2 Курсоры мыши](#Курсоры_мыши)
        *   [3.4.3 Значки](#Значки)
        *   [3.4.4 Шрифты](#Шрифты)
    *   [3.5 Звук](#Звук)
        *   [3.5.1 Звуковая тема](#Звуковая_тема)
        *   [3.5.2 Настройка xfce4-mixer](#Настройка_xfce4-mixer)
        *   [3.5.3 Xfce4-mixer и OSS4](#Xfce4-mixer_и_OSS4)
        *   [3.5.4 Xfce4-mixer и pulseaudio](#Xfce4-mixer_и_pulseaudio)
        *   [3.5.5 Звуковые мультимедиа-клавиши](#Звуковые_мультимедиа-клавиши)
            *   [3.5.5.1 ALSA](#ALSA)
            *   [3.5.5.2 PulseAudio](#PulseAudio)
            *   [3.5.5.3 OSS](#OSS)
            *   [3.5.5.4 Xfce4-volumed](#Xfce4-volumed)
            *   [3.5.5.5 Volumeicon](#Volumeicon)
            *   [3.5.5.6 Специальные клавиши](#Специальные_клавиши)
        *   [3.5.6 Добавление звука запуска системы](#Добавление_звука_запуска_системы)
    *   [3.6 Сочетания клавиш](#Сочетания_клавиш)
        *   [3.6.1 Сложные сочетания](#Сложные_сочетания)
    *   [3.7 Агент аутентификаци Polkit](#Агент_аутентификаци_Polkit)
*   [4 Советы и рекомендации](#Советы_и_рекомендации)
    *   [4.1 Интеграция с xdg-open (предпочтительные приложения)](#Интеграция_с_xdg-open_(предпочтительные_приложения))
    *   [4.2 Скриншоты](#Скриншоты)
        *   [4.2.1 Клавиша Print Screen](#Клавиша_Print_Screen)
    *   [4.3 Блокировка клавиш F1 и F11 в терминале](#Блокировка_клавиш_F1_и_F11_в_терминале)
    *   [4.4 Цветовые схемы терминала](#Цветовые_схемы_терминала)
        *   [4.4.1 Изменение стандартной цветовой схемы](#Изменение_стандартной_цветовой_схемы)
        *   [4.4.2 Цветовая схема tango](#Цветовая_схема_tango)
    *   [4.5 Цветовые профили](#Цветовые_профили)
        *   [4.5.1 Загрузка профиля](#Загрузка_профиля)
        *   [4.5.2 Создание профиля](#Создание_профиля)
    *   [4.6 Несколько мониторов](#Несколько_мониторов)
    *   [4.7 Пользовательские каталоги XDG](#Пользовательские_каталоги_XDG)
    *   [4.8 SSH Agents](#SSH_Agents)
    *   [4.9 Bluetooth](#Bluetooth)
    *   [4.10 Прокрутка в фоновом окне без фокусировки на нем](#Прокрутка_в_фоновом_окне_без_фокусировки_на_нем)
    *   [4.11 Прозрачность активного окна](#Прозрачность_активного_окна)
*   [5 Решение проблем](#Решение_проблем)
    *   [5.1 Отсутствующие значки на кнопках действий](#Отсутствующие_значки_на_кнопках_действий)
    *   [5.2 Сбиваются позиции ярлыков рабочего стола](#Сбиваются_позиции_ярлыков_рабочего_стола)
    *   [5.3 Темы GTK не работают с несколькими мониторами](#Темы_GTK_не_работают_с_несколькими_мониторами)
    *   [5.4 HTML-файлы не открываются корректно в Firefox](#HTML-файлы_не_открываются_корректно_в_Firefox)
    *   [5.5 Значки не появляются в контекстных меню](#Значки_не_появляются_в_контекстных_меню)
    *   [5.6 Настройки клавиатуры не сохраняются в xkb-plugin](#Настройки_клавиатуры_не_сохраняются_в_xkb-plugin)
    *   [5.7 GDM игнорирует локали](#GDM_игнорирует_локали)
    *   [5.8 В меню отсутствуют приложения Wine](#В_меню_отсутствуют_приложения_Wine)
    *   [5.9 Повторный вход в спящий режим](#Повторный_вход_в_спящий_режим)
    *   [5.10 Некорректное отображение символов при монтировании USB-накопителей](#Некорректное_отображение_символов_при_монтировании_USB-накопителей)
    *   [5.11 NVIDIA и xfce4-sensors-plugin](#NVIDIA_и_xfce4-sensors-plugin)
    *   [5.12 Отключение экрана](#Отключение_экрана)
    *   [5.13 Настройки предпочтений не работают](#Настройки_предпочтений_не_работают)
    *   [5.14 Лишний пункт в контекстном меню рабочего стола](#Лишний_пункт_в_контекстном_меню_рабочего_стола)
    *   [5.15 Восстановление стандартных настроек](#Восстановление_стандартных_настроек)
    *   [5.16 Отказ сеанса](#Отказ_сеанса)
...
Подробнее см. в разделе [GTK+ 3.x](/index.php/GTK%2B_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Различные_темы_приложений_между_GTK+_2_и_GTK+_3 "GTK+ (Русский)") для GTK+ 3 и [Uniform look for Qt and GTK applications](/index.php/Uniform_look_for_Qt_and_GTK_applications "Uniform look for Qt and GTK applications") для Qt.
...
Также вам может понадобиться сменить звуковое устройство по умолчанию на PluseAudio — смотрите [#Xfce4-mixer и pulseaudio](#Xfce4-mixer_и_pulseaudio).
...
```
chmod 500 ~/.config/xfce4/desktop/
...
Если вы задали предпочтительные приложения с *exo-preferred-applications*, но они не работают, посмотрите подраздел [#Интеграция с xdg-open (предпочтительные приложения)](#Интеграция_с_xdg-open_(предпочтительные_приложения)).
...
И перезайдите в систему. Если вы получили "Unable to load a failsafe session" во время входа, посмотрите [#Отказ сеанса](#Отказ_сеанса).
