...
Для корректной работы *makepkg* необходимо предварительно [установить](/index.php/%D0%A3%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%B8%D1%82%D1%8C "Установить") группу пакетов [base-devel](https://www.archlinux.org/groups/x86_64/base-devel/). Пакеты из этой группы не нужно указывать как зависимости для сборки (*makedepends*) в файлах [PKGBUILD](/index.php/PKGBUILD_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "PKGBUILD (Русский)").
...
*   Запуск *makepkg* от лица root-пользователя **запрещен** [[2]](https://lists.archlinux.org/pipermail/pacman-dev/2014-March/018911.html)[[3]](https://git.archlinux.org/pacman.git/tree/NEWS). Поскольку `PKGBUILD` может содержать произвольные команды, сборка пакетов под root-аккаунтом признана небезопасной [[4]](https://bbs.archlinux.org/viewtopic.php?id=67561). Если у пользователя нет доступа к обычному пользовательскому аккаунту, то следует запускать *makepkg* от лица ["nobody user"](http://allanmcrae.com/2015/01/replacing-makepkg-asroot/).

Чтобы собрать пакет, первым делом необходимо создать файл [PKGBUILD](/index.php/PKGBUILD_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "PKGBUILD (Русский)"), скрипт сборки, как описано в статье [Создание пакетов](/index.php/%D0%A1%D0%BE%D0%B7%D0%B4%D0%B0%D0%BD%D0%B8%D0%B5_%D0%BF%D0%B0%D0%BA%D0%B5%D1%82%D0%BE%D0%B2 "Создание пакетов"). Существующие скрипты находятся в репозиториях [Системы сборки Arch](/index.php/%D0%A1%D0%B8%D1%81%D1%82%D0%B5%D0%BC%D0%B0_%D1%81%D0%B1%D0%BE%D1%80%D0%BA%D0%B8_Arch "Система сборки Arch"), а также в [AUR](/index.php/AUR_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "AUR (Русский)"). После того, как файл `PKGBUILD` получен, переместитесь в его каталог и выполните следующую команду, чтобы собрать пакет:
...
Добавление опции `-r/--rmdeps` скажет *makepkg*, что необходимо удалить установленные зависимости позже, если они больше будут не нужны. Если вы постоянно занимаетесь сборкой пакетов и не хотите загрязнять систему пакетами-сиротами, то будет также полезно ознакомиться с [Pacman (Русский)/Tips and tricks (Русский)#Удаление неиспользуемых пакетов](/index.php/Pacman_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)/Tips_and_tricks_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Удаление_неиспользуемых_пакетов "Pacman (Русский)/Tips and tricks (Русский)").

**Примечание:**

*   Пакеты, устанавливаемые в качестве зависимостей должны находиться в настроенных репозиториях; детали настроек смотрите в [Pacman (Русский)#Репозитории](/index.php/Pacman_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Репозитории "Pacman (Русский)"). Помимо этого, можно установить зависимости вручную перед началом сборки посредством команды `pacman -S --asdeps *зависимость1 зависимость2*`.
*   При установке зависимостей используются только глобальные значения переменных, то есть любые попытки переопределения значений внутри функции упаковки пакета не сработают.

Когда все зависимости установлены и сборка пакета завершилась успешно, в рабочем каталоге появится файл пакета `*имя-и-версия-пакета*.pkg.tar.xz`. Чтобы его установить в систему, используйте опцию `-i/--install` (что идентично команде `pacman -U *имя-и-версия-пакета*.pkg.tar.xz`):
...
Для удаления оставшихся файлов и каталогов, таких как сохранённые в `$srcdir` файлы, используйте опцию `-c/--clean`. Это полезно в случае многократных сборок одного и того же пакета или обновления его версии, если вы планируете использовать один и тот же рабочий каталог. Это предотвращает добавление устаревших файлов в новые сборки:
...
За дополнительной информацией обращайтесь к [makepkg(8)](https://jlk.fjfi.cvut.cz/arch/manpages/man/makepkg.8).
