From [polkit homepage](http://www.freedesktop.org/wiki/Software/polkit/):
	...
	*polkit это набор инструментальных средств уровня приложения для определения и обработки политики, которая позволяет непривилегированным процессам разговаривать с привилегированными процессами: это платформа для централизации процесса принятия решений в отношении предоставления доступа к привилегированным операциям для непривилегированных приложений.*

Polkit используется для управления общесистемными привилегиями. Он обеспечивает организованный способ для непривилегированных процессов для связи с привилегированными. В отличие от систем, таких как sudo, он не предоставляет права root для всего процесса, а скорее позволяет более тонкий уровень управления централизованной политикой системы.

Polkit работает, ограничивая отдельные действия, например. Запуск GParted и разграничение пользователей по группам или по имени, например. Члены колесной группы. Затем он определяет, как, если это вообще возможно, эти пользователи разрешают эти действия, например. Путем идентификации в качестве членов группы, набрав их пароли.

## Contents

*   [1 Установка](#.D0.A3.D1.81.D1.82.D0.B0.D0.BD.D0.BE.D0.B2.D0.BA.D0.B0)
    *   [1.1 Агенты идентификации](#.D0.90.D0.B3.D0.B5.D0.BD.D1.82.D1.8B_.D0.B8.D0.B4.D0.B5.D0.BD.D1.82.D0.B8.D1.84.D0.B8.D0.BA.D0.B0.D1.86.D0.B8.D0.B8)
...
Установите [polkit](https://www.archlinux.org/packages/?name=polkit) пакет.

### Агенты идентификации

Агент аутентификации используется для того, чтобы заставить пользователя сеанса доказать, что пользователь сеанса действительно является пользователем (путем аутентификации как пользователь) или административным пользователем (путем аутентификации в качестве администратора). Пакет [POLKIT](https://www.archlinux.org/packages/?name=POLKIT) содержит текстовый агент проверки подлинности, называемый 'pkttyagent', который используется в качестве запасного варианта.
