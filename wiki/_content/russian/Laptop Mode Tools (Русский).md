...
# Своевременное отключение данных функций сработает _только_ если ваше
...
# об уровне заряда, вы можете включить модуль _battery-level-polling_, чтобы
...
# большим, потому как подобный режим чтения применяется ко _всем_ файлам,
# считываемым с диска.
LM_READAHEAD=3072
NOLM_READAHEAD=128
...
# _init_ с параметром _reload_. Демон перезагружается, считывая параметры
