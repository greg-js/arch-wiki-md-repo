...
Обратите внимание, что поиск шрифта происходит в `/usr/share/kbd/consolefont`.
...
**Обратите внимание:** Значение `LOCALE=` может быть как `"ru_RU.UTF-8"`, так и `"ru_RU.utf-8"` или `"ru_RU.utf8"`. Но, с целью уменьшения путаницы, все же лучше использовать вариант `LOCALE="ru_RU.UTF-8"`.
...
**Обратите внимание:** Если вышеперечисленные способы не помогли изменить шрифт в tty, вероятно, изменения не сохраняются после KMS. Для обхода этой проблемы. попробуйте добавить в initrd модуль, выполняющий modesetting (nouveau/radeon/i915 и т.п.): `MODULES="nouveau"` 
...
**Обратите внимание:** Начиная с версии Xorg 1.8, [HAL](/index.php/HAL "HAL") не используется для настройки
