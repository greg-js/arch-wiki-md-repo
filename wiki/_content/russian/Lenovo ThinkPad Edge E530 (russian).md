...
Добавлять модули **b43** и **brcmsmac** в **blacklist** нет необходимости, так как пакет [broadcom-wl](https://www.archlinux.org/packages/?name=broadcom-wl) уже содержит нужный файл. Следует также заметить, что данная сетевая карта с модулем **b43** **не работает**.
