...
1\. Получает ли ваш логин SSH дополнительную информацию от сервера, например, файл `/etc/issue`? Это может запутать SSHFS. Вас следует временно отключить серверный файл `/etc/issue`:
