...
*   [2 Настрока для одной системы](#Настрока_для_одной_системы)
    *   [2.1 Созданием необходимых цепочек](#Созданием_необходимых_цепочек)
    *   [2.2 Цепочка INPUT](#Цепочка_INPUT)
    *   [2.3 Цепочка FORWARD](#Цепочка_FORWARD)
    *   [2.4 Цепочка OUTPUT](#Цепочка_OUTPUT)
    *   [2.5 Цепочка interfaces](#Цепочка_interfaces)
    *   [2.6 Цепочка open](#Цепочка_open)
    *   [2.7 Защита от основных атак](#Защита_от_основных_атак)
        *   [2.7.1 Force SYN packets check](#Force_SYN_packets_check)
        *   [2.7.2 Force Fragments packets check](#Force_Fragments_packets_check)
        *   [2.7.3 XMAS пакеты](#XMAS_пакеты)
        *   [2.7.4 Drop all NULL packets](#Drop_all_NULL_packets)
        *   [2.7.5 Spoofing attack](#Spoofing_attack)
    *   [2.8 "Прячем" ваш компьютер](#"Прячем"_ваш_компьютер)
        *   [2.8.1 Блокирование Ping запросов](#Блокирование_Ping_запросов)
        *   [2.8.2 ICMP type match blocking](#ICMP_type_match_blocking)
        *   [2.8.3 Block nmap's uptime detection](#Block_nmap's_uptime_detection)
        *   [2.8.4 Прочие настройки ядра](#Прочие_настройки_ядра)
    *   [2.9 Сохранение правил](#Сохранение_правил)
*   [3 Настройка NAT шлюза](#Настройка_NAT_шлюза)
    *   [3.1 Настройка таблицы filter](#Настройка_таблицы_filter)
        *   [3.1.1 Создание цепочки necessary](#Создание_цепочки_necessary)
        *   [3.1.2 Настройка цепочки FORWARD](#Настройка_цепочки_FORWARD)
        *   [3.1.3 Настройка цепочек fw-interfaces и fw-open](#Настройка_цепочек_fw-interfaces_и_fw-open)
    *   [3.2 Настройка таблицы nat](#Настройка_таблицы_nat)
        *   [3.2.1 Настройка цепочки POSTROUTING](#Настройка_цепочки_POSTROUTING)
        *   [3.2.2 Настройка цепочки PREROUTING](#Настройка_цепочки_PREROUTING)
    *   [3.3 Сохранение правил](#Сохранение_правил_2)
*   [4 Работаем с knockd](#Работаем_с_knockd)
*   [5 Также Смотрите](#Также_Смотрите)
