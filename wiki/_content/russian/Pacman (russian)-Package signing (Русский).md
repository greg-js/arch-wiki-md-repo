...
*   [1 Установка](#Установка)
    *   [1.1 Настройка pacman'a](#Настройка_pacman'a)
    *   [1.2 Инициализация связки ключей](#Инициализация_связки_ключей)
*   [2 Управление связкой ключей](#Управление_связкой_ключей)
    *   [2.1 Проверка пяти мастер-ключей](#Проверка_пяти_мастер-ключей)
    *   [2.2 Добавление ключей разработчика](#Добавление_ключей_разработчика)
    *   [2.3 Добавление неофициальных ключей](#Добавление_неофициальных_ключей)
    *   [2.4 Отладка при помощи gpg](#Отладка_при_помощи_gpg)
*   [3 Решение проблем](#Решение_проблем)
    *   [3.1 Не удается импортировать ключи](#Не_удается_импортировать_ключи)
    *   [3.2 Отключение проверки подписи](#Отключение_проверки_подписи)
    *   [3.3 Сброс всех ключей](#Сброс_всех_ключей)
    *   [3.4 Удаление залежавшихся пакетов](#Удаление_залежавшихся_пакетов)
    *   [3.5 Обновление ключей через прокси](#Обновление_ключей_через_прокси)
...
Во время синхронизации обновления вы можете застрять из-за устаревшего пакета [archlinux-keyring](https://www.archlinux.org/packages/?name=archlinux-keyring). Сначала попробуйте [обновить систему](/index.php/Pacman_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Обновление_пакетов "Pacman (Русский)"), это может помочь.
