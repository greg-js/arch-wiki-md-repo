...
**Обратите внимание:** `gpg-agent.conf` должен быть создан, и должна быть установлена переменная `write-env-file`, чтобы позволить ключам *gpg-agent* вводиться в SSH, пока вы не перезапустите *gpg-agent*, и, следовательно, экспортировать эти настройки при каждом входе
 `~/.gnupg/gpg-agent.conf` 
```
  # Environment file
