...
Когда вы получите _.hcd_ файл, скопируйте его в `/lib/firmware/brcm/BCM.hcd` - _dmesg_ знает, что по этому пути лежит прошивка и в вашем случае он может меняться, поэтому проверьте вывод команды _dmesg_, чтобы убедиться в этом. Затем прошейте файл _.hcd_ с помощью утилиты _brcm_patchram_plus_, предоставляемой пакетом [brcm_patchram_plus-git](https://aur.archlinux.org/packages/brcm_patchram_plus-git/). Загрузите модуль btusb:
