...
*   В файле `/etc/ssh/ssh**d**_config`:
    *   Удостоверьтесь, что опциям `AllowTcpForwarding` и `X11UseLocalhost` присвоены значения *yes*, а `X11DisplayOffset` - *10* (это значения по умолчанию, если ничего не менялось; смотрите [sshd_config(5)](http://jlk.fjfi.cvut.cz/arch/manpages/man/sshd_config.5))
...
где имя_хоста - это имя конкретного хоста. Для получения дополнительной информации смотрите [xhost(1)](http://jlk.fjfi.cvut.cz/arch/manpages/man/xhost.1).
...
**Важно:** В [ssh(1)](http://jlk.fjfi.cvut.cz/arch/manpages/man/ssh.1) утверждается, что "*сжатие помогает в случае использования модемных линий и других медленных соединений, а в сетях с большой скоростью лишь замедляет работу*". В зависимости от конфигурации вашей сети этот совет может быть контрпродуктивен
