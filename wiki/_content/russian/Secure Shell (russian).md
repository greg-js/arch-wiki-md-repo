...
*   [1 OpenSSH](#OpenSSH)
    ...
    *   [1.2 Настройка SSH](#Настройка_SSH)
        *   [1.2.1 Клиент](#Клиент)
        *   [1.2.2 Демон](#Демон)
    *   [1.3 Управление демоном sshd](#Управление_демоном_sshd)
    *   [1.4 Подключение к серверу](#Подключение_к_серверу)
    *   [1.5 Защита SSH](#Защита_SSH)
        *   [1.5.1 Защита от атак полного перебора (brute force)](#Защита_от_атак_полного_перебора_(brute_force))
        *   [1.5.2 Ограничение входа от имени суперпользователя](#Ограничение_входа_от_имени_суперпользователя)
            *   [1.5.2.1 Отключение](#Отключение)
            *   [1.5.2.2 Ограничение](#Ограничение)
*   [2 Другие клиенты и серверы SSH](#Другие_клиенты_и_серверы_SSH)
    *   [2.1 Dropbear](#Dropbear)
    *   [2.2 Альтернатива SSH: Mobile Shell](#Альтернатива_SSH:_Mobile_Shell)
*   [3 Советы и рекомендации](#Советы_и_рекомендации)
    *   [3.1 Шифрованный туннель SOCKS](#Шифрованный_туннель_SOCKS)
        *   [3.1.1 Шаг 1: установка соединения](#Шаг_1:_установка_соединения)
        *   [3.1.2 Шаг 2: настройка браузера (или других программ)](#Шаг_2:_настройка_браузера_(или_других_программ))
    *   [3.2 Проброс X11](#Проброс_X11)
        *   [3.2.1 Настройка](#Настройка)
        *   [3.2.2 Использование](#Использование)
    *   [3.3 Проброс других портов](#Проброс_других_портов)
    *   [3.4 Увеличение скорости SSH](#Увеличение_скорости_SSH)
    *   [3.5 Монтирование удаленных файловых систем при помощи SSHFS](#Монтирование_удаленных_файловых_систем_при_помощи_SSHFS)
    *   [3.6 Поддержание подключения](#Поддержание_подключения)
    *   [3.7 Сохранение данных о подключении в конфигурационном файле ssh](#Сохранение_данных_о_подключении_в_конфигурационном_файле_ssh)
    *   [3.8 Autossh - автоматический перезапуск сессий и туннелей SSH](#Autossh_-_автоматический_перезапуск_сессий_и_туннелей_SSH)
        *   [3.8.1 Автозапуск Autossh при загрузке системы при помощи systemd](#Автозапуск_Autossh_при_загрузке_системы_при_помощи_systemd)
*   [4 Изменение номера порта SSH для активации через сокет (sshd.socket)](#Изменение_номера_порта_SSH_для_активации_через_сокет_(sshd.socket))
*   [5 Решение проблем](#Решение_проблем)
    *   [5.1 Проверка](#Проверка)
        *   [5.1.1 Удаление устаревших ключей (необязательно)](#Удаление_устаревших_ключей_(необязательно))
        *   [5.1.2 Рекомендации](#Рекомендации)
    *   [5.2 Зависание подключения SSH после выключения/перезагрузки](#Зависание_подключения_SSH_после_выключения/перезагрузки)
    *   [5.3 Подключение отклонено или проблема тайм-аута](#Подключение_отклонено_или_проблема_тайм-аута)
        *   [5.3.1 Ваш роутер настроен на проброс портов?](#Ваш_роутер_настроен_на_проброс_портов?)
        *   [5.3.2 SSH запущен и прослушивает?](#SSH_запущен_и_прослушивает?)
        *   [5.3.3 Имеются ли правила фаерволла, блокирующие соединения?](#Имеются_ли_правила_фаерволла,_блокирующие_соединения?)
        *   [5.3.4 Трафик доходит до вашего компьютера?](#Трафик_доходит_до_вашего_компьютера?)
        *   [5.3.5 Ваш провайдер или кто-то еще блокирует нужный порт?](#Ваш_провайдер_или_кто-то_еще_блокирует_нужный_порт?)
            *   [5.3.5.1 Диагностика при помощи Wireshark](#Диагностика_при_помощи_Wireshark)
            *   [5.3.5.2 Возможное решение](#Возможное_решение)
        *   [5.3.6 Read from socket failed: connection reset by peer](#Read_from_socket_failed:_connection_reset_by_peer)
    *   [5.4 "[ваша командная оболочка]: No such file or directory" / ssh_exchange_identification problem](#"[ваша_командная_оболочка]:_No_such_file_or_directory"_/_ssh_exchange_identification_problem)
    *   [5.5 Сообщение об ошибке "Terminal unknown" или "Error opening terminal "](#Сообщение_об_ошибке_"Terminal_unknown"_или_"Error_opening_terminal_")
        *   [5.5.1 Обходной путь с использованием переменной $TERM](#Обходной_путь_с_использованием_переменной_$TERM)
        *   [5.5.2 Решение с использованием файла terminfo](#Решение_с_использованием_файла_terminfo)
...
[Установите](/index.php/Pacman_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Установка_определенных_пакетов "Pacman (Русский)") пакет [openssh](https://www.archlinux.org/packages/?name=openssh) из [официальных репозиториев](/index.php/Official_repositories_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Official repositories (Русский)").
...
*   Полное отключение входа с использованием паролей значительно увеличит безопасность. Для получения дополнительной информации смотрите раздел [Ключи SSH#Отключение входа по паролю](/index.php/%D0%9A%D0%BB%D1%8E%D1%87%D0%B8_SSH#Отключение_входа_по_паролю "Ключи SSH")

### Управление демоном sshd

Демон SSH имеет различные файлы юнитов systemd.

Вы можете запустить демон для немедленного использования и/или включить его в автозагрузку при старте системы, как описано в разделе [Systemd (Русский)#Использование юнитов](/index.php/Systemd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Использование_юнитов "Systemd (Русский)"):
...
*   В файле `/etc/ssh/ssh**d**_config`:
    *   Удостоверьтесь, что опциям `AllowTcpForwarding` и `X11UseLocalhost` присвоены значения *yes*, а `X11DisplayOffset` - *10* (это значения по умолчанию, если ничего не менялось; смотрите [sshd_config(5)](https://jlk.fjfi.cvut.cz/arch/manpages/man/sshd_config.5))
    *   Установите для опции `X11Forwarding` значение *yes*
*   [Перезапустите](/index.php/Systemd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Использование_юнитов "Systemd (Русский)") [демон *sshd*](#Управление_демоном_sshd)

На клиенте включите опцию `ForwardX11` либо добавив флаг `-X` в командной строке для необходимых подключений, либо присвоив в [конфигурационном файле клиента openSSH](#Клиент) опции `ForwardX11` значение *yes*.
...
[Войдите в систему на удаленной машине](#Подключение_к_серверу), как обычно, добавив флаг `-X`, если опция *ForwardX11* не включена в конфигурационном файле клиента:
...
где имя_хоста - это имя конкретного хоста. Для получения дополнительной информации смотрите [xhost(1)](https://jlk.fjfi.cvut.cz/arch/manpages/man/xhost.1).
...
**Важно:** В [ssh(1)](https://jlk.fjfi.cvut.cz/arch/manpages/man/ssh.1) утверждается, что "*сжатие помогает в случае использования модемных линий и других медленных соединений, а в сетях с большой скоростью лишь замедляет работу*". В зависимости от конфигурации вашей сети этот совет может быть контрпродуктивен
