...
Активный католог использует [Lightweight Directory Access Protocol (LDAP)](https://en.wikipedia.org/wiki/Ldap и DNS. Эти же стандарты доступны в Linux, но их комбинирование - непростая задача. Эта статья поможет вам настроить хост ArchLinux для аутентификация в домене AD.
...
*   [2 Настройка](#Настройка)
    *   [2.1 Настройка AD](#Настройка_AD)
        *   [2.1.1 Обновление GPO](#Обновление_GPO)
    *   [2.2 Настройка Linux хоста](#Настройка_Linux_хоста)
    *   [2.3 Установка](#Установка)
    *   [2.4 Обновление DNS](#Обновление_DNS)
    *   [2.5 Настройка NTP](#Настройка_NTP)
    *   [2.6 Kerberos](#Kerberos)
        *   [2.6.1 Создание ключа Kerberos](#Создание_ключа_Kerberos)
        *   [2.6.2 Подтверждение ключа](#Подтверждение_ключа)
    *   [2.7 pam_winbind.conf](#pam_winbind.conf)
    *   [2.8 Samba](#Samba)
*   [3 Запуск и проверка сервисов](#Запуск_и_проверка_сервисов)
    *   [3.1 Запуск Samba](#Запуск_Samba)
    *   [3.2 Присоединение к Домену](#Присоединение_к_Домену)
    *   [3.3 Перезапуск Samba](#Перезапуск_Samba)
    *   [3.4 Проверка Winbind](#Проверка_Winbind)
    *   [3.5 Проверка nsswitch](#Проверка_nsswitch)
    *   [3.6 Проверка команд Samba](#Проверка_команд_Samba)
*   [4 Настройка PAM](#Настройка_PAM)
    *   [4.1 system-auth](#system-auth)
        *   [4.1.1 Раздел "auth"](#Раздел_"auth")
        *   [4.1.2 Раздел "account"](#Раздел_"account")
        *   [4.1.3 Раздел "session"](#Раздел_"session")
        *   [4.1.4 Раздел "password"](#Раздел_"password")
    *   [4.2 Проверка входа](#Проверка_входа)
*   [5 Настройка общих файлов](#Настройка_общих_файлов)
*   [6 Добавление файла keytab и включение беспарольного входа на машину через Kerber и ssh](#Добавление_файла_keytab_и_включение_беспарольного_входа_на_машину_через_Kerber_и_ssh)
    *   [6.1 Создание key tab файла](#Создание_key_tab_файла)
    *   [6.2 Включение входа через keytab](#Включение_входа_через_keytab)
    *   [6.3 Подготовка sshd на сервере](#Подготовка_sshd_на_сервере)
    *   [6.4 Добавление опций, нужных для клиента](#Добавление_опций,_нужных_для_клиента)
    *   [6.5 Проверка установки](#Проверка_установки)
    *   [6.6 Настройка для полной аутентификации Kerberos без пароля.](#Настройка_для_полной_аутентификации_Kerberos_без_пароля.)
*   [7 Генерирование keytab, которые будет принимать AD](#Генерирование_keytab,_которые_будет_принимать_AD)
    *   [7.1 Интересно знать](#Интересно_знать)
*   [8 Смотрите также](#Смотрите_также)
    *   [8.1 Коммерческие решения](#Коммерческие_решения)
...
в наш файл `.ssh/config`, который говорит *ssh* использовать эту опцию, как альтернативу: можно использовать 'ssh -o' (смотрите страницу справочного руководства [ssh(1)](https://jlk.fjfi.cvut.cz/arch/manpages/man/ssh.1)).
