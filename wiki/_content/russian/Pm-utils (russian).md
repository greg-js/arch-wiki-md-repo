...
**Примечание:** Если у вас возникли проблемы в работе с видеоадаптером после выхода из энергосберегающего режима, возможно, вам будет необходимо установить `vbetool` из [extra].

**Примечание:** Если вы настраиваете систему "с нуля", убедитесь в том, что установлен пакет [acpi](https://www.archlinux.org/packages/?name=acpi).
...
Как вы могли заметить, скрипт в процессе работы вызывает выполнение команды `rfkill (un)block bluetooth`, то есть выключает/включает bluetooth. За подробностями обратитесь к [rfkill(8)](http://jlk.fjfi.cvut.cz/arch/manpages/man/rfkill.8).
