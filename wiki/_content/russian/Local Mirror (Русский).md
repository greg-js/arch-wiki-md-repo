...
Итог - очень защищенный FTP-сервер, адаптированный под наши нужды. Обратите внимание, что сервер **не** требует пароля, поэтому не предназначен для работы в публичной сети (хотя, дело ваше). _Описание возможности работы pacman с FTP-сервером, защищенным паролем выходит за рамки этого документа._
