...
**Примечание:** Обычное поведение подсветки/копирования/вставки изменилось, потому что Vim входит в визуальный режим, когда используется мышь. Чтобы нормально выделять текст мышкой, удерживайте при этом `Shift`.
