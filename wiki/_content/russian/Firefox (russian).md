...
*   [4 Настройка](#Настройка)
    ...
    *   [4.2 Проверка орфографии](#Проверка_орфографии)
        *   [4.2.1 Общесистемные словари Hunspell](#Общесистемные_словари_Hunspell)
        *   [4.2.2 Словари-расширения](#Словари-расширения)
...
*   [6 Решение проблем](#Решение_проблем)
    ...
    *   [6.2 Расширения не работают на некоторых доменах Mozilla](#Расширения_не_работают_на_некоторых_доменах_Mozilla)
    *   [6.3 Запуск Firefox занимает много времени](#Запуск_Firefox_занимает_много_времени)
    *   [6.4 Исправление шрифтов](#Исправление_шрифтов)
    *   [6.5 Выбор клиента электронной почты](#Выбор_клиента_электронной_почты)
    *   [6.6 Ассоциации файлов](#Ассоциации_файлов)
    *   [6.7 Firefox самопроизвольно создаёт директорию ~/Desktop](#Firefox_самопроизвольно_создаёт_директорию_~/Desktop)
    *   [6.8 Плагины и блокирование всплывающих окон](#Плагины_и_блокирование_всплывающих_окон)
    *   [6.9 Не применяются изменения в файлах userChrome.css и userContent.css](#Не_применяются_изменения_в_файлах_userChrome.css_и_userContent.css)
    *   [6.10 Поведение при нажатии средней кнопки мыши](#Поведение_при_нажатии_средней_кнопки_мыши)
    *   [6.11 Клавиша Backspace не выполняет функцию "Назад"](#Клавиша_Backspace_не_выполняет_функцию_"Назад")
    *   [6.12 Firefox не запоминает авторизацию на сайте](#Firefox_не_запоминает_авторизацию_на_сайте)
    *   [6.13 Невозможно войти в полноэкранный режим или выйти из него](#Невозможно_войти_в_полноэкранный_режим_или_выйти_из_него)
    *   [6.14 Не появляется запрос на сохранение открытых вкладок](#Не_появляется_запрос_на_сохранение_открытых_вкладок)
    *   [6.15 Firefox обнаруживает неправильную версию плагина](#Firefox_обнаруживает_неправильную_версию_плагина)
    *   [6.16 Контекстное меню JavaScript не отображается на некоторых сайтах](#Контекстное_меню_JavaScript_не_отображается_на_некоторых_сайтах)
    *   [6.17 Firefox не запоминает язык проверки орфографии по умолчанию](#Firefox_не_запоминает_язык_проверки_орфографии_по_умолчанию)
    *   [6.18 Не отображаются некоторые символы MathML](#Не_отображаются_некоторые_символы_MathML)
    *   [6.19 Разрыв изображения в полноэкранном режиме](#Разрыв_изображения_в_полноэкранном_режиме)
    *   [6.20 Разрыв изображения во время скроллинга](#Разрыв_изображения_во_время_скроллинга)
    *   [6.21 WebRTC-модуль Firefox не обнаруживает микрофон](#WebRTC-модуль_Firefox_не_обнаруживает_микрофон)
    *   [6.22 Не получается авторизоваться с китайским аккаунтом](#Не_получается_авторизоваться_с_китайским_аккаунтом)
    *   [6.23 Некоторые видео воспроизводятся без звука при использовании JACK и PulseAudio](#Некоторые_видео_воспроизводятся_без_звука_при_использовании_JACK_и_PulseAudio)
...
В Firefox используется [FFmpeg](/index.php/FFmpeg "FFmpeg") для воспроизведения медиаконтента в HTML5-элементах `<audio>` и `<video>`. Перейдите на [страницу видео-теста](https://www.quirksmode.org/html5/tests/video.html) или [страницу аудио-теста](https://hpr.dogphilosophy.net/test/) для проверки поддерживаемых форматов.
...
Firefox поддерживает использование общесистемных словарей [Hunspell](/index.php/Hunspell "Hunspell"), а также словарей, установленных в качестве расширений.
...
Если выбор языка по умолчанию не сохраняется, см. раздел [#Firefox не запоминает язык проверки орфографии по умолчанию](#Firefox_не_запоминает_язык_проверки_орфографии_по_умолчанию).

#### Общесистемные словари Hunspell

Установите [Hunspell](/index.php/Hunspell "Hunspell") и словари необходимых языков.

#### Словари-расширения

Чтобы добавить больше языков, нажмите правой кнопкой мыши на любом текстовом поле, выберите *Добавить словари...* и загрузите необходимые словари со [списка словарей и языковых пакетов](https://addons.mozilla.org/ru/firefox/language-tools/).

**Совет:** Расширение для русского языка доступно в качестве пакета [firefox-spell-ru](https://www.archlinux.org/packages/?name=firefox-spell-ru).
...
Воспользуйтесь [Firejail](/index.php/Firejail "Firejail") с параметром `rlimit-as`, чтобы предотвратить слишком большое потребление памяти веб-страницами (и возможную [нехватку памяти](https://en.wikipedia.org/wiki/Out_of_memory "wikipedia:Out of memory")).
...
В качестве альтернативы можно воспользоваться кнопкой для создания скриншота в [инструментах разработчика](https://developer.mozilla.org/en-US/docs/Tools/Taking_screenshots).
...
Чтобы сделать данное изменение постоянным, см. [Environment variables#Graphical environment](/index.php/Environment_variables#Graphical_environment "Environment variables") и запускайте Firefox с помощью ярлыка (как обычно). Проверить применённые настройки можно снова в строке *Window Protocol*.
...
Если Firefox обнаруживает оконный менеджер, совместимый со спецификацией [EWMH/ICCCM](https://specifications.freedesktop.org/wm-spec/wm-spec-latest.html), Firefox попытается отправить корневому окну сообщение WM_STATE с запросом на вход в полноэкранный режим или выход из него (согласно определению оконного менеджера). Спецификация позволяет оконным менеджерам игнорировать данные сообщения, но в таком случае Firefox посчитает, что запрос был отклонён и передаст его конечному пользователю, что не приведёт к какому-либо результату, а также может помешать открытию видео в полноэкранном режиме. Временное решение заключается в задании параметру `full-screen-api.ignore-widgets` значения `true` на странице `about:config`.

Соответствующий отчёт об ошибке: [Bugzilla 1189622](https://bugzilla.mozilla.org/show_bug.cgi?id=1189622).
...
Firefox предоставляет локальный сервис китайским пользователям, в котором используется совершенно другой, локальный, аккаунт вместо интернационального. Firefox, установленный с помощью пакета [firefox](https://www.archlinux.org/packages/?name=firefox), по умолчанию использует интернациональную систему аккаунтов, а для её изменения на китайскую потребуется установить менеджер дополнений с [данной страницы](http://mozilla.com.cn/thread-343905-1-1.html), после чего можно будет авторизоваться, используя китайский аккаунт.
...
Имейте ввиду, что названия PulseAudio JACK Sink и портов могут отличаться от примеров. Также увидеть текущую настройку JACK можно с помощью такой патч-панели, как Catia из [cadence](https://www.archlinux.org/packages/?name=cadence).
