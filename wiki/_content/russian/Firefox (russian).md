...
*   [6 Решение проблем](#Решение_проблем)
    ...
    *   [6.11 Невозможно войти в полноэкранный режим или выйти из него](#Невозможно_войти_в_полноэкранный_режим_или_выйти_из_него)
    *   [6.12 Не появляется запрос на сохранение открытых вкладок](#Не_появляется_запрос_на_сохранение_открытых_вкладок)
    *   [6.13 Firefox обнаруживает неправильную версию плагина](#Firefox_обнаруживает_неправильную_версию_плагина)
    *   [6.14 Контекстное меню JavaScript не отображается на некоторых сайтах](#Контекстное_меню_JavaScript_не_отображается_на_некоторых_сайтах)
    *   [6.15 Firefox не запоминает язык проверки орфографии по умолчанию](#Firefox_не_запоминает_язык_проверки_орфографии_по_умолчанию)
    *   [6.16 Не отображаются некоторые символы MathML](#Не_отображаются_некоторые_символы_MathML)
    *   [6.17 Разрыв изображения в полноэкранном режиме](#Разрыв_изображения_в_полноэкранном_режиме)
    *   [6.18 Разрыв изображения во время скроллинга](#Разрыв_изображения_во_время_скроллинга)
    *   [6.19 WebRTC-модуль Firefox не обнаруживает микрофон](#WebRTC-модуль_Firefox_не_обнаруживает_микрофон)
    *   [6.20 Не получается авторизоваться с китайским аккаунтом](#Не_получается_авторизоваться_с_китайским_аккаунтом)
...
Если выбор языка по умолчанию не сохраняется, см. раздел [Firefox (Русский)#Firefox не запоминает язык проверки орфографии по умолчанию](/index.php/Firefox_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Firefox_не_запоминает_язык_проверки_орфографии_по_умолчанию "Firefox (Русский)").
