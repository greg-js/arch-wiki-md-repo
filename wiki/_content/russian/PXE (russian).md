...
*   [3 Настройка сервера](#Настройка_сервера)
    *   [3.1 Сеть](#Сеть)
    *   [3.2 DHCP + TFTP](#DHCP_+_TFTP)
    *   [3.3 HTTP](#HTTP)
*   [4 Установка](#Установка)
    *   [4.1 Загрузка](#Загрузка)
    *   [4.2 После загрузки](#После_загрузки)
*   [5 Альтернативные способы](#Альтернативные_способы)
    *   [5.1 NFS](#NFS)
    *   [5.2 NBD](#NBD)
    *   [5.3 Существующий сервер PXE](#Существующий_сервер_PXE)
    *   [5.4 Ошибка переименования интерфейса DHCP](#Ошибка_переименования_интерфейса_DHCP)
    *   [5.5 Системы с небольшим объемом памяти](#Системы_с_небольшим_объемом_памяти)
...
Просмотр [journald](/index.php/Systemd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Журнал "Systemd (Русский)") на сервере PXE даст дополнительную информацию о том, что именно происходит на ранних этапах процесса загрузки PXE:
...
Если вы не хотите, чтобы весь трафик маршрутизировался через ваш PXE-сервер (который не будет работать в любом случае, если вы [не настроите его правильно](/index.php/Simple_stateful_firewall_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Настройка_NAT-шлюза "Simple stateful firewall (Русский)")), вы захотите [остановить](/index.php/%D0%9E%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%B8%D1%82%D1%8C "Остановить") `dnsmasq.service` и получить новую аренду на цели установки, в зависимости от вашего сетевого расположения.
...
Вам нужно будет настроить [сервер NFS](/index.php/NFS_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "NFS (Русский)") с экспортом в корне вашего смонтированного установочного носителя, который будет `/mnt/archiso`, если вы следовали за разделом [#Подготовка](#Подготовка). После настройки сервера добавьте следующую строку в файл `/etc/exports`:
