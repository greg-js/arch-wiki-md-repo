...
**Обратите внимание:** Убедитесь, что в конце файла `/etc/ppp/options.pptpd` присутствует пустая строка, так как раннее замечались проблемы при запуске сервера из-за её отсутствия.
...
Ниже представлен пример конфигурации _iptables_ для настройки межсетевого экрана.
...
Найдите и удалите (или закомментируйте) опцию `logwtmp` в файле `/etc/pptpd.conf`. С включенной опцией используется _wtmp_ для записи лога подключений и отключений клиента.
