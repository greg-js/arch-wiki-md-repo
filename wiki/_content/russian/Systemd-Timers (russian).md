...
*   **Монотонные таймеры** - активируются после определенного промежутка времени по отношению к той или иной отправной точки. Они не сработают, если компьютер находится в режиме ожидания или выключен. Есть несколько различных монотонных таймеров, но все они имеют вид: `On*Type*Sec=`. `OnBootSec` и `OnActiveSec` являются общими монотонными таймерами.
...
Для полного объяснения опций таймера смотрите [systemd.timer(5)](http://jlk.fjfi.cvut.cz/arch/manpages/man/systemd.timer.5). Синтаксис аргументов для событий календаря и промежутка времени смотрите в [systemd.time(7)](http://jlk.fjfi.cvut.cz/arch/manpages/man/systemd.time.7).
...
Смотрите [systemd-run(1)](http://jlk.fjfi.cvut.cz/arch/manpages/man/systemd-run.1) для получения дополнительной информации и примеров.
...
*   Каждая задача может быть настроена для работы в определенной среде (смотрите [systemd.exec(5)](http://jlk.fjfi.cvut.cz/arch/manpages/man/systemd.exec.5)).
