...
**Обратите внимание:** Использование опции `-n` в команде `pacman -R` позволяет полностью удалить *все* такие файлы, относящиеся к пакету. В этом случае файлов *.pacsave* создано не будет
...
**Обратите внимание:** Поскольку файлы *.pacorig* создаются, как правило, при особых обстоятельствах, не существует универсального способа их обработки. Вам может помочь просмотр [новостей Arch](http://www.archlinux.org.ru/news/), в которых могут содержаться инструкции, если в обновлении ожидаются подобные проблемы
...
*   **pacnews-git** — Простой скрипт, нацеленный на поиск всех файлов *.pacnew* и их последующее редактирование при помощи [vimdiff](/index.php/Vim#Merging_files_.28vimdiff.29 "Vim")
