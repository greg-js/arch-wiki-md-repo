...
NetworkManager не учитывает настройки, размещенные в `/etc/sysctl.d/40-ipv6.conf`. Вы можете убедиться в этом, выполнив `$ ip -6 addr show _имя_интерфейса_` после перезагрузки: вы увидите, что кроме обычного адреса временный адрес `scope global **temporary**` не отображается.
...
Если вы добавите опцию `-I _your-global-ipv6_` в пинг-запрос, локальные хосты ответят с использованием локальных же адресов. При этом вы можете не указывать сетевой интерфейс:
...
Например, _dhcpcd_ будет продолжать пытаться запрашивать настройки сети у маршрутизаторов IPv6 (т. н. _Router solicitation_). Чтобы отключить это, добавьте следующие строки в `/etc/dhcpcd.conf`, как указано в [man-странице](/index.php/Man_page_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Man page (Русский)") `dhcpcd.conf (5)`:
...
Чтобы отключить IPv6 в NetworkManager, вызовите контекстное меню значка статуса сети и перейдите в _Edit Connections > Wired > Network name > Edit > IPv6 Settings > Method_, затем выберите _Method_ как _Ignore/Disabled_.
