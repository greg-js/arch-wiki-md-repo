**Состояние перевода:** На этой странице представлен перевод статьи [USB flash installation media](/index.php/USB_flash_installation_media "USB flash installation media"). Дата последней синхронизации: 17 марта 2019\. Вы можете [помочь](/index.php/ArchWiki_Translation_Team_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "ArchWiki Translation Team (Русский)") синхронизировать перевод, если в английской версии произошли [изменения](https://wiki.archlinux.org/index.php?title=USB_flash_installation_media&diff=0&oldid=568956).
...
**Примечание:** Альтернативно, замените букву устройства на `\\.\PhysicalDrive*X*`, где `*X*` - физический номер устройства (начинается с 0). Например: `# dd if=ISOs\archlinux-*версия*-x86_64.iso of=\\.\PhysicalDrive1 bs=4M` 
...
```
# dd if=путь/до/arch.iso of=/dev/**r**diskX bs=1m
...
*   Если появляется сообщение об ошибке "устройство не появляется через 30 секунд" ("device did not show up after 30 seconds") из-за того, что `/dev/disk/by-label/ARCH_*YYYYMM*` не монтируется, попробуйте переименовать USB-носитель в `ARCH_*YYYYMM*` (например, `ARCH_201501`).
