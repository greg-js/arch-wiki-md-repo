...
*   [3 Настрока базы MySQL](#Настрока_базы_MySQL)
...
Пакет главного демана (Директора):
...
Пакет хранилиша архивных данных (можно устанавливать на другом сервере или даже на нескольких серверах):
...
Будет создана база `bareos`, а также пользователь `bareos` без пароля (желательно пароль позже установить).

Что бы bareos-dir начал работать с MySQL, нужно сделать настроки в конфигурационном файле в секции `Catalog`. По умолчанию файл распологается в /etc/bareos/bareos-dir.d/catalog и может иметь произвольное имя с расширением `.conf`.
...
Примеры конфигурационных файлов находятся в:

```
  /usr/share/bareos/config
