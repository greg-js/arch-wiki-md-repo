...
**[MPD](http://www.musicpd.org/)** (**m**usic **p**layer **d**aemon) - это музыкальный проигрыватель, имеющий клиент-серверную архитектуру. MPD управляет плейлистами и медиатекой, используя очень мало ресурсов. Для взаимодействия с ним вам нужен отдельный [клиент](#Клиенты).
...
*   [2 Настройка MPD](#Настройка_MPD)
    *   [2.1 Глобальные настройки](#Глобальные_настройки)
        *   [2.1.1 Директория с музыкой](#Директория_с_музыкой)
        *   [2.1.2 Запуск MPD](#Запуск_MPD)
            *   [2.1.2.1 Сокет активация](#Сокет_активация)
    ...
    *   [2.2 Local configuration (per user)](#Local_configuration_(per_user))
        *   [2.2.1 Автозапуск при входе на tty](#Автозапуск_при_входе_на_tty)
...
*   [3 Клиенты](#Клиенты)
    *   [3.1 Консольные](#Консольные)
    *   [3.2 Графические](#Графические)
    *   [3.3 Web](#Web)
...
**Важно:** Пользователи PulseAudio с глобальными настройками mpd должны использовать [обходной путь](/index.php/Music_Player_Daemon/Tips_and_tricks#Local_(with_separate_mpd_user) "Music Player Daemon/Tips and tricks") для запуска демона!
...
MPD можно контролировать через `mpd.service` [используя systemd](/index.php/Systemd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Использование_юнитов "Systemd (Русский)"). Первый запуск может занять некоторое время, пока MPD будет сканировать вашу папку с музыкой.
...
Чтобы запустить MPD при входе, добавьте следующее в `~/.profile` (или другой [автозапуск файла](/index.php/Autostarting_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Коммандные_оболочки "Autostarting (Русский)")):
...
If you do not use a DE, place the line from [#Autostart on tty login](#Autostart_on_tty_login) in your [файле автозапуска](/index.php/Autostarting_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Графический "Autostarting (Русский)").
