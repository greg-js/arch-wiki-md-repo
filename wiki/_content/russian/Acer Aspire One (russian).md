...
*   [1 Прежде чем начать установку](#Прежде_чем_начать_установку)
    *   [1.1 SSD](#SSD)
*   [2 Минимальная установка для работы в текстовом режиме](#Минимальная_установка_для_работы_в_текстовом_режиме)
    *   [2.1 Подготовка загрузочной флешки](#Подготовка_загрузочной_флешки)
    *   [2.2 Установка](#Установка)
    *   [2.3 Первый старт. Подключение к Internet](#Первый_старт._Подключение_к_Internet)
    *   [2.4 Подключение SDHC карты в левый кардридер](#Подключение_SDHC_карты_в_левый_кардридер)
    *   [2.5 Настройка pacman и yaourt](#Настройка_pacman_и_yaourt)
    *   [2.6 Создание пользователя](#Создание_пользователя)
    *   [2.7 Wi-Fi](#Wi-Fi)
        *   [2.7.1 Поднятие беспроводного интерфейса](#Поднятие_беспроводного_интерфейса)
        *   [2.7.2 Настройка WPA](#Настройка_WPA)
        *   [2.7.3 Скрипт для управления подключением к сети](#Скрипт_для_управления_подключением_к_сети)
    *   [2.8 Настройка аудио](#Настройка_аудио)
    *   [2.9 Настройка графической системы](#Настройка_графической_системы)
        *   [2.9.1 Установка фреймбуфера](#Установка_фреймбуфера)
    *   [2.10 Ещё немного про оптимизацию](#Ещё_немного_про_оптимизацию)
        *   [2.10.1 Выключение кнопкой](#Выключение_кнопкой)
    *   [2.11 Программы для работы в текстовом режиме](#Программы_для_работы_в_текстовом_режиме)
        *   [2.11.1 Установка универсального проигрывателя](#Установка_универсального_проигрывателя)
*   [3 Установка графической оболочки](#Установка_графической_оболочки)
    *   [3.1 GNOME](#GNOME)
        *   [3.1.1 Установка GNOME](#Установка_GNOME)
        *   [3.1.2 Настройка Wi-Fi](#Настройка_Wi-Fi)
            *   [3.1.2.1 Настройка VPN](#Настройка_VPN)
            *   [3.1.2.2 Использование коммуникатора с WM для выхода в Internet](#Использование_коммуникатора_с_WM_для_выхода_в_Internet)
            *   [3.1.2.3 Системный монитор](#Системный_монитор)
*   [4](#)
    *   [4.1 Улучшение производительности графической системы](#Улучшение_производительности_графической_системы)
*   [5](#_2)
*   [6 Дополнительные функциональные клавиши](#Дополнительные_функциональные_клавиши)
*   [7 Выключение кнопкой](#Выключение_кнопкой_2)
*   [8 Различные приятные мелочи](#Различные_приятные_мелочи)
    *   [8.1 Подключение телефона и выход в интернет](#Подключение_телефона_и_выход_в_интернет)
        *   [8.1.1 Через USB](#Через_USB)
        *   [8.1.2 Через Bluetooth](#Через_Bluetooth)
*   [9 Установка нового ядра](#Установка_нового_ядра)
*   [10 Примеры конфигураций](#Примеры_конфигураций)
    *   [10.1 /etc/fstab](#/etc/fstab)
    *   [10.2 /etc/rc.local](#/etc/rc.local)
    *   [10.3 fdisk -l](#fdisk_-l)
    *   [10.4 /etc/X11/xorg.conf](#/etc/X11/xorg.conf)
    *   [10.5 Original Linpus Xorg.conf](#Original_Linpus_Xorg.conf)
    *   [10.6 Lines from rc.conf (kernel >=2.6.27)](#Lines_from_rc.conf_(kernel_>=2.6.27))
    *   [10.7 /etc/acerfand.conf](#/etc/acerfand.conf)
    *   [10.8 /etc/modprobe.d/sound](#/etc/modprobe.d/sound)
*   [11 Программое обеспечение](#Программое_обеспечение)
    *   [11.1 Офисные программы](#Офисные_программы)
    *   [11.2 Системные программы](#Системные_программы)
    *   [11.3 Игры и программы для общения](#Игры_и_программы_для_общения)
*   [12 Черновик](#Черновик)
    *   [12.1 Настройка внешнего порта VGA](#Настройка_внешнего_порта_VGA)
    *   [12.2 Обновление ядра](#Обновление_ядра)
*   [13 Советы](#Советы)
    *   [13.1 Настройка Internet Connection Sharing на сетевую плату](#Настройка_Internet_Connection_Sharing_на_сетевую_плату)
    *   [13.2 Настройка Internet Connection Sharing на WiFi](#Настройка_Internet_Connection_Sharing_на_WiFi)
    *   [13.3 Не загружается после установки](#Не_загружается_после_установки)
*   [14 Автоматизация](#Автоматизация)
*   [15 Установка настроенного ArchLinux из образа SSD](#Установка_настроенного_ArchLinux_из_образа_SSD)
*   [16 От автора](#От_автора)
*   [17 Полезные ссылки](#Полезные_ссылки)

## Прежде чем начать установку

Ознакомьтесь со статьёй [Руководство по установке](/index.php/%D0%A0%D1%83%D0%BA%D0%BE%D0%B2%D0%BE%D0%B4%D1%81%D1%82%D0%B2%D0%BE_%D0%BF%D0%BE_%D1%83%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%BA%D0%B5 "Руководство по установке").
...
Перенастраиваем [ISC](#Настройка_Internet_Connection_Sharing_на_WiFi) на десктопном компьютере.
...
Читаем прежде [здесь](/index.php/GNOME_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Установка "GNOME (Русский)")
...
Внешний порт VGA работает без дополнительных модификаций конфига, если внешний монитор подключен до загрузки ArchLinux. Если подключение случилось позже, то порт VGA активизируется настройками в xrandr. Смотреть секцию [Дополнительные функциональные клавиши](#Дополнительные_функциональные_клавиши).
