...
*   [1 Создание класса для нескольких принтеров](#Создание_класса_для_нескольких_принтеров)
*   [2 Внутри систем GNU/Linux](#Внутри_систем_GNU/Linux)
    *   [2.1 Использование веб-интерфейса](#Использование_веб-интерфейса)
    *   [2.2 Ручная настройка](#Ручная_настройка)
    *   [2.3 Включение обнаружения](#Включение_обнаружения)
*   [3 Между GNU/Linux и Windows](#Между_GNU/Linux_и_Windows)
    *   [3.1 Сервер Linux - клиент Windows](#Сервер_Linux_-_клиент_Windows)
        *   [3.1.1 Общий доступ с помощью Bonjour](#Общий_доступ_с_помощью_Bonjour)
        *   [3.1.2 Общий доступ с помощью IPP](#Общий_доступ_с_помощью_IPP)
        *   [3.1.3 Общий доступ с помощью Samba](#Общий_доступ_с_помощью_Samba)
    *   [3.2 Сервер Windows - клиент Linux](#Сервер_Windows_-_клиент_Linux)
        *   [3.2.1 Общий доступ с помощью LPD](#Общий_доступ_с_помощью_LPD)
        *   [3.2.2 Общий доступ с помощью IPP](#Общий_доступ_с_помощью_IPP_2)
        *   [3.2.3 Общий доступ с помощью Samba](#Общий_доступ_с_помощью_Samba_2)
            *   [3.2.3.1 Настройка через веб-интерфейс](#Настройка_через_веб-интерфейс)
            *   [3.2.3.2 Ручная настройка](#Ручная_настройка_2)
            *   [3.2.3.3 Поиск URI для серверов печати Windows](#Поиск_URI_для_серверов_печати_Windows)
*   [4 Удаленное управление](#Удаленное_управление)
    *   [4.1 Kerberos](#Kerberos)
*   [5 Решение проблем](#Решение_проблем)
    *   [5.1 Не удается отправить на печать из приложений GTK](#Не_удается_отправить_на_печать_из_приложений_GTK)
    *   [5.2 Ошибки разрешений в Windows](#Ошибки_разрешений_в_Windows)
*   [6 Другие операционные системы](#Другие_операционные_системы)
...
Для более детальной настройки вы можете отредактировать файл `/etc/cups/cupsd.conf`, нажав кнопку "Редактировать конфигурационный файл". Для получения дополнительной информации смотрите [#Ручная настройка](#Ручная_настройка).
...
Общий доступ для клиентов Windows может быть достигнут, используя [#Общий доступ с помощью Bonjour](#Общий_доступ_с_помощью_Bonjour), [#Общий доступ с помощью IPP](#Общий_доступ_с_помощью_IPP) или [#Общий доступ с помощью Samba](#Общий_доступ_с_помощью_Samba).
...
[Службы печати Bonjour](https://support.apple.com/kb/dl999) позволяет клиентам Windows легко подключаться к серверам печати Unix с [включенным обнаружением](#Включение_обнаружения).
...
Сначала, настройте сервер как описано в разделе [#Внутри систем GNU/Linux](#Внутри_систем_GNU/Linux).
...
Однажды сервер настроенный, как описано в [#Внутри систем GNU/Linux](#Внутри_систем_GNU/Linux), может также быть настроенным так, чтобы им можно было управлять удаленно. Добавьте разрешенные хосты в блок `<Location /admin>` в `/etc/cups/cupsd.conf`, используя тот же синтаксис, что и в [#Ручная настройка](#Ручная_настройка). Обратите внимание, что доступны три уровня доступа:
