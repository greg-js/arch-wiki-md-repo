...
*   [2 Управление беспроводными соединениями](#Управление_беспроводными_соединениями)
    *   [2.1 Ручная настройка](#Ручная_настройка)
        *   [2.1.1 Получение некоторой полезной информации](#Получение_некоторой_полезной_информации)
        *   [2.1.2 Активация интерфейса](#Активация_интерфейса)
        *   [2.1.3 Поиск точки доступа](#Поиск_точки_доступа)
        *   [2.1.4 Режим работы](#Режим_работы)
        *   [2.1.5 Привязка](#Привязка)
        *   [2.1.6 Получение IP-адреса](#Получение_IP-адреса)
        *   [2.1.7 Собственные стартовые скрипты/службы](#Собственные_стартовые_скрипты/службы)
            *   [2.1.7.1 Ручное беспроводное подключение при загрузке при помощи systemd и dhcpcd](#Ручное_беспроводное_подключение_при_загрузке_при_помощи_systemd_и_dhcpcd)
            *   [2.1.7.2 Systemd с wpa_supplicant и статическим IP](#Systemd_с_wpa_supplicant_и_статическим_IP)
...
*   [3 Решение проблем](#Решение_проблем)
    *   [3.1 Предостережения Rfkill](#Предостережения_Rfkill)
    *   [3.2 Уважение управляющего домена](#Уважение_управляющего_домена)
    *   [3.3 Просмотр логов](#Просмотр_логов)
    *   [3.4 Энергосбережение](#Энергосбережение)
    *   [3.5 Не удалось получить IP адрес](#Не_удалось_получить_IP_адрес)
    *   [3.6 Connection always times out](#Connection_always_times_out)
        *   [3.6.1 Lowering the rate](#Lowering_the_rate)
        *   [3.6.2 Понижение txpower](#Понижение_txpower)
        *   [3.6.3 Setting rts and fragmentation thresholds](#Setting_rts_and_fragmentation_thresholds)
    *   [3.7 Внезапные отключения](#Внезапные_отключения)
        *   [3.7.1 Причина #1](#Причина_#1)
        *   [3.7.2 Причина #2](#Причина_#2)
        *   [3.7.3 Причина #3](#Причина_#3)
*   [4 Решение проблем с драйверами и прошивками](#Решение_проблем_с_драйверами_и_прошивками)
    *   [4.1 wlan-ng](#wlan-ng)
    *   [4.2 rt2x00](#rt2x00)
    *   [4.3 RT2500](#RT2500)
    *   [4.4 RT61](#RT61)
    ...
    *   [4.12 ipw3945 (Альтернативный метод)](#ipw3945_(Альтернативный_метод))
    *   [4.13 orinoco](#orinoco)
    *   [4.14 ndiswrapper](#ndiswrapper)
    *   [4.15 prism54](#prism54)
    *   [4.16 ACX100/111](#ACX100/111)
    *   [4.17 BCM43XX](#BCM43XX)
    *   [4.18 b43](#b43)
    *   [4.19 rtl8187](#rtl8187)
    *   [4.20 zd1211rw](#zd1211rw)
    *   [4.21 Тестирование установки](#Тестирование_установки)
...
Некоторые беспроводные чипсеты в дополнение к нужному драйверу также требуют прошивку. Многие образы прошивок присутствуют в пакете [linux-firmware](https://www.archlinux.org/packages/?name=linux-firmware) и установлены по умолчанию, однако, проприетарные модули прошивок не включены и должны быть установлены отдельно. Это описано в разделе [#Установка драйвера/прошивки](#Установка_драйвера/прошивки).
...
**Совет:** Это не обязательно, но лучше сперва установить пользовательские инструменты, описанные в разделе [#Ручная настройка](#Ручная_настройка), особенно если ожидается возникновение какой-либо проблемы
...
Также проверьте вывод команды `ip link`, чтобы убедиться, что сетевой интерфейс ([обычно](/index.php/Network_configuration_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Имена_устройств "Network configuration (Русский)") название начинается с буквы "w", например, `wlp2s1`) был создан. Затем поднимите интерфейс командой `ip link set *интерфейс* up`. Например, если интерфейс называется `wlan0`:
...
(/=alternatives) |
| [Управляется вручную](#Ручная_настройка),
...
| [Управляется автоматически](#Автоматическая_настройка),
...
**Примечание:** Если вы получаете ошибки вида `RTNETLINK answers: Operation not possible due to RF-kill`, убедитесь, что аппаратный переключатель находится в положении *on*. Для получения дополнительной информации смотрите раздел [#Предостережения Rfkill](#Предостережения_Rfkill)
...
**Совет:** В зависимости от вашего местоположения, вам может понадобиться установить правильное имя [управляющего домена](#Уважение_управляющего_домена) чтобы надлежащим образом увидеть все доступные сети.
...
*   **WPA/WPA2**
    В зависимости от того, что вы получили в [#Поиск точки доступа](#Поиск_точки_доступа), выполните эту команду: `# wpa_supplicant -i wlan0 -c <(wpa_passphrase *ваш_SSID* *ваш_ключ*)` 
...
**Примечание:** Для просмотра дополнительных примеров обратитесь к разделу [Настройка сети#Настройка IP-адреса](/index.php/%D0%9D%D0%B0%D1%81%D1%82%D1%80%D0%BE%D0%B9%D0%BA%D0%B0_%D1%81%D0%B5%D1%82%D0%B8#Настройка_IP-адреса "Настройка сети"). Информация, представленная здесь, идентична
...
Запустите и/или включите юнит, как это описано в разделе [systemd (Русский)#Использование юнитов](/index.php/Systemd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Использование_юнитов "Systemd (Русский)"), не забыв при этом указать название интерфейса:
