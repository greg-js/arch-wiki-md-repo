...
**Примечание:** Вы должны запускать команду chroot c правами суперпользователя, иначе вы не сможете подключиться. Обратитесь к страницам man для получения дополнительной информации по `Match, ChrootDirectory` и `ForceCommand`.
...
**Примечание:** Запомните, что автоматическое монтирование выполняется с правами суперпользователя, поэтому вы не можете использовать `.ssh/config` вашего пользователя.
...
Для начала, прочитайте следующую страницу вики [SSH Checklist](/index.php/Secure_Shell#Checklist "Secure Shell"). Проблемы, которые следует проверить:
