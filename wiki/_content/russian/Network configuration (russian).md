...
*   [Список приложений/Интернет#Управление подключениями](/index.php/%D0%A1%D0%BF%D0%B8%D1%81%D0%BE%D0%BA_%D0%BF%D1%80%D0%B8%D0%BB%D0%BE%D0%B6%D0%B5%D0%BD%D0%B8%D0%B9/%D0%98%D0%BD%D1%82%D0%B5%D1%80%D0%BD%D0%B5%D1%82#Управление_подключениями "Список приложений/Интернет")
...
*   [2 Установка имени узла](#Установка_имени_узла)
*   [3 Драйвер устройства](#Драйвер_устройства)
    *   [3.1 Проверка состояния](#Проверка_состояния)
    *   [3.2 Загрузка модуля](#Загрузка_модуля)
*   [4 Сетевые интерфейсы](#Сетевые_интерфейсы)
    *   [4.1 Имена устройств](#Имена_устройств)
        *   [4.1.1 Получение текущих имен устройств](#Получение_текущих_имен_устройств)
        *   [4.1.2 Изменение имени устройства](#Изменение_имени_устройства)
    *   [4.2 Установка MTU и длины очереди для устройства](#Установка_MTU_и_длины_очереди_для_устройства)
    *   [4.3 Включение и отключение сетевых интерфейсов](#Включение_и_отключение_сетевых_интерфейсов)
*   [5 Настройка IP-адреса](#Настройка_IP-адреса)
    *   [5.1 Динамический IP-адрес](#Динамический_IP-адрес)
        *   [5.1.1 systemd-networkd](#systemd-networkd)
        *   [5.1.2 dhcpcd](#dhcpcd)
    *   [5.2 Статический IP-адрес](#Статический_IP-адрес)
        *   [5.2.1 Ручное распределение](#Ручное_распределение)
        *   [5.2.2 Постоянная конфигурация при загрузке с использованием systemd](#Постоянная_конфигурация_при_загрузке_с_использованием_systemd)
        *   [5.2.3 Вычисление адресов](#Вычисление_адресов)
*   [6 Загрузка конфигурации](#Загрузка_конфигурации)
*   [7 Дополнительные настройки](#Дополнительные_настройки)
    *   [7.1 ifplugd для ноутбуков](#ifplugd_для_ноутбуков)
    *   [7.2 Объединение сетевых интерфейсов (bonding) или LAG](#Объединение_сетевых_интерфейсов_(bonding)_или_LAG)
    *   [7.3 Создание алиасов для IP-адресов](#Создание_алиасов_для_IP-адресов)
        *   [7.3.1 Пример](#Пример)
    *   [7.4 Изменение MAC/аппаратного адреса](#Изменение_MAC/аппаратного_адреса)
    *   [7.5 Раздача интернета](#Раздача_интернета)
    *   [7.6 Настройка маршрутизатора](#Настройка_маршрутизатора)
    *   [7.7 Разрешение имени узла (hostname resolution) локальной сети](#Разрешение_имени_узла_(hostname_resolution)_локальной_сети)
    *   [7.8 "Неразборчивый" режим](#"Неразборчивый"_режим)
*   [8 Решение проблем](#Решение_проблем)
    *   [8.1 Смена компьютера при использовании кабельного модема](#Смена_компьютера_при_использовании_кабельного_модема)
    *   [8.2 Проблема масштабирования TCP window](#Проблема_масштабирования_TCP_window)
        *   [8.2.1 Диагностика](#Диагностика)
        *   [8.2.2 Способы решения проблемы](#Способы_решения_проблемы)
            *   [8.2.2.1 Плохой](#Плохой)
            *   [8.2.2.2 Хороший](#Хороший)
            *   [8.2.2.3 Лучший](#Лучший)
        *   [8.2.3 Дополнительная информация](#Дополнительная_информация)
    *   [8.3 Realtek: нет соединения / проблема WOL](#Realtek:_нет_соединения_/_проблема_WOL)
        *   [8.3.1 Способ 1: включение сетевой платы непосредственно в Linux](#Способ_1:_включение_сетевой_платы_непосредственно_в_Linux)
        *   [8.3.2 Способ 2: откат/замена драйвера Windows](#Способ_2:_откат/замена_драйвера_Windows)
        *   [8.3.3 Способ 3: включение WOL в драйвере Windows](#Способ_3:_включение_WOL_в_драйвере_Windows)
        *   [8.3.4 Способ 4: более новый драйвер Realtek для Linux](#Способ_4:_более_новый_драйвер_Realtek_для_Linux)
        *   [8.3.5 Способ 5: включение *LAN Boot ROM* в BIOS/CMOS](#Способ_5:_включение_LAN_Boot_ROM_в_BIOS/CMOS)
    *   [8.4 Для чипсетов Atheros отсутствует интерфейс](#Для_чипсетов_Atheros_отсутствует_интерфейс)
...
Найдите в интернете необходимый для вашего чипсета модуль/драйвер. Вот некоторые общие модули: `8139too` для плат с чипсетом Realtek, `sis900` для плат с чипсетом SiS. Когда вы узнаете, какой модуль необходимо использовать, попробуйте [загрузить его вручную](/index.php/Kernel_modules_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Загрузка_модуля_вручную "Kernel modules (Русский)"). Если вы увидите сообщение об ошибке, говорящее, что модуль не найден (the module was not found), возможно, драйвер не включен в состав ядра Arch. Вы можете осуществить поиск имени модуля в [AUR](/index.php/Arch_User_Repository_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "Arch User Repository (Русский)").

Если udev не определяет и не загружает нужный модуль автоматически во время старта системы, обратитесь к разделу [Модули ядра#Загрузка](/index.php/%D0%9C%D0%BE%D0%B4%D1%83%D0%BB%D0%B8_%D1%8F%D0%B4%D1%80%D0%B0#Загрузка "Модули ядра").
...
**Примечание:** При изменении схемы именования интерфейсов не забудьте обновить все конфигурационные файлы, имеющие отношение к сети, и пользовательские файлы юнитов systemd. Конкретно, если у вас включены [статические профили netctl](/index.php/Netctl_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Основной_метод "Netctl (Русский)"), выполните `netctl reenable *профиль*`, чтобы обновить сгенерированный файл службы
...
[Включите и запустите](/index.php/Systemd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Использование_юнитов "Systemd (Русский)") юнит `network@*интерфейс*`, указав ваше имя интерфейса.

**Совет:** Если вы предпочитаете обходиться без скриптов, добавьте необходимые команды в качестве дополнения к строкам `ExecStart=` и `ExecStop=`. Смотрите [пример для беспроводной сети](/index.php/Wireless_network_configuration_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Systemd_с_wpa_supplicant_и_статическим_IP "Wireless network configuration (Русский)")
...
Смотрите раздел [netctl (Русский)#Объединение сетевых интерфейсов (бондинг)](/index.php/Netctl_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Объединение_сетевых_интерфейсов_(бондинг) "Netctl (Русский)").
...
Необходимым условием является [#Установка имени узла](#Установка_имени_узла), после чего разрешение имени узла работает в локальной системе самостоятельно:
