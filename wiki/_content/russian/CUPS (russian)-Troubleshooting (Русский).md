**Состояние перевода:** На этой странице представлен перевод статьи [CUPS/Troubleshooting](/index.php/CUPS/Troubleshooting "CUPS/Troubleshooting"). Дата последней синхронизации: 8 января 2019\. Вы можете [помочь](/index.php/ArchWiki_Translation_Team_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "ArchWiki Translation Team (Русский)") синхронизировать перевод, если в английской версии произошли [изменения](https://wiki.archlinux.org/index.php?title=CUPS/Troubleshooting&diff=0&oldid=562433).
...
*   [1 Введение](#Введение)
*   [2 Проблемы, возникающие в результате обновлений](#Проблемы,_возникающие_в_результате_обновлений)
    *   [2.1 CUPS останавливается](#CUPS_останавливается)
    *   [2.2 Для всех заданий - "остановлено"](#Для_всех_заданий_-_"остановлено")
    *   [2.3 Для всех заданий - "Принтер не отвечает"](#Для_всех_заданий_-_"Принтер_не_отвечает")
    *   [2.4 Версия PPD не совместима с gutenprint](#Версия_PPD_не_совместима_с_gutenprint)
*   [3 Проблемы с сетью](#Проблемы_с_сетью)
...
*   [4 USB-принтеры](#USB-принтеры)
    ...
    *   [4.4 Плохие разрешения](#Плохие_разрешения)
*   [5 Проблемы с HP](#Проблемы_с_HP)
    *   [5.1 CUPS: "/usr/lib/cups/backend/hp failed"](#CUPS:_"/usr/lib/cups/backend/hp_failed")
    *   [5.2 CUPS: "Печать завершена", но принтер не печатает.](#CUPS:_"Печать_завершена",_но_принтер_не_печатает.)
    ...
    *   [5.4 CUPS: "Filter failed"](#CUPS:_"Filter_failed")
        *   [5.4.1 Отсутствует ghostscript](#Отсутствует_ghostscript)
        *   [5.4.2 Отсутствует foomatic-db](#Отсутствует_foomatic-db)
        *   [5.4.3 Avahi не включен](#Avahi_не_включен)
        *   [5.4.4 Устаревший плагин](#Устаревший_плагин)
        *   [5.4.5 Устаревшая конфигурация принтера](#Устаревшая_конфигурация_принтера)
    *   [5.5 CUPS: печатает только пустую страницу и страницу с сообщением об ошибке на HP LaserJet](#CUPS:_печатает_только_пустую_страницу_и_страницу_с_сообщением_об_ошибке_на_HP_LaserJet)
    *   [5.6 HPLIP 3.13: Плагин установлен, но HP Device Manager жалуется на его отсутствие](#HPLIP_3.13:_Плагин_установлен,_но_HP_Device_Manager_жалуется_на_его_отсутствие)
    *   [5.7 hp-toolbox: "Unable to communicate with device"](#hp-toolbox:_"Unable_to_communicate_with_device")
        *   [5.7.1 Разрешение проблемы](#Разрешение_проблемы)
        *   [5.7.2 Виртуальный CDROM у принтеров](#Виртуальный_CDROM_у_принтеров)
        *   [5.7.3 Сетевые принтеры](#Сетевые_принтеры)
    *   [5.8 hp-setup просит указать PPD-файл для обнаруженного принтера](#hp-setup_просит_указать_PPD-файл_для_обнаруженного_принтера)
    *   [5.9 hp-setup: "Qt/PyQt 4 initialization failed"](#hp-setup:_"Qt/PyQt_4_initialization_failed")
    *   [5.10 hp-setup: находит принтер автоматически, но сразу после этого сообщает "Unable to communicate with device" при печати тестовой страницы](#hp-setup:_находит_принтер_автоматически,_но_сразу_после_этого_сообщает_"Unable_to_communicate_with_device"_при_печати_тестовой_страницы)
    *   [5.11 hp-setup: "KeyError: 'family-class'"](#hp-setup:_"KeyError:_'family-class'")
*   [6 Другие](#Другие)
    *   [6.1 Принтер "приостановлен - "Paused"" или "Остановлен" cо статусом "Рендеринг завершен"](#Принтер_"приостановлен_-_"Paused""_или_"Остановлен"_cо_статусом_"Рендеринг_завершен")
        *   [6.1.1 Низкий уровень чернил](#Низкий_уровень_чернил)
    *   [6.2 Завершение печати из-за ошибок авторизации](#Завершение_печати_из-за_ошибок_авторизации)
    *   [6.3 Unknown supported format: application/postscript](#Unknown_supported_format:_application/postscript)
    *   [6.4 Ошибка задания для печати (Print-Job) client-error-document-format-not-supported](#Ошибка_задания_для_печати_(Print-Job)_client-error-document-format-not-supported)
    *   [6.5 Не удается получить список драйверов принтера](#Не_удается_получить_список_драйверов_принтера)
    *   [6.6 lp: Error - Scheduler Not Responding](#lp:_Error_-_Scheduler_Not_Responding)
    *   [6.7 Сообщение об ошибке: "Using invalid Host"](#Сообщение_об_ошибке:_"Using_invalid_Host")
    *   [6.8 Не удается отправить на печать из LibreOffice](#Не_удается_отправить_на_печать_из_LibreOffice)
    *   [6.9 Вывод принтера сдвинут](#Вывод_принтера_сдвинут)
    *   [6.10 Принтер не работает (приостановлен - "Paused") после ошибки](#Принтер_не_работает_(приостановлен_-_"Paused")_после_ошибки)
    *   [6.11 Samsung: URF ERROR - Incomplete Session by time out](#Samsung:_URF_ERROR_-_Incomplete_Session_by_time_out)
    *   [6.12 Brother: Принтер печатает несколько копий](#Brother:_Принтер_печатает_несколько_копий)
    *   [6.13 Обычный пользователь не может изменять настройки принтера или удалять определенные задания](#Обычный_пользователь_не_может_изменять_настройки_принтера_или_удалять_определенные_задания)
...
Каждая система может отличаться, поэтому обратитесь к вики-странице [udev (Русский)#Список атрибутов устройства](/index.php/Udev_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#Список_атрибутов_устройства "Udev (Русский)").
...
Иногда принтер печатает несколько копий документа (например, MFC-9330CDW напечатал 10 копий). Решение заключается в [обновлении прошивки принтера](/index.php/CUPS/%D0%9F%D1%80%D0%B8%D0%BD%D1%82%D0%B5%D1%80%D0%BE%D1%81%D0%BF%D0%B5%D1%86%D0%B8%D1%84%D0%B8%D1%87%D0%BD%D1%8B%D0%B5_%D0%BF%D1%80%D0%BE%D0%B1%D0%BB%D0%B5%D0%BC%D1%8B#Обновление_прошивки "CUPS/Принтероспецифичные проблемы").
