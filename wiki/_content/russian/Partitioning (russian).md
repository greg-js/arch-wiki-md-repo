...
**Совет:** Чтобы вывести информацию из таблицы разделов (определенного устройства), выполните `parted */dev/sda* print` или `fdisk -l */dev/sda*`, где `*/dev/sda*` имя блокового устройства.

Имеется два основных формата таблицы разделов. Они описаны далее в [#Главная загрузочная запись (MBR)](#Главная_загрузочная_запись_(MBR)) (MBR) и [#Таблица разделов GUID](#Таблица_разделов_GUID) (GPT). Ниже описаны их особенности и как выбирать между ними. Третьей, менее распространенной альтернативой является использование безраздельного диска, которая также обсуждается ниже.
...
**Совет:** При разбиении диска MBR постарайтесь оставить по крайней мере 33 512-байтовых секторов (16,5 КБ) свободного неразделенного пространства в конце диска на случай, если вы когда-нибудь решите [преобразовать его в GPT](/index.php/Gdisk#Преобразовать_между_MBR_и_GPT "Gdisk"). Место будет необходимо для резервного заголовка GPT.
