...
Или, если вы захотите использовать файл _startfluxbox_, замените код на следующий:
...
**Обратите внимание:** в файле `~/.xinitrc` команда `exec` может встречаться только один раз!

_Примечание_ : если fluxbox "падает" при запуске, то это может быть связано с локалью. Установив переменную LC_ALL в значение по умолчанию, "C", можно избежать "падений"[1](https://bbs.archlinux.org/viewtopic.php?t=25797).
