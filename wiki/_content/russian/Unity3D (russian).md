...
*   [2 Android Remote](#Android_Remote)
    *   [2.1 Подготовка компьютера](#Подготовка_компьютера)
        *   [2.1.1 Установка пакетов](#Установка_пакетов)
        *   [2.1.2 Настройка редактора](#Настройка_редактора)
    *   [2.2 Подготовка Android](#Подготовка_Android)
    *   [2.3 Проверка](#Проверка)
*   [3 Решение проблем](#Решение_проблем)
    *   [3.1 Unity падает при первом запуске до/при входа(е) в систему](#Unity_падает_при_первом_запуске_до/при_входа(е)_в_систему)
    *   [3.2 Unity падает при попытке загрузить проект](#Unity_падает_при_попытке_загрузить_проект)
    *   [3.3 Unity падает, если отсутствует ~/.config/user-dirs.dirs](#Unity_падает,_если_отсутствует_~/.config/user-dirs.dirs)
    *   [3.4 Незначительные зависания во время тестирования игры (NVIDIA)](#Незначительные_зависания_во_время_тестирования_игры_(NVIDIA))
