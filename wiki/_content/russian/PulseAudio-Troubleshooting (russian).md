...
*   [1 Звук](#Звук)
    *   [1.1 Автоматический бесшумный режим](#Автоматический_бесшумный_режим)
    *   [1.2 Аудиоустройство с отключенным звуком](#Аудиоустройство_с_отключенным_звуком)
    *   [1.3 Нет звука, в то время как Master включен](#Нет_звука,_в_то_время_как_Master_включен)
    *   [1.4 Приложение с отключенным звуком](#Приложение_с_отключенным_звуком)
    *   [1.5 Настройка громкости не работает должным образом](#Настройка_громкости_не_работает_должным_образом)
    *   [1.6 Громкость приложений меняется, когда регулируется Общая громкость](#Громкость_приложений_меняется,_когда_регулируется_Общая_громкость)
    *   [1.7 Звук становится каждый раз громче, как только запускается новое приложение](#Звук_становится_каждый_раз_громче,_как_только_запускается_новое_приложение)
    *   [1.8 На звуковой карте M-Audio Audiophile 2 496 звук только Mono](#На_звуковой_карте_M-Audio_Audiophile_2_496_звук_только_Mono)
    *   [1.9 Нет звука ниже определённого уровня](#Нет_звука_ниже_определённого_уровня)
    *   [1.10 Тихая громкость внутреннего микрофона](#Тихая_громкость_внутреннего_микрофона)
    *   [1.11 Клиенты изменяют основной выход звука (т. е. переход громкости к 100% после запущенного приложения)](#Клиенты_изменяют_основной_выход_звука_(т._е._переход_громкости_к_100%_после_запущенного_приложения))
    *   [1.12 Нет звука после возобновления из спящего режима](#Нет_звука_после_возобновления_из_спящего_режима)
    *   [1.13 Каналы ALSA отключены, когда наушники подсоединены/отсоединены неправильно](#Каналы_ALSA_отключены,_когда_наушники_подсоединены/отсоединены_неправильно)
*   [2 Микрофон](#Микрофон)
    *   [2.1 PulseAudio не обнаруживает микрофон](#PulseAudio_не_обнаруживает_микрофон)
    *   [2.2 PulseAudio использует неправильный микрофон](#PulseAudio_использует_неправильный_микрофон)
    *   [2.3 Нет микрофона на ThinkPad T400/T500/T420](#Нет_микрофона_на_ThinkPad_T400/T500/T420)
    *   [2.4 Нет микрофона на Acer Aspire One](#Нет_микрофона_на_Acer_Aspire_One)
    *   [2.5 Статический шум в записи микрофона](#Статический_шум_в_записи_микрофона)
        *   [2.5.1 Определение звуковых карт в системе (1/5)](#Определение_звуковых_карт_в_системе_(1/5))
        *   [2.5.2 Определить частоту дискретизации звуковой карты (2/5)](#Определить_частоту_дискретизации_звуковой_карты_(2/5))
        *   [2.5.3 Установка частоты дескритизации в настройках PulseAudio (3/5)](#Установка_частоты_дескритизации_в_настройках_PulseAudio_(3/5))
        *   [2.5.4 Перезапустите PulseAudio для применения новых настроек (4/5)](#Перезапустите_PulseAudio_для_применения_новых_настроек_(4/5))
        *   [2.5.5 Наконец, проверьте запись и её воспроизведение (5/5)](#Наконец,_проверьте_запись_и_её_воспроизведение_(5/5))
        *   [2.5.6 Иная возможная причина](#Иная_возможная_причина)
        *   [2.5.7 Если используется USB микрофон](#Если_используется_USB_микрофон)
    *   [2.6 Нет микрофона в Steam или Skype с enable-remixing = no](#Нет_микрофона_в_Steam_или_Skype_с_enable-remixing_=_no)
    *   [2.7 Искажение микрофона из-за автоматической регулировки](#Искажение_микрофона_из-за_автоматической_регулировки)
*   [3 Качество звука](#Качество_звука)
    *   [3.1 Включить эхо/шумоподавление](#Включить_эхо/шумоподавление)
    *   [3.2 Глюки, пропуски или потрескивания](#Глюки,_пропуски_или_потрескивания)
    *   [3.3 Статический шум при использовании наушников](#Статический_шум_при_использовании_наушников)
    *   [3.4 Определение номера фрагмента по умолчанию и размера буфера в PulseAudio](#Определение_номера_фрагмента_по_умолчанию_и_размера_буфера_в_PulseAudio)
        *   [3.4.1 Отключение планирования, основанного на таймере (0/4)](#Отключение_планирования,_основанного_на_таймере_(0/4))
        *   [3.4.2 Поиск параметров вашего аудиоустройства (1/4)](#Поиск_параметров_вашего_аудиоустройства_(1/4))
        *   [3.4.3 Вычисление размера вашего фрагмента в миллисекундах и количества фрагментов (2/4)](#Вычисление_размера_вашего_фрагмента_в_миллисекундах_и_количества_фрагментов_(2/4))
        *   [3.4.4 Редактирование конфигурационного файла PulseAudio (3/4)](#Редактирование_конфигурационного_файла_PulseAudio_(3/4))
        *   [3.4.5 Перезапуск демона PulseAudio (4/4)](#Перезапуск_демона_PulseAudio_(4/4))
    *   [3.5 Прерывистый звук с аналогового объемного звучания](#Прерывистый_звук_с_аналогового_объемного_звучания)
    *   [3.6 Тормозит звук](#Тормозит_звук)
    *   [3.7 Прерывистый/искажённый звук](#Прерывистый/искажённый_звук)
*   [4 Аппаратные средства и карты](#Аппаратные_средства_и_карты)
    *   [4.1 Спустя некоторое время, при выключенном мониторе отсутствует звук на выходе HDMI](#Спустя_некоторое_время,_при_выключенном_мониторе_отсутствует_звук_на_выходе_HDMI)
    *   [4.2 Отсутствие звука на выходе HDMI при использовании удаленного сервера](#Отсутствие_звука_на_выходе_HDMI_при_использовании_удаленного_сервера)
    *   [4.3 Нет карты](#Нет_карты)
    *   [4.4 Запуск приложения прерывает звук другого приложения](#Запуск_приложения_прерывает_звук_другого_приложения)
    *   [4.5 Единственное указанное устройство является "фиктивным выходом" или вновь подключенные карты не определяются](#Единственное_указанное_устройство_является_"фиктивным_выходом"_или_вновь_подключенные_карты_не_определяются)
    *   [4.6 Не возможно выбрать устройства HDMI 5/7.1](#Не_возможно_выбрать_устройства_HDMI_5/7.1)
    *   [4.7 Не удалось создать устройство входа: устройство приостановлено](#Не_удалось_создать_устройство_входа:_устройство_приостановлено)
    *   [4.8 Одновременный вывод на несколько звуковых карт / устройств](#Одновременный_вывод_на_несколько_звуковых_карт_/_устройств)
    *   [4.9 Одновременный вывод на несколько устройств вывода на одной звуковой карте не работает](#Одновременный_вывод_на_несколько_устройств_вывода_на_одной_звуковой_карте_не_работает)
    *   [4.10 Некоторые профили, такие как SPDIF не задействованы по умолчанию на карте](#Некоторые_профили,_такие_как_SPDIF_не_задействованы_по_умолчанию_на_карте)
*   [5 Bluetooth](#Bluetooth)
    *   [5.1 Отключение поддержки Bluetooth](#Отключение_поддержки_Bluetooth)
    *   [5.2 Проблемы воспроизведения гарнитуры Bluetooth](#Проблемы_воспроизведения_гарнитуры_Bluetooth)
    *   [5.3 Автоматическое переключение на Bluetooth или USB-гарнитуру](#Автоматическое_переключение_на_Bluetooth_или_USB-гарнитуру)
    *   [5.4 Устройство сопряжено, но не проигрывает звук](#Устройство_сопряжено,_но_не_проигрывает_звук)
*   [6 Приложения](#Приложения)
    *   [6.1 Содержащие Flash](#Содержащие_Flash)
    *   [6.2 Ошибка с правами доступа](#Ошибка_с_правами_доступа)
    *   [6.3 Audacity](#Audacity)
*   [7 Другие проблемы](#Другие_проблемы)
    *   [7.1 Плохой файл настроек](#Плохой_файл_настроек)
    *   [7.2 Невозможно обновить настройки звукового устройства в pavucontrol](#Невозможно_обновить_настройки_звукового_устройства_в_pavucontrol)
    *   [7.3 Не удалось создать устройство вывода: устройство вывода приостановлено](#Не_удалось_создать_устройство_вывода:_устройство_вывода_приостановлено)
    *   [7.4 Pulse переписывает настройки ALSA](#Pulse_переписывает_настройки_ALSA)
    *   [7.5 Предотвращение перезагрузки Pulse, после того как процесс был убит (kill)](#Предотвращение_перезагрузки_Pulse,_после_того_как_процесс_был_убит_(kill))
    *   [7.6 Не удаётся запустить Демон](#Не_удаётся_запустить_Демон)
        *   [7.6.1 Вывод статуса ошибок PulseAudio утилитами проверки](#Вывод_статуса_ошибок_PulseAudio_утилитами_проверки)
    *   [7.7 Демон уже запущен](#Демон_уже_запущен)
    *   [7.8 Сабвуфер перестает работать после окончания каждой песни](#Сабвуфер_перестает_работать_после_окончания_каждой_песни)
    *   [7.9 Невозможно выбрать настройку объемного звучания, кроме "Surround 4.0"](#Невозможно_выбрать_настройку_объемного_звучания,_кроме_"Surround_4.0")
    *   [7.10 Планировщик в реальном времени](#Планировщик_в_реальном_времени)
    *   [7.11 Ошибка pactl "неверный параметр" с отрицательным процентным аргументом](#Ошибка_pactl_"неверный_параметр"_с_отрицательным_процентным_аргументом)
    *   [7.12 Резервное устройство не определяется](#Резервное_устройство_не_определяется)
    *   [7.13 Отправка большого количества пакетов RTP/UDP](#Отправка_большого_количества_пакетов_RTP/UDP)
...
*   `analog_gain_control=1` - Аналоговая АРУ - 'Автоматическая Регулировка Усиления' осуществляется путем прямого изменения громкости - скорее всего приведет к [искажениям](#Искажение_микрофона_из-за_автоматической_регулировки).
...
Если вы используете [module-echo-cancel](#Включить_эхо.2Fшумоподавление), вам вероятно не понадобятся другие приложения для дополнительной постобработки звука.
...
Смотрите [#Резервное устройство не определяется](#Резервное_устройство_не_определяется).
...
Если звук Flash заикается, можете попробовать использовать Flash через ALSA напрямую. Подробно смотрите [ALSA/dmix без захвата аппаратного устройства](/index.php/PulseAudio_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#ALSA/dmix_без_захвата_аппаратного_устройства "PulseAudio (Русский)").
...
Смотрите также [#Определение номера фрагмента по умолчанию и размера буфера в PulseAudio](#Определение_номера_фрагмента_по_умолчанию_и_размера_буфера_в_PulseAudio).
