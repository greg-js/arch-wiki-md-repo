...
**Обратите внимание:** Проприетарные драйвера [NVIDIA](/index.php/NVIDIA "NVIDIA") и [catalyst](/index.php/Catalyst "Catalyst") так же поддерживают KMS, но так как они не используют встроенную реализацию ядра, то не обеспечивают работу драйвера FBdev для отображения консоли с высоким разрешением.
...
Первоначально, настройками видео карты занимался X сервер. В силу этого, было не просто достигнуть качества графики в tty консолях. Кроме того, каждый раз переключаясь с X в tty с помощью (`Ctrl+Alt+F1`), сервер должен был передать контроль над видеокартой ядру, что было медленным и вызывало мерцания. Особенно "болезненный" процесс происходил когда контроль возвращался обратно X серверу (`Ctrl+Alt+F7`).

Сейчас с Kernel Mode Setting (KMS), ядру доступна установка режимов видео карты. Это делает графику качественной во время загрузки, в виртуальной консоли (tty) и позволяет быстрее переключаться в X.

## Установка

Сперва, запомните, что для _любых_ методов которые Вы используете, необходимо _всегда_ отключать:
...
**Обратите внимание:** Пользователям Intel может понадобиться добавление `intel_agp` перед `i915` для подавления ошибок ACPI.
...
Пересоберите образ ядра (смотрите статью о [mkinitcpio](/index.php/Mkinitcpio "Mkinitcpio") для большей информации):
...
**Обратите внимание:** Некоторые драйвера [Xorg](/index.php/Xorg "Xorg") не работают с отключенным KMS. Читайте wiki для Вашего оборудования для подробностей.