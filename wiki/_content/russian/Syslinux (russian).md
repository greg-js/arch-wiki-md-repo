...
*   [1 Системы с BIOS](#Системы_с_BIOS)
    *   [1.1 Обзор процесса загрузки](#Обзор_процесса_загрузки)
    *   [1.2 Установка](#Установка)
        *   [1.2.1 Автоматическая установка](#Автоматическая_установка)
        *   [1.2.2 Ручная установка](#Ручная_установка)
            *   [1.2.2.1 Таблица разделов MBR](#Таблица_разделов_MBR)
            *   [1.2.2.2 Таблица разделов GUID (GPT)](#Таблица_разделов_GUID_(GPT))
*   [2 Системы с UEFI](#Системы_с_UEFI)
    *   [2.1 Недостатки UEFI Syslinux](#Недостатки_UEFI_Syslinux)
    *   [2.2 Установка](#Установка_2)
*   [3 Настройка](#Настройка)
    *   [3.1 Примеры](#Примеры)
        *   [3.1.1 Приглашение командной строки](#Приглашение_командной_строки)
        *   [3.1.2 Текстовое меню загрузки](#Текстовое_меню_загрузки)
        *   [3.1.3 Графическое меню загрузки](#Графическое_меню_загрузки)
    *   [3.2 Параметры ядра](#Параметры_ядра)
    *   [3.3 Автозагрузка](#Автозагрузка)
    *   [3.4 Безопасность](#Безопасность)
    *   [3.5 Передача управления другому загрузчику (chainloading)](#Передача_управления_другому_загрузчику_(chainloading))
    *   [3.6 Chainloading для других систем Linux](#Chainloading_для_других_систем_Linux)
    *   [3.7 Использование memtest](#Использование_memtest)
    *   [3.8 HDT](#HDT)
    *   [3.9 Перезагрузка и выключение](#Перезагрузка_и_выключение)
    *   [3.10 Очистка экрана](#Очистка_экрана)
    *   [3.11 Раскладка клавиатуры](#Раскладка_клавиатуры)
    *   [3.12 Скрытие меню](#Скрытие_меню)
    *   [3.13 PXELINUX](#PXELINUX)
    *   [3.14 Загрузка файлов образа ISO9660 при помощи memdisk](#Загрузка_файлов_образа_ISO9660_при_помощи_memdisk)
*   [4 Решение проблем](#Решение_проблем)
    *   [4.1 Использование приглашения Syslinux](#Использование_приглашения_Syslinux)
    *   [4.2 Fsck не работает на корневом разделе](#Fsck_не_работает_на_корневом_разделе)
    ...
    *   [4.5 Windows загружается, игнорируя Syslinux](#Windows_загружается,_игнорируя_Syslinux)
    *   [4.6 После выбора пункта меню ничего не происходит](#После_выбора_пункта_меню_ничего_не_происходит)
    *   [4.7 Невозможно удалить ldlinux.sys](#Невозможно_удалить_ldlinux.sys)
    *   [4.8 Белый блок в верхнем левом углу при использовании vesamenu](#Белый_блок_в_верхнем_левом_углу_при_использовании_vesamenu)
    *   [4.9 Chainloading Windows не работает, когда она установлена на другом диске](#Chainloading_Windows_не_работает,_когда_она_установлена_на_другом_диске)
    *   [4.10 Чтение логов загрузчика](#Чтение_логов_загрузчика)
...
*   Если вы обновляете Syslinux с версии 5.xx (или старше) до версии 6.xx, пожалуйста, переустановите (а не обновите) Syslinux BIOS вручную (без использования установочного скрипта) один раз, следуя указаниям из раздела [#Ручная установка](#Ручная_установка). Установочный скрипт не способен корректно обновить Syslinux до версии 6.xx
...
3.  Создайте или отредактируйте файл `/boot/syslinux/syslinux.cfg`, следуя указаниям из раздела [#Настройка](#Настройка).
...
*   Создайте или отредактируйте файл `$esp/EFI/syslinux/syslinux.cfg`, следуя указаниям из раздела [#Настройка](#Настройка)
...
**Примечание:** Любые конфигурационные файлы, приведенные здесь, должны быть отредактированы, чтобы установить правильные параметры ядра. Смотрите раздел [#Параметры ядра](#Параметры_ядра)
...
Для получения дополнительных подробностей смотрите [документацию по Syslinux](http://git.kernel.org/?p=boot/syslinux/syslinux.git;a=blob;f=doc/menu.txt) или [Syslinux wiki](https://wiki.syslinux.org/wiki/index.php/Menu).
...
Если вы не хотите, чтобы выводилось меню Syslinux, используйте [#Приглашение командной строки](#Приглашение_командной_строки), при этом установив параметр `PROMPT` в значение `0`. Убедитесь, что в вашем `syslinux.cfg` указана опция `DEFAULT`.
...
Для получения дополнительных подробностей про chainloading смотрите [Syslinux wiki](https://wiki.syslinux.org/wiki/index.php/Comboot/chain.c32).
...
Syslinux поддерживает прямую загрузку из ISO-образов при помощи модуля [memdisk](https://wiki.syslinux.org/wiki/index.php/MEMDISK). Для просмотра примеров обратитесь к разделу [Использование Syslinux и memdisk](/index.php/Multiboot_USB_drive#Using_Syslinux_and_memdisk "Multiboot USB drive").
...
Замените код mbr тем, что есть на диске с windows (детали [выше](#Передача_управления_другому_загрузчику_(chainloading))), и добавьте `swap` в опции.
