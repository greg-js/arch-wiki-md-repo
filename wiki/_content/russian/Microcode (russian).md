...
Если Arch находится на [съемном носителе](/index.php/Installing_Arch_Linux_on_a_USB_key "Installing Arch Linux on a USB key"), вы должны установить микрокод для обоих производителей процессоров.
...
**Совет:** Для [Arch Linux на съемном носителе](/index.php/Installing_Arch_Linux_on_a_USB_key "Installing Arch Linux on a USB key") добавьте оба файла `initrd` в настройки загрузчика. Их порядок не имеет значения, если они оба указаны до реального образа initramfs.
