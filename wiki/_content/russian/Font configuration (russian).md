...
*   [1 Пути шрифтов](#Пути_шрифтов)
*   [2 Настройка Fontconfig](#Настройка_Fontconfig)
    *   [2.1 Предварительные установки](#Предварительные_установки)
    *   [2.2 Anti-aliasing (сглаживание)](#Anti-aliasing_(сглаживание))
    *   [2.3 Hinting (Хинтинг)](#Hinting_(Хинтинг))
        *   [2.3.1 Байт-код Переводчик (Byte-Code Interpreter, BCI)](#Байт-код_Переводчик_(Byte-Code_Interpreter,_BCI))
        *   [2.3.2 Autohinter (Авто Хинтинг)](#Autohinter_(Авто_Хинтинг))
        *   [2.3.3 Hintstyle (Стиль Хинтинга)](#Hintstyle_(Стиль_Хинтинга))
    *   [2.4 Субпиксельное отображение](#Субпиксельное_отображение)
        *   [2.4.1 LCD filter (ЖК-фильтр)](#LCD_filter_(ЖК-фильтр))
        *   [2.4.2 Расширенная спецификация ЖК-фильтра (LCD filter)](#Расширенная_спецификация_ЖК-фильтра_(LCD_filter))
    *   [2.5 Отключение авто хинтинга (auto-hinter) для жирных (bold) шрифтов](#Отключение_авто_хинтинга_(auto-hinter)_для_жирных_(bold)_шрифтов)
    *   [2.6 Заменить или установить шрифты по умолчанию](#Заменить_или_установить_шрифты_по_умолчанию)
    *   [2.7 Белый список и чёрный список шрифтов](#Белый_список_и_чёрный_список_шрифтов)
    *   [2.8 Отключение растровых шрифтов](#Отключение_растровых_шрифтов)
    *   [2.9 Отключить масштабирование растровых шрифтов](#Отключить_масштабирование_растровых_шрифтов)
    *   [2.10 Создать стили для жирных и курсивных «некомплектных» шрифтов](#Создать_стили_для_жирных_и_курсивных_«некомплектных»_шрифтов)
    *   [2.11 Изменение главного правила](#Изменение_главного_правила)
    *   [2.12 Запрос текущих настроек](#Запрос_текущих_настроек)
*   [3 Приложения без поддержки fontconfig](#Приложения_без_поддержки_fontconfig)
*   [4 Решение проблем](#Решение_проблем)
    *   [4.1 Искаженные шрифты](#Искаженные_шрифты)
    *   [4.2 Calibri, Cambria, Monaco, и т. д. не отображаются правильно](#Calibri,_Cambria,_Monaco,_и_т._д._не_отображаются_правильно)
    *   [4.3 Приложения переопределяют hinting](#Приложения_переопределяют_hinting)
    *   [4.4 Приложения не используют hinting из настроек DE](#Приложения_не_используют_hinting_из_настроек_DE)
    *   [4.5 Неправильный hinting в приложениях GTK на НЕ-Gnome системах](#Неправильный_hinting_в_приложениях_GTK_на_НЕ-Gnome_системах)
    *   [4.6 Проблема шрифтов в Сгенерированных файлах PDF](#Проблема_шрифтов_в_Сгенерированных_файлах_PDF)
...
**Примечание:** некоторые приложения, такие как [GNOME](/index.php/GNOME_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "GNOME (Русский)") могут [переопределить настройки сглаживания по умолчанию](#Решение_проблем).
...
**Примечание:** некоторые приложения, такие как [GNOME](/index.php/GNOME_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "GNOME (Русский)") могут [переопределить настройки хинтинга по умолчанию](#Решение_проблем).
...
Растровые шрифты иногда используются в качестве резервных шрифтов, заместо отсутствующих/недоступных шрифтов, которые могут делать текст пиксельным (со ступеньками), или слишком большим. Чтобы отключить такое поведение, используйте `70-no-bitmaps.conf` [#Предварительные установки](#Предварительные_установки).
