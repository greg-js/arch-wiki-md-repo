...
**Примечание:** Чтобы создать локальный репозиторий с пакетами из *core* (или других репозиториев) при отсутствии интернет-подключения, смотрите раздел [Pacman tips#Installing packages from a CD/DVD or USB stick](/index.php/Pacman_tips#Installing_packages_from_a_CD.2FDVD_or_USB_stick "Pacman tips")
...
Если вы включаете *testing*, также необходимо включить [community-testing](#community-testing).
...
Этот репозиторий содержит самую свежую версию окружения рабочего стола [GNOME](/index.php/GNOME "GNOME") до ее перевода в главный репозиторий *testing*.

Для включения, добавьте нижеприведенные строки в ваш файл `/etc/pacman.conf`. Репозиторий *gnome-unstable* должен быть первым в списке репозиториев (*т.е.*, выше соответствующей записи для репозитория *testing*).
...
Информацию о багах в процессе упаковки сообщайте на нашей [системе отслеживания ошибок](https://bugs.archlinux.org/), прочая информация должна направляться непосредсвенно разработчикам на [GNOME Bugzilla](https://bugzilla.gnome.org/).

#### kde-unstable

Этот репозиторий содержит самую свежую *бета-версию* или *версию-кандидат на выпуск* [KDE](/index.php/KDE "KDE") Plasma и Приложений KDE.

Для включения данного репозитория, добавьте нижеследующие строки в ваш файл `/etc/pacman.conf`. Репозиторий *kde-unstable* должен быть первым в списке репозиториев (*т.е.*, выше соответствующей записи для репозитория *testing*).
