...
В файле `desc` хранится описание пакета — список зависимостей, размер, хэш-сумма MD5 и т.д.
...
Теперь необходимо [переустановить](#Установка_отдельных_пакетов) испорченный пакет. При этом обязательно используйте ключ `--force`, поскольку вы лишь распаковали системные файлы, но *pacman* об этом не знает. *pacman* корректно заменит ваш файл разделяемой библиотеки на тот, что имеется в пакете.
