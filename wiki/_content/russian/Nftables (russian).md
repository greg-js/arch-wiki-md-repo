...
**Состояние перевода:** На этой странице представлен перевод статьи [nftables](/index.php/Nftables "Nftables"). Дата последней синхронизации: 5 декабря 2019\. Вы можете [помочь](/index.php/ArchWiki_Translation_Team_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9) "ArchWiki Translation Team (Русский)") синхронизировать перевод, если в английской версии произошли [изменения](https://wiki.archlinux.org/index.php?title=Nftables&diff=0&oldid=590976).

[nftables](http://netfilter.org/projects/nftables/) — проект netfilter, целью которого ставится замена существующего набора межсетевых экранов {ip,ip6,arp,eb}tables. Была разработана новая система фильтрации пакетов, добавлена пользовательская утилита ntf, а также создан слой совместимости с {ip,ip6}tables. nftables использует набор хуков, систему отслеживания соединений, систему очередей и подсистему логирования netfilter.
...
Описание остальных семейств можно найти в разделе `ADDRESS FAMILIES` руководства [nft(8)](https://jlk.fjfi.cvut.cz/arch/manpages/man/nft.8).
...
Для семейств адресов IPv4/IPv6/Inet `*хук*` может принимать значения `prerouting`, `input`, `forward`, `output` или `postrouting`. Описание других возможных хуков вы найдёте в руководстве [nft(8)](https://jlk.fjfi.cvut.cz/arch/manpages/man/nft.8).

Параметр `*приоритет*` должен быть целочисленным. Цепочки с более низкими значениями обрабатываются раньшеу. Значение приоритета может быть отрицательным [[4]](https://wiki.nftables.org/wiki-nftables/index.php/Configuring_chains#Base_chain_types).
...
Ниже приведён пример правила с маркером и команда для его удаления. Аргумент `--number` позволяет выводить IP-адреса в численном виде.
...
*   [What comes after "iptables"?](http://developers.redhat.com/blog/2016/10/28/what-comes-after-iptables-its-successor-of-course-nftables/) — nftables со временем заменит iptables