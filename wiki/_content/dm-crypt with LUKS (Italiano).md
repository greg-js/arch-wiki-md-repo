...
**Notes:** Questo articolo è in fase di traduzione. Seguite per ora le istruzioni della versione inglese. (Discuss in [Talk:ArchWiki Translation Team (Italiano)#Pagine Marcate come "out of date" e "Traslateme"](https://wiki.archlinux.org/index.php/Talk:ArchWiki_Translation_Team_(Italiano)#Pagine_Marcate_come_.22out_of_date.22_e_.22Traslateme.22))
...
**Reason:** A partire dal [media di installazione 2012.07.15](https://www.archlinux.org/news/install-media-20120715-released/), AIF (ovvero Arch Installation Framework) non è più incluso ma sono forniti gli [Arch Install Scripts](https://github.com/falconindy/arch-install-scripts) per facilitare il processo di installazione. Molti contenuti della pagina dovranno essere aggiornati. Molti collegamenti esterni sono molto vecchi e potrebbero confondere gli utenti che non sono a conoscenza dei principali cambiamenti nelle configurazioni di Arch Linux. Tenere questa informazione a mente nel leggere la pagina. (Discuss in [Talk:Dm-crypt with LUKS (Italiano)#](https://wiki.archlinux.org/index.php/Talk:Dm-crypt_with_LUKS_(Italiano)))
...
<table class="wikitable" style="margin:0 5em 0.5em 0.5em 0.5em;">

<tbody>

<tr>

<th scope="col" style="text-align:left">Available options</th>

<th scope="col" style="text-align:left">Cryptsetup defaults</th>
...
<th scope="row" style="text-align:right">--cipher, -c</th>

<td>`aes-cbc-essiv:sha256`</td>

<td>Use the AES-[cipher](/index.php/Disk_encryption#Ciphers_and_modes_of_operation "Disk encryption") with [CBC/ESSIV](http://en.wikipedia.org/wiki/Disk_encryption_theory#Cipher-block_chaining_.28CBC.29).</td>

<td>`aes-xts-plain64`</td>

<td>[XTS](http://en.wikipedia.org/wiki/Disk_encryption_theory#XEX-based_tweaked-codebook_mode_with_ciphertext_stealing_.28XTS.29). For volumes >2TiB use `aes-xts-plain64` (requires kernel >= 2.6.33).</td>

</tr>

<tr>

<th scope="row" style="text-align:right">--key-size, -s</th>

<td>`256`</td>

<td>The cipher is used with 256 bit key-size.</td>

<td>`512`</td>

<td>[XTS splits the supplied key](http://en.wikipedia.org/wiki/XEX-TCB-CTS#Issues_with_XTS) into fraternal twins. For an effective AES-256 the XTS key-size must be `512`.</td>

</tr>

<tr>

<th scope="row" style="text-align:right">--hash, -h</th>

<td>`sha1`</td>

<td>Hash algorithm used for [PBKDF2](/index.php/Disk_encryption#Keys.2C_keyfiles_and_passphrases "Disk encryption").</td>
...
<th scope="row" style="text-align:right">--iter-time, -i</th>

<td>`1000`</td>

<td>Number of milliseconds to spend with PBKDF2 passphrase processing.</td>

<td>`5000`</td>

<td>Using a hash stronger than sha1 results in less iterations if iter-time is not increased.</td>

</tr>

<tr>

<th scope="row" style="text-align:right">--use-random</th>

<td>`--use-**u**random`</td>

<td>/dev/u[random](/index.php/Random "Random") is used as randomness source for the (long-term) volume master key.</td>

<td>`--use-random`</td>

<td>Avoid generating an insecure master key if low on entropy. Will block if the entropy pool is used up.</td>

</tr>

<tr>

<th scope="row" style="text-align:right">--verify-passphrase, -y</th>

<td>Yes</td>

<td>Default only for luksFormat and luksAddKey.</td>

<td>-</td>

<td>No need to type for archlinux at the moment.</td>
...
**This article or section is a candidate for merging with [Persistent block device naming](/index.php/Persistent_block_device_naming "Persistent block device naming").**

**Notes:** Anything not specific to storing LUKS keyfiles should get merged there. (Discuss in [Talk:Dm-crypt with LUKS (Italiano)#](https://wiki.archlinux.org/index.php/Talk:Dm-crypt_with_LUKS_(Italiano)))
...
**Reason:** Grub-legacy is not available anymore and for [GRUB2](/index.php/GRUB2 "GRUB2") a 1-2MB gap between MBR and the beginning of other written data (e.g. first partition or LUKS-key) is needed for embedding GRUB2's `core.img`. Information regarding GPT and/or UEFI combinations are required. GRUB2 has changed parsing configuration. (Discuss in [Talk:Dm-crypt with LUKS (Italiano)#](https://wiki.archlinux.org/index.php/Talk:Dm-crypt_with_LUKS_(Italiano)))
...
**Reason:** Even if LVM is used it is bad practice to specify a false root device to the kernel. The rest is now already covered up above and maybe the whole paragraph should be a candidate for deletion? (Discuss in [Talk:Dm-crypt with LUKS (Italiano)#](https://wiki.archlinux.org/index.php/Talk:Dm-crypt_with_LUKS_(Italiano)))
...
**Reason:** Backlinking from [Syslinux](/index.php/Syslinux "Syslinux") should be done and more instruction for different setups instead of only duplicating stub content among 2 articles is required. (Discuss in [Talk:Dm-crypt with LUKS (Italiano)#](https://wiki.archlinux.org/index.php/Talk:Dm-crypt_with_LUKS_(Italiano)))
...
**Reason:** GRUB Legacy and LILO are dropped. (Discuss in [Talk:Dm-crypt with LUKS (Italiano)#](https://wiki.archlinux.org/index.php/Talk:Dm-crypt_with_LUKS_(Italiano)))
...
**This article or section is a candidate for merging with [#Wipe LUKS header](#Wipe_LUKS_header).**

**Notes:** Calculating the header size should get expanded there and linked by few simple sentences from here. (Discuss in [Talk:Dm-crypt with LUKS (Italiano)#](https://wiki.archlinux.org/index.php/Talk:Dm-crypt_with_LUKS_(Italiano)))
...
**Reason:** The Header size is now aligned to 1MB multiples for better block size alignment to hardware geometry. For a 1MB Header this is 2050 (or 2048?) Blocks of 512 Bytes. However Payload offset will be something like 4096 (4MB) or more. (Discuss in [Talk:Dm-crypt with LUKS (Italiano)#](https://wiki.archlinux.org/index.php/Talk:Dm-crypt_with_LUKS_(Italiano)))
...
**This article or section is a candidate for merging with [#LVM:_Logical_Volume_Manager](#LVM:_Logical_Volume_Manager).**

**Notes:** Device mapper stacking is already explained there. Duplicates should get merged one way (whereto?) and linked then. Maybe keeping theory there and practice here? (Discuss in [Talk:Dm-crypt with LUKS (Italiano)#](https://wiki.archlinux.org/index.php/Talk:Dm-crypt_with_LUKS_(Italiano)))
...
**Reason:** As of the [2012.07.15 installation media release](https://www.archlinux.org/news/install-media-20120715-released/) the AIF (Arch Installation Framework) and grub-legacy are dropped. These outdated instructions still give the backbones to understanding what to do. (Discuss in [Talk:Dm-crypt with LUKS (Italiano)#](https://wiki.archlinux.org/index.php/Talk:Dm-crypt_with_LUKS_(Italiano)))
...
**Reason:** The `lvm2` hook activates the (encrypted) root volume group long before sysvinit (or systemd) can run from there. Letting sysvinit later run a second LVM activation in addition serves no purpose. Read [LVM#Configure system](/index.php/LVM#Configure_system "LVM"). However this error is duplicated within the [#Encrypting a LVM setup](#Encrypting_a_LVM_setup) section. (Discuss in [Talk:Dm-crypt with LUKS (Italiano)#](https://wiki.archlinux.org/index.php/Talk:Dm-crypt_with_LUKS_(Italiano)))
...
The script with installation instructions is available here: [ftp://ftp.heise.de/pub/ct/listings/1203-146.zip](ftp://ftp.heise.de/pub/ct/listings/1203-146.zip) (Author: Juergen Schmidt, ju at heisec.de; License: GPLv2). There is also an AUR package: [chkboot](https://aur.archlinux.org/packages/chkboot/)<sup><small>AUR</small></sup>
...
**Reason:** Explained separately for each link beneath. (Discuss in [Talk:Dm-crypt with LUKS (Italiano)#](https://wiki.archlinux.org/index.php/Talk:Dm-crypt_with_LUKS_(Italiano)))
...
Retrieved from "[https://wiki.archlinux.org/index.php?title=Dm-crypt_with_LUKS_(Italiano)&oldid=415634](https://wiki.archlinux.org/index.php?title=Dm-crypt_with_LUKS_(Italiano)&oldid=415634)"
