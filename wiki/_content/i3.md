...
See [Configuring i3](http://i3wm.org/docs/userguide.html#configuring) for details. The rest of this article assumes the _i3_ configuration file to be in the folder `~/.config` (contrary to _i3-config-wizard_, which creates `~/.i3/config`).

### Configuration wizard and alternative keyboard layouts

When _i3_ is first started, it offers to run the configuration wizard _i3-config-wizard_. This tool creates `~/.i3/config` by rewriting a template configuration file in `/etc/i3/config.keycodes`. It makes two kinds of modifications to the default template:
...
Alternatively, _xautolock_ can lock the screen after awakening from sleep or hibernation with the `-lockaftersleep` option. The `-time` option locks the screen after a given time period.
...
See also [DPMS](/index.php/DPMS "DPMS") and [Pm-utils#Creating your own hooks](/index.php/Pm-utils#Creating_your_own_hooks "Pm-utils").
...
Retrieved from "[https://wiki.archlinux.org/index.php?title=I3&oldid=413043](https://wiki.archlinux.org/index.php?title=I3&oldid=413043)"
