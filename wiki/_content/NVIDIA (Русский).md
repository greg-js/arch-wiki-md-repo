...
*   Для очень новых моделей графических ускорителей может потребоваться установка (см. [install](/index.php/Install "Install")) пакета [nvidia-beta](https://aur.archlinux.org/packages/nvidia-beta/)<sup><small>AUR</small></sup>, т.к. стабильная версия драйвера может не поддерживать новые функции, добавленные в эти карты.
...
*   Для карт GeForce 5 FX series [NV30-NV36], установите пакет [nvidia-173xx-dkms](https://aur.archlinux.org/packages/nvidia-173xx-dkms/)<sup><small>AUR</small></sup>. Последняя поддерживаемая версия Xorg 1.15.
*   Для карт GeForce 2/3/4 MX/Ti series [NV11, NV17-NV28], установите пакет [nvidia-96xx-dkms](https://aur.archlinux.org/packages/nvidia-96xx-dkms/)<sup><small>AUR</small></sup>. Последняя поддерживаемая версия Xorg 1.12.
...
**Причина:** Данные команды устанавливают мониторы в режим _TwinView_? (обсуждение: [Talk:NVIDIA (Русский)#](https://wiki.archlinux.org/index.php/Talk:NVIDIA_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)))
...
Если контроль яркости не заработает после применения данной опции, попробуйте установить [nvidia-bl](https://aur.archlinux.org/packages/nvidia-bl/)<sup><small>AUR</small></sup> или [nvidiabl](https://aur.archlinux.org/packages/nvidiabl/)<sup><small>AUR</small></sup>.
...
<td>Использовать только один графический процессор для рендеринга.</td>
...
<td>Включить SLI и позволить драйверу автоматически выбрать режим рендеринга.</td>
...
<td>Включить SLI и использовать режим поочередного рендеринга кадров.</td>
...
<td>Включить SLI и использовать режим разделённого рендеринга кадров.</td>
...
<td>Включить SLI и использовать сглаживание SLI. Используйте в сочетании с полным сглаживанием сцены, для улучшения качества визуализации.</td>
...
**Обратите внимание:** Также, возможно отредактировать и переписать BIOS графического процессора, используя DOS (предпочтительнее) или с использованием Win32 окружения с помощью [nvflash](http://www.mvktech.net/component/option,com_remository/Itemid,26/func,select/id,127/orderby,2/page,1/)<sup>[[устаревшая ссылка](https://en.wikipedia.org/wiki/Wikipedia:Link_rot "wikipedia:Wikipedia:Link rot") 2013-05-25]</sup> и [NiBiTor 6.0](http://www.mvktech.net/component/option,com_remository/Itemid,26/func,select/id,135/orderby,2/page,1/)<sup>[[устаревшая ссылка](https://en.wikipedia.org/wiki/Wikipedia:Link_rot "wikipedia:Wikipedia:Link rot") 2013-05-25]</sup>. Преимущество данного способа в том, что вы можете поднять не только напряжение, но и повысить стабильность программных методов разгона, такие как Coolbits. [Руководство по модификации BIOS архитектуры Fermi (англ.)](http://ivanvojtko.blogspot.sk/2014/03/how-to-overclock-geforce-460gtx-fermi.html)
...
**Эта статья или раздел является кандидатом на объединение с [VDPAU](/index.php/VDPAU "VDPAU").**

**Причина:** Оставить только специфику и объединить с общей информаций в основной статье. (обсуждение: [Talk:NVIDIA (Русский)#](https://wiki.archlinux.org/index.php/Talk:NVIDIA_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)))
...
**Причина:** Остальная часть статьи не имеет смысла - нужен acpid или нет, советы противоречат друг другу. (обсуждение: [Talk:NVIDIA#Check power source (acpid)](https://wiki.archlinux.org/index.php/Talk:NVIDIA#Check_power_source_.28acpid.29))
...
Используйте [nvclock](https://aur.archlinux.org/packages/nvclock/)<sup><small>AUR</small></sup>, который доступен в [AUR](/index.php/AUR "AUR").
...
**Причина:** Ссылка [#Включение разгона](#.D0.92.D0.BA.D0.BB.D1.8E.D1.87.D0.B5.D0.BD.D0.B8.D0.B5_.D1.80.D0.B0.D0.B7.D0.B3.D0.BE.D0.BD.D0.B0) для описания всех опций _Coolbits_. (обсуждение: [Talk:NVIDIA (Русский)#](https://wiki.archlinux.org/index.php/Talk:NVIDIA_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)))
...
**Причина:** Не понятно, что эти шаги дают, нужны уточнения. (обсуждение: [Talk:NVIDIA (Русский)#](https://wiki.archlinux.org/index.php/Talk:NVIDIA_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)))
...
**Reason:** Verify that the BIOS related suggestions work and are not coincidentally set while troubleshooting. (Discuss in [Talk:NVIDIA (Русский)#'/dev/nvidia0' Input/Output error... suggested fixes](https://wiki.archlinux.org/index.php/Talk:NVIDIA_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)#.27.2Fdev.2Fnvidia0.27_Input.2FOutput_error..._suggested_fixes))
