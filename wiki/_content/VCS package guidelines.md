...
It is recommended to have following version format: _RELEASE.rREVISION_ where _REVISION_ is a monotonically increasing number that uniquely identifies the source tree (VCS revisions do this). If there are no public releases and no repository tags then zero could be used as a release number or you can drop _RELEASE_ completely and use version number that looks like _rREVISION_. If there are public releases but repo has no tags then the developer should get the release version somehow e.g. by parsing the project files.
