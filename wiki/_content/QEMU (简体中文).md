...
[安装](/index.php/%E5%AE%89%E8%A3%85 "安装") [qemu](https://www.archlinux.org/packages/?name=qemu)
...
运行 QEMU那需要一个硬盘镜像, 除非你直接从 CD-ROM 或网络引导 (and not doing so to install an operating system to a hard disk image).硬盘镜像是一个文件，它模拟储存在硬盘上的内容。
...
**Reason:** tunctl has been replaced by ip tuntap (Discuss in [Talk:QEMU (简体中文)#](https://wiki.archlinux.org/index.php/Talk:QEMU_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87)))
...
<table class="wikitable"><caption>配置文件选项</caption>
...
<td>qemu_${vm}_type</td>

<td>调用的 QEMU 二进制文件。如果指定，会被加到 `/usr/bin/qemu-` 之后，这个程序会被使用来启动虚拟机。也就是说，如你可以启动比如 qemu-system-arm 镜像使用 qemu_my_arm_vm_type="system-arm"。如果不指定，`/usr/bin/qemu` 会被使用。</td>
...
<td>启动的 QEMU 命令行。默认带有 `-name ${vm} -pidfile /var/run/qemu/${vm}.pid -daemonize -nographic`选项。</td>

</tr>

<tr>

<td>qemu_${vm}_haltcmd</td>

<td>安全关闭虚拟机的命令。我使用 `-monitor telnet:..` 以及通过发送`system_powerdown` 到 monitor，通过 ACPI 关闭我的虚拟机。你可以使用 ssh 或其他方法。</td>

</tr>

<tr>

<td>qemu_${vm}_haltcmd_wait</td>

<td>等待关闭虚拟机的时间。默认是30秒。rc-script 会在这个时间后 kill qemu 进程。</td>
...
Retrieved from "[https://wiki.archlinux.org/index.php?title=QEMU_(简体中文)&oldid=416998](https://wiki.archlinux.org/index.php?title=QEMU_(简体中文)&oldid=416998)"
