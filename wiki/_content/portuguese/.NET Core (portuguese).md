**Status de tradução:** Esse artigo é uma tradução de [.NET Core](/index.php/.NET_Core ".NET Core"). Data da última tradução: 2019-06-18\. Você pode ajudar a sincronizar a tradução, se houver [alterações](https://wiki.archlinux.org/index.php?title=.NET_Core&diff=0&oldid=575879) na versão em inglês.
...
Acredita-se que isso seja causado por um conflito entre as bibliotecas sdk mono e msbuild e as do dotnet core. Para corrigir isso, exporte o caminho manualmente no seu shell (substituindo o número da versão conforme necessário), por exemplo:
