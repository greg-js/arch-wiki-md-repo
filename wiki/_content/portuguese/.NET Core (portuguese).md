**Status de tradução:** Esse artigo é uma tradução de [.NET Core](/index.php/.NET_Core ".NET Core"). Data da última tradução: 2019-06-28\. Você pode ajudar a sincronizar a tradução, se houver [alterações](https://wiki.archlinux.org/index.php?title=.NET_Core&diff=0&oldid=576515) na versão em inglês.
...
Telemetria está habilitada por padrão, mas pode ser desabilitada colocando o seguinte em seu `.bashrc` (ou em um script sob `/etc/profile.d/` para todos usuários):
