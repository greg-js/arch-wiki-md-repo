**Status de tradução:** Esse artigo é uma tradução de [Install Arch Linux from existing Linux](/index.php/Install_Arch_Linux_from_existing_Linux "Install Arch Linux from existing Linux"). Data da última tradução: 2019-12-15\. Você pode ajudar a sincronizar a tradução, se houver [alterações](https://wiki.archlinux.org/index.php?title=Install_Arch_Linux_from_existing_Linux&diff=0&oldid=591863) na versão em inglês.
...
**Dica:** Para evitar baixar novamente todos os pacotes, considere seguir [Pacman/Dicas e truques#Cache do pacman compartilhado na rede](/index.php/Pacman/Dicas_e_truques#Cache_do_pacman_compartilhado_na_rede "Pacman/Dicas e truques") ou use a opção `-c` do *pacstrap* para usar o cache de pacotes do host.
