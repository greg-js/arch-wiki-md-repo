...
**Status de tradução:** Esse artigo é uma tradução de [Dm-crypt/Drive preparation](/index.php/Dm-crypt/Drive_preparation "Dm-crypt/Drive preparation"). Data da última tradução: 2019-11-21\. Você pode ajudar a sincronizar a tradução, se houver [alterações](https://wiki.archlinux.org/index.php?title=Dm-crypt/Drive_preparation&diff=0&oldid=582799) na versão em inglês.
...
*   [1 Apagando o disco com segurança](#Apagando_o_disco_com_segurança)
    ...
    *   [1.2 Métodos específicos do dm-crypt](#Métodos_específicos_do_dm-crypt)
        ...
        *   [1.2.3 Limpar o cabeçalho do LUKS](#Limpar_o_cabeçalho_do_LUKS)
...
Isto deve ser repetido para todas as partições criadas e que possuem sistema de arquivos. Por exemplo, ao configurar [LVM dentro do LUKS](/index.php/Dm-crypt/Encrypting_an_entire_system#LVM_on_LUKS "Dm-crypt/Encrypting an entire system"), esse procedimento tem que ser feito em todos os volumes lógicos.

#### Limpar o cabeçalho do LUKS
...
Apagar o cabeçalho do LUKS deletará a chave master PBKDF2-encrypted (AES), sais(salts) e assim em diante.
...
Um cabeçalho do LUKS1, com chave padrão de 256 bit, ocupa 1024KiB. É recomendado também sobrescrever os primeiros 4 KiB escritos pelo dm-crypt, então 1028 KiB têm que ser apagados. Isto é `1052672` Byte.
...
Em caso de dúvida, seja generoso e sobrescreva os primeiros 20 MiB(ou mais):
...
Ou use `shred`. Para por exemplo, sobrescrever os primeiros 20 MiB por 20 vezes:
...
Esta seção somente se aplica a um sistema criptografado. Depois que o disco foi sobrescrito com segurança, um apropriado particionamento tem que ser escolhido, levando em conta os requisitos do dm-crypt e efeitos que as várias escolhas terão na manutenção do sistema resultante.
...
*   Se os dipositivos LVM/RAID são criados em cima da camada criptografada, será possível adicionar, remover ou redimensionar os sistemas de arquivos da mesma partição criptografada deliberadamente, e somente uma chave ou senha será necessária para todos. Desde que a camada criptografada reside na partição física não será possível se aproveitar da habilidade do LVM e RAID para expandir para múltiplos discos.
...
Veja [dm-crypt/Criptografando todo um sistema#Partição de boot criptografada (GRUB)](/index.php/Dm-crypt/Encrypting_an_entire_system#Encrypted_boot_partition_(GRUB) "Dm-crypt/Encrypting an entire system").