...
O UEFI inicia aplicativos EFI, por exemplo [gerenciadores de boot](#Gerenciador_de_boot), [Shell UEFI](/index.php?title=UEFI_Shell&action=edit&redlink=1 "UEFI Shell (page does not exist)"), etc. Esses aplicativos geralmente são armazenados como arquivos na [partição de sistema EFI](/index.php/Parti%C3%A7%C3%A3o_de_sistema_EFI "Partição de sistema EFI"). Cada fornecedor pode armazenar seus arquivos na partição do sistema EFI sob a pasta `/EFI/*nome_fornecedor*`. Os aplicativos podem ser iniciados adicionando uma entrada de inicialização à NVRAM ou a partir do shell UEFI.
...
| Nome | Firmware | [Tabela de partição](/index.php/Partition_table "Partition table") | Multiboot | [Sistema de arquivos](/index.php/Sistema_de_arquivos "Sistema de arquivos") | Notas |
...
O gerenciador de boot só consegue acessar o sistema de arquivos no qual ele está instalado.[[3]](https://bugzilla.syslinux.org/show_bug.cgi?id=33) |
